...
| `Cargo.toml` | MODIFIED | Version bump to `0.6.0-dev.12` (D-188 clause (d): breaking change rides the `0.6.0-dev.11` → `0.6.0-dev.12` prerelease counter increment — no train bump; stable `0.6.0` was never released; retargeted 2026-07-29). S-639-1 delivery (F4) obligation — same PR/commit series as the code change; tracked as F3 story checklist item. |
| `CHANGELOG.md` | MODIFIED | `### Breaking Changes` entry under the `0.6.0-dev.12` section. SEMVER resolved: prerelease counter increment (D-188 clause (d); supersedes clause (d)'s 0.6→0.7 train-bump reasoning — retargeted 2026-07-29). S-639-1 delivery (F4) obligation — same PR/commit series as the code change; tracked as F3 story checklist item. |
...
| `CLAUDE.md` | MODIFIED | S-288-pr4 dispatch-fork gotcha at line 248 (`"absent → platform path byte-for-byte unchanged"`) becomes stale — after #639 the platform path is no longer byte-for-byte unchanged when `--field` or `--on-behalf-of` are present (it exits 64 pre-flight instead). S-639-1 delivery (F4) obligation — same PR/commit series as the code change; tracked as F3 story checklist item. Distinct from the Item 3 CLAUDE.md change (toolchain masking gotcha); both land on develop but in separate story commits. |
| `docs/adr/0014-jsm-request-type-dispatch.md` | MODIFIED | Line 161 carries the same `"byte-for-byte unchanged"` claim about the platform path. Requires an amendment note (not a new ADR — see §4 "no new ADR warranted"). S-639-1 delivery (F4) obligation — same PR/commit series as the code change; tracked as F3 story checklist item. |
...
amendment note. S-639-1 delivery (F4) obligation — same PR/commit series as the code
change; tracked as F3 story checklist item.
...
1. ~~**SEMVER impact of #639**~~: RESOLVED — D-188 clause (d): breaking change rides the
   `0.6.0-dev.11` → `0.6.0-dev.12` prerelease counter increment (no train bump — stable `0.6.0` was never released; retargeted 2026-07-29); version becomes `0.6.0-dev.12`. `Cargo.toml` bump and
