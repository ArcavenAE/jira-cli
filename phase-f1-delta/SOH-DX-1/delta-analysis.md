...
| `Cargo.toml` | MODIFIED | Version bump to `0.7.0-dev.1` (DEC-188 clause (d): breaking change rides the 0.6→0.7 train bump). S-639-1 delivery (F4) obligation — same PR/commit series as the code change; tracked as F3 story checklist item. |
| `CHANGELOG.md` | MODIFIED | `### Breaking Changes` entry under the 0.7.0 section. SEMVER resolved: MINOR bump (DEC-188 clause (d)). S-639-1 delivery (F4) obligation — same PR/commit series as the code change; tracked as F3 story checklist item. |
