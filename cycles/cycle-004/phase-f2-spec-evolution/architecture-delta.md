...
bundle (issues #759, #760; D-334; A-PA-LOW-001). It is structured as a delta: only what
...
| 6 | LOW | `store_pair`'s "best-effort remove any stale `*.tmp-*` siblings before writing" was a blanket delete — two concurrent `jr` processes (login racing refresh) for the same profile could have process B's cleanup delete process A's in-flight temp file, breaking A's rename. `refresh_coordinator.rs` single-flights only within one process. | ADR-0021 §3 changes the cleanup to an age-gated delete (`STALE_TMP_THRESHOLD` = 30s) — a temp file younger than the threshold is assumed to be another process's in-flight write and is left alone; only a genuinely abandoned (crashed) prior attempt is removed. The residual cross-process concurrency boundary is stated explicitly (no new locking added; out of D-334's scope). |
