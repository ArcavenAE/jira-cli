...
2. Recorded 8 new Decisions Log entries, D-312 through D-319 (collision-checked: highest pre-existing ID was D-311, confirmed via corpus-wide grep — no collision): cycle-003 opened; `auth_method` as first-class intrinsic profile property; additive `env`/role tag on profile; per-profile credentials (api-token symmetric with OAuth, one-time keychain migration); API-token auth stays coequal/first-class; un-defer ADR-0011; 2LO service-account CI deferred; Device Authorization Grant rejected as design basis.
...
| Agent | Task | Output |
|-------|------|--------|
| state-manager | Initialize cycle-003 in STATE.md (frontmatter, 8 Decisions Log entries D-312..D-319, Phase Progress + Current Phase Steps rows, Session Resume Checkpoint); create cycle-003 scaffolding (`burst-log.md`, `session-checkpoints.md`); archive final cycle-002 checkpoint; commit + push to factory-artifacts | `STATE.md`; `cycles/cycle-003/burst-log.md` (this file); `cycles/cycle-003/session-checkpoints.md`; `cycles/cycle-002/session-checkpoints.md` |
...
2. Recorded 6 new Decisions Log entries, D-320 through D-325 (collision-checked: highest pre-existing ID was D-319, confirmed via corpus-wide grep — no collision): F1 delta-analysis APPROVED (D-320); refresh override removed, `auth refresh` always follows the profile's intrinsic `auth_method` (D-321, resolves F1 Open Question 8); `auth logout` is session-clear only / non-destructive, `auth remove` remains full-delete (D-322, resolves Open Question 6); explicit `--api-token` flag added to `auth login` (D-323, resolves Open Question 5); `env` tag surfaced as an `auth list` table column + `auth status`/JSON, BC-1.6.046 snapshot updated (D-324, resolves Open Question 7); accepted architect recommendations — no version bump, ADR-0011 amended in place, one combined new ADR-0020, `--oauth` deprecated indefinitely with no hard removal date (D-325).
...
**Closes:** nothing new. Resolves F1 Open Questions 5/6/7/8 (D-321..324) and F1 Open Questions 1/2/3 + deprecation window (D-325). **Does NOT close:** any cycle-002 standing Drift/Standing Items — all carried forward unchanged in STATE.md. Deferred to F2 (noted, not blocking): input-hash drift check at the F2 gate; NFR-S-B nfr-catalog vs CLAUDE.md doc-drift reconciliation.
...
| Agent | Task | Output |
|-------|------|--------|
| state-manager | Record F1 delta-analysis human-gate APPROVAL in STATE.md (frontmatter, 6 Decisions Log entries D-320..D-325, Phase Progress + Current Phase Steps rows, Session Resume Checkpoint); commit the F1 deliverable; archive the prior checkpoint; commit + push to factory-artifacts | `STATE.md`; `cycles/cycle-003/burst-log.md` (this file); `cycles/cycle-003/session-checkpoints.md`; `cycles/cycle-003/phase-f1-delta-analysis/delta-analysis.md` |
...
5. **New ADR authored**: `specs/architecture/decisions/ADR-0020-per-profile-credential-ownership-env-tagging-and-oauth-default-at-creation.md` — combined ADR covering per-profile credential layout (D-315), `env` tag (D-314), and OAuth-default-at-creation (D-313), per D-325(c).
6. **ADR-0011 amended** (Deferred → Accepted, D-317/D-325(b)) — status change, Trigger-met rationale, Sequencing section binding the F4 newtype call-site sweep to land after the credential-storage/migration stories. **Routing correction this burst:** `docs/adr/0011-type-level-profile-fence.md` is a MAIN-REPO tracked file on `develop`, not a `.factory` artifact — committing the amendment directly to `develop` (bypassing PR review) or to `factory-artifacts` would violate the branch-workflow invariant and the F2/F4 artifact-boundary. The amended content is instead **STAGED** at `cycles/cycle-003/phase-f2-spec-evolution/adr-0011-amendment-staged.md` (committed to factory-artifacts this burst) with an explicit header marking it not-yet-applied; `develop`'s working-tree copy of `docs/adr/0011-type-level-profile-fence.md` was reverted (`git restore`) to keep `develop` clean. The F4 story `S-cycle3-adr0011-newtype` MUST apply this staged amendment to the main-repo file as part of its implementation PR.
7. **nfr-catalog.md updated**: NFR-SCA-2 routing DEFER → FIX-IN-CYCLE (design accepted via ADR-0011 amendment, D-317; F4 implementation pending); NFR-S-B routing SECURITY-DECIDE → RESOLVED (doc-drift fix — CLAUDE.md already documents `JR_AUTH_HEADER` as `#[cfg(debug_assertions)]`-gated per SD-002; the row's prior description was stale). Neither change adds/removes an NFR row (`total_nfrs` unchanged at 42).
...
**Closes:** nothing at the human-gate level yet (F2 authoring is complete, the gate itself has not run). Resolves the F2 spec-authoring obligations from D-317/D-325 (ADR-0011 amendment, new ADR-0020) and D-313/314/315/321/322/323/324 (BC delta encoding each decision).
...
9. **Recorded D-326 and D-327** (resolves adversary finding M-3 — undocumented human decisions from the fix round): D-326 (no-copy migration, supersedes D-325(a)'s "additive keychain keys + lazy migration" language — the lazy-migration clause is reversed, the additive-keychain-keys part stands); D-327 (env-var non-interactive-only trigger, refines D-313). D-325(a) annotated SUPERSEDED in the Decisions Log (not removed).
...
**Outcome:** cycle-003 (`auth-profile-dx`) Phase F2 spec-evolution FIX round is COMPLETE: the no-copy migration redesign (D-326) closes the pass-1 CRITICAL migration-lockout; all pass-2 HIGH/MED seam issues are closed; D-326/327 recorded and D-325(a) annotated superseded. BC count 731→733, VP count unchanged at 41, holdouts unchanged at 106. Adversary pass-3 (convergence check) and the human F2 gate are NEXT.
...
**Codifications:** BC-1.4.032/033's no-copy redesign is final pending F4 implementation; ADR-0020 amended in place to reflect Decision 8 (airtight non-interactive OAuth guard) and the D-327 env-var trigger refinement; ADR-0011 amendment (staged) unaffected by this burst.

**Closes:** adversary pass-1 finding C-1; adversary pass-2 findings H-1, H-2, M-1, M-2, M-3 (M-3 closed via this burst's D-326/D-327 recording). Does NOT close: DEC-NAMESPACE-COLLISION-RISK monitoring (still standing, re-verified clean this burst — highest allocated ID is now D-327), or any pre-existing cycle-002 Drift/Standing item.
...
## Burst: Burst 7 — F2 human approval gate APPROVED (D-328); all 4 LOW residuals swept; cycle-003 advances F2 → F3 (2026-09-01)
...
2. **Fixed all 4 LOW residuals this burst** (verified via `git -C .factory diff` before staging):
   ...
   - **NEW-1** (surfaced by the fresh-context consistency audit run at the F2 gate, not by adversary pass-4): added a `D-326` traceability citation to BC-1.4.032/033's `Trace:` lines in `bc-1-auth-identity.md` (both now read "F2-gate fix (2026-09-01, D-326, HUMAN DECISION)") and to ADR-0020 §Decision 2's heading (now "...HUMAN DECISION, D-326)") — the no-copy redesign was previously traceable only as an undated "HUMAN DECISION" with no DEC-ID anchor in either document.
...
**Outcome:** cycle-003 (`auth-profile-dx`) Phase **F2 is CLOSED — human-approved (D-328)**. All 4 LOW residuals fixed. BC count unchanged at 733, VP count unchanged at 41, holdouts unchanged at 106. Pipeline transitions **PAUSED → ACTIVE**, phase **F2 → F3**. Phase F3 (incremental stories) is the immediate next activity.
...
**Trigger:** Burst 7 closed F2 (human-approved, D-328) and advanced the pipeline to F3 (incremental stories), ACTIVE. This burst is the F3 story-decomposition dispatch itself — MANIFEST → CREATE → INTEGRATE sub-bursts, followed by a fresh-context consistency audit and a pre-gate governance fix, all landing in one commit per the Single-Commit Burst Protocol.

**Actions taken:**
1. **MANIFEST:** `decomposition-manifest.md` authored — proposes 7 stories (renaming F1's preliminary `S-cycle3-percred-migration` to `S-cycle3-credential-absence-guard` per D-326's no-copy redesign), a BC coverage matrix (**24/24 BCs — 14 new + 10 amended — assigned to exactly one story, zero duplicates, zero orphans**), and a VP coverage matrix (**9/9 VPs — VP-AUTHDX-001..009 — assigned, zero orphaned**).
...
## Burst: Burst 9 — F3 human approval gate APPROVED (D-329); cycle-003 advances F3 → F4 (2026-09-01)
...
3. **Recorded D-329** in the Decisions Log — full text captures the approval, the coverage/graph/schedule summary, and both ratified items.
...
**Outcome:** cycle-003 (`auth-profile-dx`) Phase **F3 (incremental stories) is APPROVED at the human gate (D-329).** All 7 stories are `status: ready`. Phase **F4 (delta implementation) is now ACTIVE** — Wave 1 (`S-cycle3-env-tag` + `S-cycle3-percred-storage`, parallel) is the next dispatch. Pipeline stays **ACTIVE**; phase advances **F3 → F4**.
...
## Burst: Burst 10 — F4 Wave 1 story 1 (`S-cycle3-env-tag`) delivered + squash-merged to `develop`; D-330 auto-merge authorization recorded (2026-09-02)

**Parent-commit:** the F3-gate-approval burst commit (v3.39, D-329 + phase F3→F4) — most recent prior `.factory/` commit on `factory-artifacts`. `develop` tip: was `87f17aff`, now `4d0ae2d5` (PR #752 merge commit `4d0ae2d56e880a7a7645954f6da6193c5c62564e`, this burst).
...
**Outcome:** cycle-003 (`auth-profile-dx`) Phase **F4 (delta implementation) is ACTIVE.** Wave 1: **1 of 7 stories merged** (`S-cycle3-env-tag`, PR #752, `develop` @ `4d0ae2d5`). Wave 1 story 2/2 (`S-cycle3-percred-storage`) is next. Human authorized auto-merge for cycle-003 F4 story PRs (D-330). Pipeline stays **ACTIVE**; phase stays **F4**.

**NEXT:** rebase the `S-cycle3-percred-storage` worktree onto the new `develop` tip (`4d0ae2d5`) and dispatch its per-story TDD delivery. On CI green + dual-review convergence, auto-merge per D-330 (pause only for material/escalated findings). On Wave 1 full completion, proceed to Wave 2 (`S-cycle3-credential-absence-guard`, P0, HIGH-risk).

**Codifications:** D-330 recorded this burst (auto-merge authorization for cycle-003 F4 story PRs). No BC/VP content added or changed — this is a delivery/governance burst, not a spec-authoring one.
...
- `total_nfrs`: unchanged at 42.
- DEC IDs: 329 → **330** (D-330 recorded this burst).
...
3. **Wave 2 adversary review returned 2 findings, both non-blocking — verdict SAFE TO PASS — and both dispositioned this burst:**
   ...
   - Adversary additionally confirmed, as verification rather than new findings: the migration recovery loop CLOSES (a successful login writes exactly the pair the loader subsequently reads); D-326's no-copy invariant holds at every production call site; `load_oauth_tokens` (OAuth) is unaffected; the exit-64 error surfaces cleanly with byte-exact BC text end-to-end.
...
## Burst: Burst 13 — F4 Wave 3 COMPLETE: `S-cycle3-remove-logout-semantics` delivered + squash-merged; SEC-1 HIGH found+fixed pre-merge; both Wave-2-carried obligations closed; D-331 recorded (2026-09-02)
...
- `total_stories`: unchanged at **168** (no story-file status flip this burst — no new spec content added).
- `total_nfrs`: unchanged at 42.
- DEC IDs: **330 → 331** (D-331 recorded this burst).
...
1. **`S-cycle3-adr0011-newtype` (Wave 4, 13 pts) delivered end-to-end via per-story TDD, including a security review:** threaded a `Profile(String)` newtype hard-fence through ~38 signatures / ~259 call sites (`config.rs`, `cache.rs`, `api/client.rs`, `api/auth.rs`, and callers), un-deferred ADR-0011 (D-317) and **applied the staged amendment** to `docs/adr/0011-type-level-profile-fence.md` (Status Deferred→Accepted — verified this burst by reading the merged file: the Status section now reads "Accepted (amended 2026-09-01, cycle-003 `auth-profile-dx`, D-317...)"). Behavior-preserving: `Debug`/`Display`/`AsRef<str>` byte-identical to the prior `&str` call sites; `compile_fail` doctest fence present and load-bearing. PR #758 opened against `develop`, then squash-merged — merge commit `b7e513f9`; `develop` tip `5e9dba8a` → `b7e513f9`. Reviews: local **APPROVE**, security **PASS**, AI (pr-reviewer) **APPROVE-WITH-NITS** — 1 new LOW finding: `src/profile.rs` module doc and the ADR-0011 body retain residual "sweep not done" language that now contradicts the merged reality (non-blocking, tracked as a follow-up).
2. **`S-cycle3-oauth-default-creation` (Wave 4, 13 pts, P0) delivered end-to-end via per-story TDD, including a security review:** added the interactive OAuth-default picker at profile creation (D-313), an explicit `--api-token` flag (D-323), and the D-327 non-interactive-only env-var-suppression guard (`JR_EMAIL`/`JR_API_TOKEN` presence suppresses the picker only under `--no-input`/non-TTY, never on an interactive TTY). PR #761 opened against `develop` (rebased onto `b7e513f9`), then squash-merged — merge commit `b70dd6f4`; `develop` tip `b7e513f9` → `b70dd6f4` (current tip). Reviews: local **APPROVE-WITH-NITS**, security **PASS-WITH-NOTES**, AI (pr-reviewer) **APPROVE-WITH-NITS**.
3. **Two MEDIUM findings found and FIXED pre-merge on `S-cycle3-oauth-default-creation`:** (1) a CWE-400 (uncontrolled resource consumption) gap in the interactive picker's TTY guard; (2) a missing VP-AUTHDX-001 regression test. Both closed in the same PR before merge, verified present in the final reviewed diff — the first live test of D-331's "every HIGH/MEDIUM finding addressed" clause against MEDIUM-severity findings specifically (PR #757's SEC-1 exercised the HIGH clause); both resolved without a human pause, confirming D-331 behaves as specified.
...
**Outcome:** cycle-003 (`auth-profile-dx`) Phase **F4 (delta implementation) is ACTIVE.** **Wave 4 is COMPLETE: 6 of 7 stories merged** (`S-cycle3-env-tag` PR #752, `S-cycle3-percred-storage` PR #755, `S-cycle3-credential-absence-guard` PR #756, `S-cycle3-remove-logout-semantics` PR #757, `S-cycle3-adr0011-newtype` PR #758, `S-cycle3-oauth-default-creation` PR #761; `develop` @ `b70dd6f4`). The ADR-0011 amendment is now APPLIED. Two MEDIUM findings found and fixed pre-merge on PR #761 confirm D-331's MEDIUM-finding clause behaves as specified. Wave 4 integration gate is running; Wave 5 (`S-cycle3-chosen-flow-reconcile`, the final cycle-003 story) is next. Pipeline stays **ACTIVE**; phase stays **F4**.

**NEXT:** run/complete the Wave 4 integration gate (mirror the Wave 1–2 gate shape). On PASSED, stand up a worktree for `S-cycle3-chosen-flow-reconcile` (Wave 5, final story) rebased onto `b70dd6f4` and dispatch its per-story TDD delivery — scope is the D-321 refresh-override removal in `cli/auth/mod.rs::chosen_flow_for_profile`. On CI green + reviewer MERGE RECOMMENDATION + every HIGH/MEDIUM finding addressed, auto-merge per D-331. On Wave 5 merge, cycle-003 F4 is COMPLETE (7/7 stories shipped) — proceed to F5. Get an explicit human decision before acting on the open demo-recording question. Address the 4 tracked follow-ups in a future maintenance pass.

**Codifications:** No new DEC ID this burst. D-331's MEDIUM-finding clause exercised for the first time (PR #761) and confirmed to behave as specified. No new BC/VP content added or changed — this is a delivery/review-finding/governance burst, not a spec-authoring one.

**Closes:** `S-cycle3-adr0011-newtype` and `S-cycle3-oauth-default-creation` as open work (delivered/merged); **Wave 4 as a whole** (both its stories merged); the ADR-0011-staged-not-applied item (now applied and verified); the two found+fixed MEDIUM findings on PR #761 (verified). **Does NOT close:** the Wave 4 integration gate (running, report pending); D-321's refresh-override removal (Wave 5's sole remaining obligation); the `STORY-INDEX.md` grep-count residual (still flagged); the standing LOW oauth-migration-write drift item; the 4 newly tracked follow-ups (F1 MED, `JR_OAUTH_CODE` gating, adr0011 doc-drift, `remove.rs` doc-comment — all deferred to a future maintenance pass); the OPEN demo-recording/demo-retention human question (NOT decided); any other pre-existing Drift/Standing item.
...
- `total_stories`: unchanged at **168** (no story-file status flip this burst — no new spec content added).
- `total_nfrs`: unchanged at 42.
- DEC IDs: unchanged at 331 (no new decision recorded this burst; D-331 applied, not amended).
...
3. **D-321's Decisions Log row updated** from "Not yet implemented — Wave 5's sole obligation" to **IMPLEMENTED**, citing PR #762 @ `1dfcd013`. **D-330's applied-PR list extended** to include PR #762.
...
**Codifications:** No new DEC ID this burst (D-321's row updated to IMPLEMENTED, not superseded; D-330's applied-PR list extended). No new BC/VP content added or changed — delivery/review-finding/governance bookkeeping only.
...
- `total_nfrs`: unchanged at 42.
- DEC IDs: unchanged at 331 (no new decision recorded this burst; D-321 row updated in place, not superseded — same ID).
...
3. **MED-1 (BC-1.1.016 ↔ D-321 spec/code contradiction) — RESOLVED, spec-only, no code change.** `.factory/specs/prd/bc-1-auth-identity.md` BC-1.1.016 originally stated Precondition 2 as an OR of "(a) `--oauth` passed on `login` OR `refresh`" vs "(b) implicit oauth-method-profile refresh" — self-contradicting D-321/BC-1.2.051 (both shipped in PR #762, Burst 15), which make `--oauth` uniformly INERT on `refresh` (only the profile's stored `auth_method` governs). Reconciled: Precondition 2(a) narrowed to a **`login`-only** trigger; 2(b) restated as depending SOLELY on the profile's resolved `auth_method`, regardless of whether `--oauth`, an inert `--api-token`, or no flag accompanies `refresh`; EC-1.1.016-1/EC-1.1.016-2 and the BC's own `**Trace**` field updated to cross-reference D-321 and the shipped, test-pinned behavior (`tests/auth_chosen_flow_reconcile.rs::test_ac_002_...`). Symmetric with the already-correct EC-1.1.016-3 treatment of `--api-token` on an oauth-method profile. Zero BC/VP/holdout count change (edit is prose-only within an existing BC body).
...
2. **LOW-1 (`S-cycle3-chosen-flow-reconcile` description overclaim) — FIXED, both BC-file and STORY-INDEX halves.** The row (both tables) claimed `chosen_flow_for_profile` was "removed entirely" / "not merely simplified" — factually wrong: the function still exists in `src/cli/auth/mod.rs` and is still called from `refresh.rs`; only the per-call `oauth_override: bool` parameter and its branch were removed (D-321). Corrected to "simplified to single-argument form" in both tables, with an explicit parenthetical noting the function's continued existence and call site. The BC-file half (`specs/prd/bc-1-auth-identity.md` BC-1.2.048) was already corrected by product-owner in this same burst (uncommitted edit, committed alongside this one).
...
## Burst: Burst 22 — RELEASE v0.7.0-dev.4 + CYCLE CLOSE: human authorized and executed the dev release; cycle-003 auth-profile-dx CLOSED (D-333) (2026-09-03)

**Parent-commit:** the F7-CLOSE burst commit (v3.51, `e83c511b`) — most recent prior `.factory/` commit on `factory-artifacts`. `develop` tip advances this burst: **`c9bb74f4` → `42e92b46`** (version-bump PR #767 squash-merged).

**Trigger:** the human explicitly authorized "Proceed with the Dev release" this session, following Phase F7's human-approved CONVERGENCE (D-332) and the pre-release docs fix (PR #766, `c9bb74f4`) recorded at the prior burst. This is the one remaining human action for cycle-003 — release execution — held open at the prior burst.

**Note on burst-log continuity:** Burst 21 (F7 human approval gate APPROVED — CONVERGED, D-332; HIGH-3/LOW-2/LOW-3 resolved via PR #766) was recorded in STATE.md (frontmatter, Phase Progress, Decisions Log, Drift/Standing) at commit `e83c511b` but its full prose narrative was never backfilled into this burst-log.md file — that backfill remains an outstanding, non-blocking documentation debt (tracked here, not fixed by this burst; the authoritative record of Burst 21's content is `git show e83c511b:STATE.md` plus this file's own Burst 20 entry for the audit context that preceded it).

**Actions taken:**

1. **Release cut.** Version-bump PR #767 squash-merged to `develop` (`c9bb74f4` → `42e92b46`): `Cargo.toml`/`Cargo.lock` bumped to `0.7.0-dev.4`; `CHANGELOG.md` `[Unreleased]` section rolled to `[0.7.0-dev.4] - 2026-09-03`. Annotated tag `v0.7.0-dev.4` pushed at `42e92b46`. GitHub Actions `release.yml` run `33769389700` triggered and concluded **SUCCESS**. GitHub prerelease published `2026-09-03T14:58:38Z` with 10 assets (5 targets × archive + sha256: `x86_64`/`aarch64` `linux-gnu`, `x86_64`/`aarch64` `apple-darwin`, `x86_64` `windows-msvc`).
2. **STATE.md refreshed via one full-content Write (v3.51 → v3.52), Single-Commit Burst Protocol (D-247), no Edit chain.** Frontmatter: `pipeline` → **`RELEASED`** (mirrors the exact value cycle-002's own release-cut burst used, v3.29 / commit `198c37fd`); `phase` stays **F7** (release-cut does not advance the phase field, same precedent); `activation_head`/`activation_version` advance `c9bb74f4` → `42e92b46` / `v0.7.0-dev.3` → `v0.7.0-dev.4`; `cycle_003_status` → CLOSED + RELEASED. New **D-333** recorded (human release authorization + execution). Phase Progress: new `RELEASE v0.7.0-dev.4 (cycle-003)` row added (RELEASED — SHIPPED). Current Phase Steps: two new rows appended (release cut; cycle-003 CLOSED), oldest two of the prior five dropped (F6 regression + Kani-substitution row and the F7 pre-gate audit row — both already fully detailed in Bursts 19/20). Session Resume Checkpoint replaced with the CLOSED + RELEASED position; the prior F7-CONVERGED/RELEASE-HELD checkpoint (v3.51) queued for archival to `cycles/cycle-003/session-checkpoints.md` (not yet performed this burst — see STATE.md's own "Note (this burst)" caveat, consistent with the pending v3.49/v3.50 archival noted at prior bursts). Convergence Status / Concurrent Cycles / Constraints Carried Forward paragraphs updated to record CLOSED + RELEASED and "no cycle currently has open work." Drift/Standing Items: one new top entry added for this burst; all prior entries (Burst 21 F7-close, the bookkeeping input-hash cascade, the Burst 20 F7 pre-gate audit, and everything below) carried forward **verbatim** — zero resolutions to those beyond this burst's own release/close bookkeeping.
3. **S-7.02 cycle-closing checklist satisfied.** Per the checklist, the two process-gap deferral candidates recorded at Burst 21 (STORY-INDEX index-currency gap; story-template-compliance gap) were confirmed present in STATE.md Drift/Standing as justified deferrals (target: a future SELF-IMPROVEMENT/maintenance cycle) and are now additionally codified in a **newly-created `cycles/cycle-003/lessons.md`** (mirroring cycle-002's `lessons.md` precedent, same S-7.02 trigger point) — two Process-Level entries, each marked `[deferral]` with the same justification and target as the STATE.md Drift/Standing text. Cycle-003 is CLOSED only with this satisfied; it is now satisfied.
4. **Commit hygiene sweep.** Staged this burst: `STATE.md`, `cycles/cycle-003/burst-log.md` (this file), `cycles/cycle-003/lessons.md` (new). Explicitly NOT staged, per standing instruction and unrelated to cycle-003 (pre-existing dirty since before this session started): `regression-state.json`, `sidecar-learning.md`, and the modified `cycles/cycle-003/code-delivery/S-cycle3-env-tag/demos/AC-004-005-auth-list-table-env-column.gif`.
5. **Count guards.** No BC/VP/holdout/story content changed this burst (pure release + bookkeeping burst) — `scripts/check-bc-cumulative-counts.sh` and `scripts/check-spec-counts.sh` were not re-run (no spec content touched); counts carried forward unchanged: 733 BCs / 41 VPs / 106 holdouts / 168 stories.

**Adversary verdict:** N/A — this burst is a release-execution + cycle-close bookkeeping burst (state-manager only), not an adversarial-review pass. Phase F7's adversarial/convergence work was already completed and human-approved at the prior burst (D-332).

**NEXT:** optional post-pipeline session review (`/vsdd-factory:session-review`); otherwise the pipeline is idle — no active cycle — awaiting the human's direction on the next feature bundle or maintenance cycle. The `S-PG-*` SELF-IMPROVEMENT backlog (now carrying 5 justified-deferral candidates across cycle-002 and cycle-003) is a natural candidate for that next cycle.

**Codifications:** **D-333** — human authorized and executed the cycle-003 dev release v0.7.0-dev.4; cycle-003 CLOSED. No BC/VP/holdout added, removed, or renumbered by this burst (733/41/106/168 all unchanged).

**Closes:** the one remaining open item from Burst 21 — the release trigger itself (RELEASE HELD → RELEASED). cycle-003 (`auth-profile-dx`) is now fully **CLOSED**. **Does NOT close:** any of the tracked non-blocking follow-ups (MED-1 VP-count re-verification, LOW-4/LOW-6 doc nits, the 4-story template-compliance gap, the 6-file bookkeeping input-hash cascade, the Burst-21-backfill-into-this-file debt noted above, or any cycle-002 standing item) — all carried forward verbatim to a future maintenance/self-improvement cycle, now also indexed in `cycles/cycle-003/lessons.md`.

### Counts reconciled this burst

- BCs: 733 (unchanged). VPs: 41 (unchanged). Holdout scenarios: 106 (unchanged). `total_stories`: unchanged at **168**.
- DEC IDs: 331 → **333** (new: D-333).
- `develop` HEAD: `c9bb74f4` → **`42e92b46`** (version-bump PR #767).
- `activation_version`: `v0.7.0-dev.3` → **`v0.7.0-dev.4`**.
...
| Agent | Task | Output |
|-------|------|--------|
| state-manager | Record the human-authorized release (PR #767 @ `42e92b46`, tag `v0.7.0-dev.4`, `release.yml` run `33769389700` SUCCESS, GitHub prerelease 10 assets/5 targets); refresh STATE.md (frontmatter, Phase Progress, Current Phase Steps, new D-333, Convergence Status/Concurrent Cycles/Constraints, Session Resume Checkpoint, Drift/Standing); create `cycles/cycle-003/lessons.md` codifying the two S-7.02 process-gap deferrals; append this burst-log entry; commit + push to `factory-artifacts` (Single-Commit Burst Protocol, D-247) | `STATE.md`; `cycles/cycle-003/burst-log.md` (this file); `cycles/cycle-003/lessons.md` (new) |

**Files touched (Dim-1): 3 unique files this burst, all committed in the state-manager's own single atomic commit**

- `STATE.md`
- `cycles/cycle-003/burst-log.md`
- `cycles/cycle-003/lessons.md` (new)

**Dim-2 Attestation:** No BC/VP/holdout content changed this burst (pure release-execution + bookkeeping burst). Counts unchanged: 733 BCs / 41 VPs / 106 holdouts / 168 stories.
...
**Dim-6 Attestation:** Source code changed via version-bump PR #767 only (`Cargo.toml`, `Cargo.lock`, `CHANGELOG.md`) — no functional `src/`/`tests/` change. `develop` HEAD advances `c9bb74f4` → `42e92b46`.

**Dim-7 Attestation:** `release.yml` run `33769389700` concluded **SUCCESS** (build/publish across 5 targets, GitHub prerelease published with 10 assets). No `scripts/check-*` re-run this burst (no spec content changed).
