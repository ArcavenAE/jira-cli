...
| ID | Severity | Classification | Title | Status | Notes |
|----|----------|----------------|-------|--------|-------|
| ADV-P27-INFO-001 | INFO | CI-caching / cache-key-desync | `.github/workflows/ci.yml :: msrv` — `Swatinem/rust-cache` runs before the step-scoped `RUSTUP_TOOLCHAIN` takes effect, so its cache key derives from `rustc -vV` resolved via `rust-toolchain.toml` (stable) while `cargo check` builds under 1.85.0; pre-fix both were stable, the fix newly desynchronizes them. No correctness impact — cargo's fingerprint includes the rustc version and rust-cache's default key includes the job id; cost is cache-hit rate only | OPEN — no action; noted for future msrv-runtime investigation | |
| ADV-P27-INFO-002 | INFO | docstring-overclaim / comment-satisfiable-assertion | `tests/ci_gate_completeness.rs::test_verify_test_job_has_zero_test_floor` — two of eight assertions match substrings occurring more than once (`FAIL (POL-11)` 3×, all echo diagnostics; `exit 1` 3×, all operative), so each is in principle comment-satisfiable; the other six occur exactly once and only in operative position, so the guard holds regardless | CLOSED — fix round 12 (docstring scoped; `exit 1` deliberately left coarse with reasoning documented inline) | Concurs with ADV-P28-INFO-001 |
| ADV-P27-INFO-003 | INFO | disclosed-limitation / MSRV-scope | `.github/workflows/ci.yml :: msrv` scope comment; `CLAUDE.md` "No let-chains" — MSRV floor is enforceable only over lib + bins; a let-chain in an inline `#[cfg(test)]` module in `src/` or anywhere in `tests/` is not caught. Alternative is blocked — `--all-targets` fails at 1.85 on `wiremock 0.6.5`. Accurately disclosed in both places | OPEN — disclosed limitation, not a defect | Concurs with ADV-P29-INFO-001 |
| ADV-P27-INFO-004 | INFO | `[process-gap]` orchestrator-briefing-inaccuracy | Orchestrator dispatch briefing described `tests/ci_gate_completeness.rs` as "new, 186 lines"; it is a MODIFY (+186 to a pre-existing 937-line file on origin/develop). Derived from `git diff --stat` without checking base-branch existence. Pass-27 scoped via `git diff` and was not misled | OPEN — recorded as drift item BRIEFING-DERIVED-FROM-DIFFSTAT-MISCLASSIFIES-FILES | |
...
- **Post-capture routing:** INFO-002 CLOSED fix round 12 (docstring scoping); INFO-001/003 OPEN as disclosed limitations; INFO-004 OPEN as BRIEFING-DERIVED-FROM-DIFFSTAT-MISCLASSIFIES-FILES drift item
...
| ADV-P28-INFO-001 | INFO | docstring-overclaim / comment-satisfiable-assertion | `tests/ci_gate_completeness.rs::test_verify_test_job_has_zero_test_floor` docstring (~:785) claimed "every assertion targets a form that can ONLY appear in the operative command, never in a prose comment or diagnostic echo." Three of its own assertions do not meet that bar — `FAIL (POL-11)`, `Check passed:`, `exit 1`. The two named under "Specifically:" are correctly operative-only. An overclaiming guard docstring is how a maintainer stops scrutinising the guard | CLOSED — fix round 12 (scoped to "TWO of the assertions below"; the three weaker pins now explicitly named as coarser-grained) | Concurs with ADV-P27-INFO-002 |
| ADV-P28-INFO-002 | INFO | comment-inaccuracy / non-applicable-failure-mode | `.github/workflows/ci.yml :: test`, `CARGO_TERM_COLOR: never` comment (~:57) + mirrored test docstring (~:780) — the stated failure mode does not apply to libtest: it emits the summary as a plain `"\ntest result: "` prefix and colorizes only the `ok`/`FAILED` token after it, so `grep -E "^test result: "` anchors correctly even under `--color=always`. The override is harmless and defensively reasonable, but the comment asserts a concrete break that would not occur here | OPEN — suggested rewording to defensive-hardening framing; not applied in round 12 | |
| ADV-P28-INFO-003 | INFO | dead-code / vacuous-condition | `tests/ci_gate_completeness.rs` ~:533-534, ~:610, ~:614 — three dead sub-conditions: `line.starts_with("    if:") && !line.starts_with("        ")` (and the `"    steps:"` variant) — a string beginning with exactly four spaces cannot begin with eight, so the second conjunct is unconditionally true. `step_if_indent` (~:680) is a `let mut` only ever assigned the constant `8`. Harmless; implies a distinction the code isn't making | OPEN — not applied in round 12 | |
| ADV-P28-INFO-004 | INFO | spec-declaration / undeclared-flag-addition | `.github/workflows/ci.yml :: msrv`, `cargo check --all-features --locked` — `--locked` was an undeclared addition; AC-2/AC-3 specified only the SHA swap, the `with: {toolchain}` input, and the `env: {RUSTUP_TOOLCHAIN}`. Strictly beneficial (reinforces AC-8's exact `=7.2.1` pin). Corroborated independently by pass-29 | CLOSED — fix round 12 (declared in AC-3 tied to AC-8; ci.yml File Structure row extended; CHANGELOG line added) | Concurs with ADV-P29-INFO-003 |
...
- **Post-capture routing:** LOW-001 + INFO-001 + INFO-004 FIXED/CLOSED fix round 12 (`1e696128`); INFO-002 OPEN (rewording suggested, not applied); INFO-003 OPEN (dead-code, not applied)
...
| ADV-P29-INFO-001 | INFO | disclosed-limitation / MSRV-scope / empirically-confirmed | `.github/workflows/ci.yml :: msrv` — MSRV floor genuinely unenforceable for `src/` inline `#[cfg(test)]` modules and all of `tests/`. Confirmed empirically rather than on trust: `RUSTUP_TOOLCHAIN=1.85.0 cargo check --all-targets --all-features --locked` fails `error[E0658]` at `wiremock-0.6.5/src/matchers.rs:214`/`:215`. `ci.yml` comment and the new `CLAUDE.md` convention both disclose it accurately | OPEN — disclosed limitation | Concurs with ADV-P27-INFO-003 |
| ADV-P29-INFO-002 | INFO | pre-existing / test-coverage-floor-tolerance | `tests/mutants_glob_existence.rs::assert_examine_globs_coverage_floor` — `FLOOR = 11` against 16 live `examine_globs` entries in `.cargo/mutants.toml` — tolerates 5 silent removals before firing. Pre-existing, not introduced by this delta; noted only because the same commit touched the file | OPEN — pre-existing, candidate for follow-up | |
| ADV-P29-INFO-003 | INFO | spec-declaration / undeclared-flag-addition | `.github/workflows/ci.yml :: msrv` — `--locked` undocumented; AC-3 specified only the `env:` addition and neither CHANGELOG nor CLAUDE.md mentioned it | CLOSED — fix round 12 | Concurs with ADV-P28-INFO-004 — cross-pass corroboration is why this was promoted into the fix round despite INFO severity |
...
- **Post-capture routing:** MED-001 + LOW-001 FIXED this burst (spec-side); LOW-002 + INFO-003 FIXED/CLOSED fix round 12 (`1e696128`); INFO-001 OPEN (disclosed MSRV-scope limitation); INFO-002 OPEN (pre-existing coverage-floor tolerance)
