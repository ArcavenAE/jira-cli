...
**All four errors this session were the orchestrator's, not any fix-round agent's or reviewer's** — the first was attributed explicitly in the round-14 burst's CORRECTION record; the second through fourth were self-corrected or independently re-derived by a downstream agent before any reached a final, uncorrected permanent artifact. Recorded here because the pattern (not the individual instances) is the lesson: four independent recurrences of the same failure mode — a claim about actual state (itemized or repository) drifting from what re-deriving it from source would show — in one session, with the corrective catching three of the four before commit and widening from itemization-summaries to dispatch-brief premises, confirms the codified rule is both necessary and generalizes beyond its original scope.

_Trigger: `1cbcc3b8` false "CHANGELOG row widened" disposition clause (round 13, caught by three independent adversary reviewers in window 33/34/35) + round-14 burst-briefing "3 FIXED/8 OPEN" INFO-tally miscount (caught by the state-manager cross-checking the summary against the itemization before commit, confirmed by orchestrator recount) + round-17/18 burst "5 FIXED/8 OPEN" vs itemized "4 FIXED/9 OPEN" INFO-tally miscount (caught by the state-manager before commit) + round-18 burst PART 4 "fc9ad59b touches ci.yml and tests/ci_gate_completeness.rs, so both have grown" false premise (caught by the spec agent re-deriving both trails from `git` before writing, confirmed by the orchestrator's own `git show --stat` re-check and by the state-manager's independent third re-derivation) — all four 2026-08-05, all S-626-1 Step 4.5. Recorded in the ADVERSARY-33-34-35+FIX-ROUND-14 burst family (`7ab5b9d0` + this follow-up commit) and extended by the FIX-ROUND-18 (SPEC-SIDE VERIFICATION DISCLOSURE) burst and its PART 4 correction follow-up._
_Tagged: [orchestrator-error] [disposition-drift] [itemization-integrity] [summary-line-hygiene] [premise-verification] [soh-dx-1] [step-4.5-grind] [codified]_
