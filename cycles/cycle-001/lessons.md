...
**Pattern (four datapoints, all this session, all self-caught by the orchestrator, a state-manager cross-check, or a downstream agent verifying a handed-off premise before acting on it — never by an adversary reviewer):**
...
3. **Round 17/18 burst (2026-08-05):** A third instance, same shape as datapoint 2 — a disposition line tallied INFO findings as 5 FIXED/8 OPEN against an itemized list that actually showed 4 FIXED/9 OPEN. As with datapoint 2, the pairwise sums (5+8=13, 4+9=13) both checked out, so the arithmetic self-check alone would have let the wrong figure through. Caught by the state-manager cross-checking the disposition against the itemized list before it was written into a permanent artifact, and corrected before commit.

4. **Round 18 burst, PART 4 correction (2026-08-05):** A structurally different instance — not a tally-vs-itemization mismatch, but a **factual premise about the repository, stated in a dispatch brief and never independently checked before being asserted.** The orchestrator's round-18 dispatch brief stated that commit `fc9ad59b` "touches `ci.yml` and `tests/ci_gate_completeness.rs`, so both [commit-authorization trails] have grown again." In fact `fc9ad59b` touches only `tests/ci_gate_completeness.rs` (`git show fc9ad59b --stat`: one file, 84 insertions/10 deletions) — it does not appear anywhere in the list of commits touching `ci.yml`. The spec agent dispatched against this brief did not take the premise on faith: it independently re-derived the file-authorization trail (12→13 commits, adding `fc9ad59b`) and the POL-11 guard-step trail (`git show <commit> -- .github/workflows/ci.yml` per candidate, scoped to the step hunk) separately, found the guard-step trail unchanged at 7, and explicitly flagged the disagreement with the brief's framing rather than silently conforming the story to a premise it could not confirm. The orchestrator independently re-verified via `git show fc9ad59b --stat --format=''` and confirmed the agent's finding, correcting its own PART 4 in a follow-up message rather than letting the wrong premise stand. The state-manager burst-recording this correction re-derived the same trails a third, independent time via `git log`/`git show` before writing anything, and again found no disagreement with the (now corrected) numbers.

**Root cause, common to all four:** a claim about "what the itemized/actual state is" was composed *from expectation or a running mental model*, rather than *derived by re-reading the source artifact at the point of writing*. For datapoints 1–3 the source artifact was an itemized finding list; for datapoint 4 it was the commit itself (`git show`/`git log`) rather than a mental model of "what a commit like this usually touches." The mechanism is identical either way: a same-total or same-shape check that only validates internal consistency (sums reconciling, or a plausible-sounding two-file story) is blind to this error class, because it never re-touches the ground truth.

**Pattern widened (2026-08-05, round 18):** datapoints 1–3 are all itemized-list-to-summary drift (a rolled-up count or disposition composed alongside, not derived from, an itemized list). Datapoint 4 shows the same failure mode one layer earlier in the pipeline: **a factual premise about the repository — which files a commit touched, whether a cited test exists — asserted in a dispatch brief without being checked against `git` first.** This is the same class as the `test_ci_yml_has_windows_latest_in_test_matrix` false-fabrication accusation recorded separately as `ORCHESTRATOR-FALSE-FABRICATION-ACCUSATION` (STATE.md drift items, FIX-ROUND-18 burst) — both are cases of the orchestrator stating a confident claim about the codebase's actual content without first re-deriving it from source, and both were caught by a downstream agent that verified the premise it had been handed and refused to proceed on it (or corrected it) rather than complying. Two different agents, same behavior, both correct, in the same burst.

**Prescription (actionable, applies to any process — orchestrator dispositions, burst-briefing summary lines, dispatch-brief factual premises, changelog "N entries" declarations — that asserts a claim about itemized state or repository content without re-deriving it at the point of writing):**
...
>
> **Extended (datapoint 4):** a dispatch brief's factual premises about the repository (which files a commit touches, whether a cited symbol/test exists, what a diff contains) MUST be derived from the artifact itself (`git show`, `git log`, `git ls-tree`, a direct file read) before being written into the brief — not composed from what the commit is expected to contain given its stated purpose. The corrective is the same rule applied one step earlier: derive premises from the artifact before putting them in a brief, not only when summarising results afterward.
>
> A downstream consumer of the disposition or premise (state-manager, reviewer, implementer, or the orchestrator itself on a later pass) should default to recomputing/re-deriving from source whenever a checkable claim is handed to it, and treat any mismatch as a stop-and-report condition (per the standing "do not silently adjust a number that doesn't reconcile" burst directive) rather than propagating an unverified premise forward — this is what caught datapoint 2 before it was committed, datapoint 3 again, and datapoint 4 before it reached the story.

5. **Round 19 burst (2026-08-05).** The orchestrator's stage-2 dispatch brief for the pass-45/46/47 fix round asserted BC-level consequences for two guard-strength deltas it had correctly observed at the test-file layer — "`test_ci_gate_pass_fail_semantics_are_structurally_placed` 7→8" and "`test_verify_msrv_job_pins_toolchain_and_rustup_toolchain_env` 3→4" — phrased in the brief as if BC-X.13.007 itself gained assertions from these two functions. Neither function is governed by BC-X.13.007: that BC's own Verification Status block and `VP-CIGATE-001` (twelve assertions) are exclusively about `test_verify_test_job_has_zero_test_floor`, which round 19 left at 12→12, untouched. The two changed functions instead assert the *sibling* `ci-gate`/`msrv` structural/pass-fail-semantics guards, pinned at the S-CIGATE-1/S-626-1 story-AC layer (AC-001/002/003/AC-3/M1/M2) with **no corresponding BC or VP registered in this PRD** — the gap the same burst opened as `BC-COVERAGE-GAP-FOR-CI-GATE-GUARDS`. The dispatched BC-layer agent did not take the brief's framing on faith: it re-derived which BC (if any) governs each of the seven fixed test functions directly from `cross-cutting.md`'s existing "does not govern" carve-out language, found none, and wrote the boundary clarification as a disclosure rather than asserting a false coverage increase. Caught before it reached a permanent BC artifact.

6. **Round 48/49/50 window burst (2026-08-06).** The orchestrator relayed pass-50's finding that "zero cancelled `ci.yml` runs exist in the last 200 to sample" into the burst narrative without independently re-checking it against `gh run list`. The claim was false: four cancelled runs exist in the sampled window (`29736816386`, `29735639851`, `29701963083`, `29699658785`), all PR-triggered, and `CI Gate` correctly failed in all four — the reviewer's underlying conclusion (the *already-started-then-cancelled* path is handled correctly today) survived the correction, but the supporting evidence claim did not. Caught by the story-writer re-deriving the run list directly from the API while drafting S-CIGATE-2's EC-004, not by the orchestrator itself. Distinct from datapoints 1–5 in shape: this is neither an itemization-summary drift (1–3) nor a dispatch-brief premise about a commit's own content (4) nor a cross-layer governance claim (5) — it is a **relay of an unverified upstream claim into a downstream narrative**, one hop removed from the original source. Same root cause as all five priors: a claim about ground truth ("N matching things exist") was carried forward from what a prior message asserted, rather than re-derived from the artifact (here, the GitHub API) at the point of writing the burst record.

7. **Window 48/49/50 persistence burst, dispatch-sequencing instance (2026-08-06).** The orchestrator dispatched the state-manager persistence agent concurrently with a story-writer agent that was independently revising `S-CIGATE-2` (a research pass had just rejected the story's v1.0 Option A/Option B recommendation and the human had approved Option C), violating the standing "state-manager runs last in a burst" sequencing constraint. Both agents wrote to `stories/STORY-INDEX.md` in the same window: the story-writer landed `v1.5.71`/124 stories/story v2.0 on disk; the state-manager's dispatch brief, drafted before that revision existed, still described v1.0 (7 ACs, "fix approach not yet chosen", `STORY-INDEX v1.5.69→v1.5.70`). The state-manager's own `git add`+commit at the end of its burst captured the *then-current* disk state, so the committed blob was correct (`v1.5.71`, story v2.0) — but the state-manager's own narrative fields (STATE.md `current_step`, `Last Updated`, ADV-P1-INDEX.md's Window Summary, burst-log.md's Burst Summary) still described the superseded v1.0 state until a follow-up correction pass fixed them. No clobber occurred (the tree was correct throughout); the drift was purely in the state-manager's own narrative lagging a concurrent writer it did not know about. Caught when the team lead checked the actual tree state — `stories/STORY-INDEX.md`'s frontmatter and the story file's own version field — rather than trusting either agent's dispatch brief or its own prior draft. **Distinct in shape from datapoints 1–6:** none of the prior six involve two agents concurrently writing the same artifact; this is a scheduling/sequencing violation at the orchestrator-dispatch layer, one level upstream of the itemization, premise, governance, and relay errors already catalogued. Root cause: the orchestrator parallelized state-manager and story-writer on the assumption that the two agents touched disjoint surfaces, without verifying that assumption against the index file both would need to write.

**All seven errors this session were the orchestrator's, not any fix-round agent's or reviewer's** — the first was attributed explicitly in the round-14 burst's CORRECTION record; the second through seventh were self-corrected, independently re-derived, or caught by a human/downstream check before any reached a final, uncorrected permanent artifact. Recorded here because the pattern (not the individual instances) is the lesson: seven independent recurrences of the same failure mode — a claim about actual state (itemized, repository, cross-layer-governance, a relayed upstream claim, or now a concurrently-mutated shared artifact) drifting from what re-deriving it from source would show — in one session, with the corrective catching six of the seven before commit and widening from itemization-summaries to dispatch-brief premises to cross-layer governance claims to relayed-claim propagation to concurrent-dispatch sequencing violations, confirms the codified rule is both necessary and generalizes beyond its original scope. The seventh instance in particular sharpens the prescription below: it is not enough to re-derive a claim from source at the point of writing — when a burst dispatches multiple agents, the state-manager (or any agent narrating a shared artifact's state) must also verify no other concurrently-dispatched agent has mutated that same artifact between when its brief was drafted and when it commits.

_Trigger: `1cbcc3b8` false "CHANGELOG row widened" disposition clause (round 13, caught by three independent adversary reviewers in window 33/34/35) + round-14 burst-briefing "3 FIXED/8 OPEN" INFO-tally miscount (caught by the state-manager cross-checking the summary against the itemization before commit, confirmed by orchestrator recount) + round-17/18 burst "5 FIXED/8 OPEN" vs itemized "4 FIXED/9 OPEN" INFO-tally miscount (caught by the state-manager before commit) + round-18 burst PART 4 "fc9ad59b touches ci.yml and tests/ci_gate_completeness.rs, so both have grown" false premise (caught by the spec agent re-deriving both trails from `git` before writing, confirmed by the orchestrator's own `git show --stat` re-check and by the state-manager's independent third re-derivation) + round-19 burst stage-2 brief asserting BC-level 7→8/3→4 assertion-count growth for two test functions BC-X.13.007 does not govern (caught by the dispatched BC-layer agent re-deriving governance from `cross-cutting.md`'s own carve-out language before writing) + window-48/49/50 burst relayed pass-50's unverified "zero cancelled runs" claim into the burst narrative (caught by the story-writer re-deriving the run list from the API while drafting S-CIGATE-2 EC-004) + window-48/49/50 persistence burst dispatched state-manager concurrently with a story-writer revising S-CIGATE-2, violating "state-manager runs last in a burst" and leaving the state-manager's own narrative describing a superseded v1.0/Option-B state while the committed tree already held v2.0/Option-C (caught by the team lead checking the tree directly) — all seven 2026-08-05/2026-08-06, all S-626-1 Step 4.5 / S-CIGATE-2. Recorded in the ADVERSARY-33-34-35+FIX-ROUND-14 burst family (`7ab5b9d0` + follow-up commit), extended by the FIX-ROUND-18 (SPEC-SIDE VERIFICATION DISCLOSURE) burst and its PART 4 correction follow-up, extended by the ADVERSARY-45-46-47+FIX-ROUND-19 burst, extended again by the ADVERSARY-48-49-50 window burst, and extended once more by the ADVERSARY-48-49-50 window burst's own persistence-correction pass._
_Tagged: [orchestrator-error] [disposition-drift] [itemization-integrity] [summary-line-hygiene] [premise-verification] [cross-layer-governance] [relayed-claim-propagation] [dispatch-sequencing] [concurrent-writer-race] [soh-dx-1] [step-4.5-grind] [codified]_

### [codified] APPEND-ONLY-VS-IN-PLACE-CORRECTION: the freeze on `pr-description.md` is a property of capture artifacts, not a general rule for authored fields

**Pattern.** Round 19's `pr-description.md` immutability decision (a pasted verbatim runner log from run `e9b2269`, deliberately not edited — grounded in ISO 15489's authenticity/annotation rule and 21 CFR 11 §11.10(e) "shall not obscure previously recorded information") was correct. In the same burst's follow-up, the orchestrator instructed the state-manager to "update" a stale CI-status clause in `ADV-P1-INDEX.md`'s `feature_head:` frontmatter field ("CI 13/14 SUCCESS + Test (windows-latest) IN_PROGRESS at burst time, mergeStateStatus BLOCKED pending that job"). The state-manager applied a straight overwrite; the orchestrator then reversed itself, citing the `pr-description.md` precedent, and instructed append-only treatment instead (restore the original clause, append "resolved post-burst 2026-08-06: …" rather than replacing it). The state-manager complied but pushed back on the justification: `feature_head:` is not a capture artifact — it is authored state-manager commentary summarizing current understanding, the same class of field as `pass:`, `convergence:`, `total_findings:` in the same frontmatter, and the same class as the Verification Status table cells that this exact round's fix (M-1/M-2/M-3) had already **overwritten in place** without anyone treating that as falsification. The orchestrator re-examined the pushback and agreed: the extension was an over-generalization.

**Root cause.** Two structurally different obligations were collapsed into one rule. (1) **Capture/evidence artifacts** — a pasted runner log, a transcript, a merged-PR body, anything that *purports to be an unaltered emission from an external process* — carry an authenticity obligation: the live file, read as it currently stands, must not misrepresent what the process actually emitted. Editing one doesn't just update information, it fabricates a different capture under the same label. (2) **Authored summary fields** — status cells, frontmatter one-liners, Verification Status grades, narrative assessments — exist specifically to state current understanding and are *expected* to change as facts change; that is what versioning (and, upstream of that, git history itself) is for. Applying rule (1)'s append-only discipline to a rule (2) field doesn't protect anything — it produces append-chains that assert two contradictory facts in one value while gaining no integrity guarantee, since the field was never a claimed capture in the first place.

**Prescription (actionable, applies to any decision about whether a correction may overwrite or must append):**

> Before choosing append-only vs. in-place correction, classify the artifact: does the field, section, or file **purport to be an unaltered emission of a process** (a log, a transcript, a runner's stdout, a third-party body) — or does it **purport to be the author's current best understanding**, subject to revision as facts change? Append-only (restore-and-annotate, never obscure) applies only to the first class. Ordinary in-place correction, narrated in the correcting commit message and (where the artifact versions) the version-history trail, applies to the second — and remains correct even when the field's own wording sounds definitive ("CI is BLOCKED", "mergeStateStatus: CLEAN"), because definitiveness of phrasing is not the same property as claimed-capture status.
>
> When genuinely unsure which class an artifact belongs to, the tell is: would editing it make the *file itself*, read in isolation with no git access, claim to be something it isn't? If yes (a runner log that no longer matches what the runner printed), append-only. If no (a status summary that simply states an updated fact), in-place correction is not falsification — git history is the append-only ledger for authored fields, exactly as `burst-log.md` is for `STATE.md`'s own historical narrative.

**Disposition:** the `ADV-P1-INDEX.md:16` append was left in place (harmless, strictly more informative, not worth the churn of reverting) but is not precedent; the four equivalent `STATE.md` sites the orchestrator had already asked the state-manager to overwrite in an earlier commit (`ac2ad8e2`) were correctly left as in-place corrections, not retroactively converted to append-chains; and append-only was explicitly declined as a standing convention for `feature_head:` or any other status field going forward — the classification above governs case by case.

_Trigger: round-19 follow-up burst (2026-08-06) — orchestrator instructed an overwrite of `ADV-P1-INDEX.md:16`'s stale CI clause, the state-manager applied it, the orchestrator reversed itself citing the `pr-description.md` precedent and required append-only, the state-manager complied but flagged the precedent as over-extended (citing this round's own M-1/M-2/M-3 in-place Verification-Status corrections as internal counter-evidence), and the orchestrator agreed the extension was wrong. Commits: `ac2ad8e2` (original overwrite, left standing), `a449b320` (append applied per the — now superseded — instruction), this lesson (recorded per the orchestrator's explicit request to capture the durable distinction)._
_Tagged: [orchestrator-error] [orchestrator-self-correction] [record-integrity] [append-vs-overwrite] [pr-description-precedent] [soh-dx-1] [step-4.5-grind] [codified]_

### [codified] SILENT-MERGE-CONCATENATION-DEFEATS-CONFLICT-DETECTION: git-marked conflicts are a lower bound on reconciliation risk, not a complete inventory

**Pattern.** RESUME+RECONCILE-667 (2026-08-07) reconciled PR #667 with `develop` after S-CIGATE-2's merge (`df203233`) put the two branches on a collision course: both had independently edited `tests/ci_gate_completeness.rs::test_ci_gate_pass_fail_semantics_are_structurally_placed`. Git resolved three real hunks with visible conflict markers — all doc-comment prose, correctly union-resolved with no ambiguity. But git also *silently* auto-merged a fourth edit to the same function with zero conflict markers: it concatenated S-CIGATE-2's complete M2-a..M2-o assertion rewrite with S-626-1's own tail-appended round-19 addition (a literal `run: exit 1` assertion, "Assertion 4 (F-01)"). Because S-CIGATE-2 had already changed the shipped `ci-gate` run line to the `check-ci-gate.sh` invocation, the concatenated test asserted two mutually exclusive things about the same line and failed red 18/19 on the very first run after the merge. Local verification (fmt, clippy, full test suite, self-test fixtures) was what surfaced the failure — not the merge itself, which reported clean with three conflicts resolved and nothing else flagged.

**Root cause.** Git's three-way merge algorithm resolves non-overlapping line-range edits to the same function automatically, without a conflict marker, as long as the edited line ranges within the file don't literally overlap by git's own diff-hunk boundaries. Two edits to *different parts of the same logical function* — one editor's full-body rewrite, another editor's tail-appended addition — can pass that literal test while being semantically incompatible. The three human-visible conflicts were the ones where git's own heuristic detected literal overlap; this fourth one sat just outside that heuristic's detection radius. A developer (or agent) who checks "are there any `<<<<<<<` markers left" and finds none has verified nothing about this class of defect.

**Prescription (actionable, applies to any post-merge/post-rebase reconciliation, human or agent):**

> Conflict markers tell you where git detected disagreement — they do not tell you where semantic disagreement actually occurred. Treat them as a lower bound on reconciliation risk, never a complete inventory. After every merge that touches a file both branches independently edited — including a merge git reports as fully clean with zero conflicts — run the full test suite before treating the reconciliation as complete. For any function or block both branches touched (checked via `git log --oneline <base>..<branch-a> -- <file>` and the same for branch-b, intersected against the file's function boundaries), do a manual base/ours/theirs/merged four-way read of that specific function, not just a skim of the diff. A test suite that was 100% green pre-merge and stays 100% green post-merge with zero manual review is the actual signal that reconciliation succeeded — not the absence of `<<<<<<<` markers.
...
### [codified] PREMATURE-DRIFT-CLOSURE: closing a drift item on one reconciled instance closes the instance, not the class

**Pattern.** `CLOSED-STORY-CONTRADICTS-SHIPPED-BEHAVIOR` was marked CLOSED during the S-CIGATE-2-DELIVERY+SESSION-CLOSE burst (2026-08-07), on the strength of S-CIGATE-4 reconciling `S-CIGATE-1-ci-gate-aggregator.md` AC-003 in place (the "`mutants` MUST NOT appear in `ci-gate.needs`" prohibition, superseded by `S-MUTATION-CI-TIMEOUT-1`/`S-CIGATE-2`). FIX-ROUND-20 (2026-08-07, this burst) found **three more instances of the identical shape** — a closed spec asserting something the shipped product no longer does — none of which S-CIGATE-4's fix had touched: (1) `BC-X.13.007`'s Invariants bullet still claimed the sibling `ci-gate` guard pinned "a literal `run: exit 1` body," invalidated by S-626-1's own commit `7f702bf6` retiring that exact assertion the same week; (2) `S-CIGATE-1-ci-gate-aggregator.md` AC-002's body still described the retired inline `contains()` condition as the gate's pass/fail mechanism, when the real mechanism since `S-CIGATE-2` is `scripts/check-ci-gate.sh::evaluate_needs` — flagged this round but deliberately NOT fixed, because correcting it changes what a *closed* story asserts as testable, which the state-manager judged requires a human ruling rather than an in-cycle spec-hygiene fix; (3) `BC-X.13.007`'s Behavior item 2 described the POL-11 canary as proving only that the binary was *launched*, invalidated by round 20's own strengthening of that exact check to require a non-zero passed count. Items (1) and (3) were fixed this burst; item (2) was flagged and left open pending human ruling.

**Root cause.** The original closure treated "the specific contradiction the triggering finding pointed at is now reconciled" as equivalent to "the class of drift this item exists to track is closed." Those are different claims. A drift item that names a *pattern* (closed specs asserting stale shipped-behavior claims) is not retired by fixing the one instance that prompted it — the pattern is a property of the repo's velocity (specs get closed, then the product keeps shipping against a still-open PR, and the closed spec has no mechanism to notice it has gone stale), not a property of any single finding. Three more instances surfacing in the very next round, all independently discovered by three different adversarial findings (ADV-P49-MED-003, ADV-P49-LOW-001, ADV-P50-LOW-002's ripple), is direct evidence the pattern was never closed — only its first known instance was.

**Prescription (actionable, applies to any drift-item closure decision):**

> Before closing a drift item that names a *class* of defect (as opposed to a single incident), ask: did the fix address the specific triggering instance, or did it address the mechanism that lets the class recur? If the answer is "the specific instance," the correct disposition is CLOSED-FOR-THIS-INSTANCE (or leave OPEN with the instance noted as resolved), not CLOSED. Reserve full CLOSED status for drift items where either (a) the fix was structural — a CI guard, a template constraint, a process change that makes the whole class mechanically unreachable — or (b) a systematic sweep was run across every plausible site of the same pattern and confirmed clean, not just the one site the triggering finding named. A single adversarial pass finding "this specific spot is stale" is evidence of the class, not a bound on it.

**Disposition:** `CLOSED-STORY-CONTRADICTS-SHIPPED-BEHAVIOR` reopened to OPEN in this burst, reframed to record that S-CIGATE-4 closed one instance, not the class — three further instances found in the very next round (two fixed this burst, one flagged pending human ruling on whether correcting a closed story's testable assertions requires explicit authorization).

_Trigger: FIX-ROUND-20 (2026-08-07) — the spec-side work dispatched against window pass-48/pass-49/pass-50's remaining findings independently surfaced three more class members of a drift item the prior burst had marked fully CLOSED four days earlier._
_Tagged: [drift-item-discipline] [premature-closure] [class-vs-instance] [soh-dx-1] [step-4.5-grind] [codified]_

### [codified] LOCAL-VERIFICATION-MISSES-PLATFORM-MATRIX — second instance, identical mechanics, two days apart

**Pattern.** S-CIGATE-2's delivery burst (2026-08-07) first surfaced this class: thirteen adversarial reviews and extensive mutation testing, all run on macOS, never caught that a `#[cfg(unix)]`-gated test orphaned its helper functions on Windows, where `-D warnings` promoted the resulting dead-code warning to a hard build error; CI caught it on first real contact with the Windows leg. FIX-ROUND-20's own hardening of the POL-11 canary (commit `424d64de`) reproduced the identical failure shape two days later: the strengthened canary matched `Running tests/ci_gate_completeness.rs` with a bare forward slash. macOS/Linux cargo prints exactly that form, so every local run and every prior adversarial pass saw it succeed. `windows-latest` cargo prints `Running tests\ci_gate_completeness.rs` — backslash — so the canary's anchor-line lookup silently found nothing, reported zero passed assertions, and failed the entire Windows CI leg (fixed same-day in `177b3727` with a `[/\\]` character class) even though all 1,102 lib-binary tests on that leg had genuinely passed. Both instances share the same shape: a string literal or regex written and validated against one platform's toolchain output, silently wrong on another, invisible to any amount of same-platform review no matter how many passes or how rigorous.

**Root cause.** Neither the adversarial review protocol nor local pre-push verification runs on more than one OS. A defect of this class is not a *logic* error any review of the code's intent would catch — the guard is doing exactly what its author intended, on the platform the author tested it on. It is a *platform-string-literal* error, detectable only by (a) executing the code on the other platform, or (b) recognizing during authorship that any string comparison against tool output (`cargo test` `Running` lines, path separators, line endings) is platform-variant and must be written defensively from the start. The repo caught the first instance via CI; it also caught the second instance via Ci — but only after the defective guard had already shipped to `develop`-bound `ci.yml` and needed a same-day follow-up commit, on a class this repo had already named a drift item for once.

**Prescription (actionable, applies to any guard, test, or string comparison that inspects the output of `cargo`, a shell tool, or any other platform-variant toolchain command):**

> Any regex or substring match against tool-emitted output (compiler/test-runner banners, paths, line-ending-sensitive text) must be written to already tolerate every platform in the CI matrix (currently ubuntu/macos/windows) at first authorship — not discovered reactively when the Windows leg goes red. Concretely: path separators use a `[/\\]` (or equivalent) character class, not a literal `/` or `\`; line endings are normalized before comparison, never assumed LF; and any new guard of this shape should be paired, at authorship time, with a mental (or literal, in a fixture) checklist of "what does this look like on the platform I'm NOT currently running." A guard-strengthening pass is exactly the moment new platform-variant literals get introduced — treat every strengthening of an existing guard as carrying the same platform-matrix risk as writing a brand new one.

**Disposition:** fixed same-day in `177b3727`, proven against both forward-slash and backslash fixtures plus a zero-passed-count fixture confirming the strengthened non-zero-passed assertion still fires correctly. Drift item `LOCAL-VERIFICATION-MISSES-PLATFORM-MATRIX` updated from its first (MEDIUM) instance to record this second, structurally identical recurrence — orchestrator flagged for a severity re-look given two instances in two days sharing the same root cause and the same blind spot in review methodology.

_Trigger: FIX-ROUND-20 (2026-08-07) — the round's own POL-11 canary strengthening (`424d64de`) introduced a Windows-only false-negative regression, caught by CI and fixed same-day (`177b3727`), two days after the first instance of this exact class shipped in S-CIGATE-2's delivery burst._
_Tagged: [platform-matrix] [ci-verification-gap] [windows] [string-literal-fragility] [soh-dx-1] [step-4.5-grind] [codified]_

### [codified] WRITING-THE-GUARD-FINDS-DEFECTS-THE-FINDING-DIDN'T-NAME: implementing a pin surfaced an unnamed fail-open

**Pattern.** FIX-ROUND-20's `ADV-P50-LOW-002` finding named a real gap — the POL-11 named-canary check proved the `ci_gate_completeness` binary was *invoked*, never that it *reported results* — and round 20 closed it with a canary that locates the binary's own `test result:` line and requires a non-zero passed count from it. Implementing the regression pin for that fix (this burst, commit `ada50a34`) surfaced a second, unnamed problem in the fix itself: a naive pin could be satisfied by `grep`-ing for *any* `test result:` line anywhere in the full CI capture, not specifically the canary binary's own line. With 103 test binaries reporting results in the same log, a global grep would find some other binary's passing line and pass the check even if the canary itself reported zero — reproducing the exact fail-open class the fix was written to close, one layer down, inside the fix's own guard. Nothing in `ADV-P50-LOW-002`, in round 20's implementation, or in any of the 50 prior adversarial passes named this specific risk. It was found only by the mechanical act of writing a regression pin that had to decide, concretely, which `test result:` line counted — `tail -n +"${_canary_running_line}"` scopes the lookup forward from the canary's own `Running` announcement, closing the gap.

**Root cause.** A review pass reads code and reasons about what could go wrong; writing a test (or a pin) requires deciding, for a specific input, exactly what "correct" looks like — and that decision-forcing step surfaces ambiguities and adjacent failure modes that a reader can walk past without noticing, because a reader is never forced to commit to one concrete answer. This is the same shape `FIX-PASS-CATCHES-MORE-THAN-REVIEW-PASS` (drift item, INFO, open since round 12-ish) already names in general — this burst is a fresh, concrete datapoint for it, one layer more specific: the thing found was not just "an extra defect while fixing," but a fail-open *in the guard being built to close a fail-open*, discovered by the exercise of implementing the guard rather than by reviewing the plan for it.

**Prescription (actionable, applies to writing any regression pin for a finding that itself describes a scoping or under-specification defect):**

> When a finding says "check X doesn't scope its search/match narrowly enough" (an under-specification or over-broad-match class), do not stop at making the fix's own assertion pass — ask explicitly, while implementing the pin: "if I wrote the most naive version of this same check, what would it wrongly accept?" For a log/text-search fix specifically: could the correct-looking match come from the wrong source (a different binary, a different step, a different run)? Record the naive-wrong-answer scenario as its own instrument or comment, even if it never becomes a separate finding — the point is that implementing a fix for a scoping defect is exactly the moment a second, narrower scoping defect is easiest to introduce and hardest to catch by review alone.

**Disposition:** closed in the same commit (`ada50a34`) that implemented the fix — `tail -n +"${_canary_running_line}"` (Instrument 2b) scopes the `test result:` lookup to lines at-or-after the canary's own `Running` line, so a passing result from any of the other 102 test binaries cannot satisfy the check. No separate finding or drift item was needed because the gap was closed before it ever shipped; recorded here as a process lesson about *how* it was found, not as an open item.

_Trigger: REGRESSION-PIN+EC+GUARD-STORY burst (2026-08-07) — implementing the human-authorized regression pin for round 20's `ADV-P50-LOW-002` fix (commit `ada50a34`) surfaced a fail-open in the pin's own naive form before it was written, distinct from and not named by the original finding._
_Tagged: [test-writing-finds-more-than-review] [fail-open-in-the-fix] [regression-pin-discipline] [soh-dx-1] [step-4.5-grind] [codified]_

### [codified] FRONTIER-DESIGN-IS-THE-ACTIVE-VARIABLE: three deliberately-varied frontiers found 30 findings on a tree three prior windows had already worked over

**Pattern.** Window 51/52/53 dispatched three passes against a frozen head (`ada50a34`) that had already survived windows 42/43/44 (0/3), 45/46/47 (1/3), and 48/49/50 (0/3) — extensive, real adversarial attention, not a fresh tree. Given a completely novel triple of inspection frontiers (vacuous satisfaction; reconciliation seams; spec-to-shipped-behavior) that no prior pass had used, all three still returned NOT CLEAN, together surfacing 30 new findings including 5 HIGH. This is the fifth reinforcement of `DECAY-CURVE-MEASURES-QUESTION-EXHAUSTION-NOT-CONVERGENCE`, but the datapoint is sharper than prior ones: it was produced *by construction*, not by accident. The human approved the three frontiers up front, specifically because they were distinct from every frontier used in the prior 50 passes, and the yield confirmed the hypothesis directly rather than as an incidental byproduct of some other dispatch.

**Root cause.** A CLEAN pass or a decaying finding count answers "has this specific line of questioning been exhausted," not "has the review space been exhausted." Seven consecutive windows (42/43/44 through 48/49/50) each closed without reaching 3/3, and each of those windows' passes used somewhat different framings — but none of them deliberately curated three maximally-orthogonal frontiers the way window 51/52/53 did. The fact that doing so on purpose immediately produced material no prior pass had found is evidence that the review space is far larger than any single frontier, or even an unplanned sequence of frontiers, can cover — and that novelty decay within one frontier is not evidence the space itself is exhausted.

**Prescription (actionable, applies to any future adversarial window on a heavily-reviewed surface):**
...
### [codified] HARDEN-THE-NODE-THAT-CARRIES-THE-SIGNAL: twenty fix rounds strengthened the aggregator while the aggregated job stayed unguarded

**Pattern.** Pass 51 (vacuous-satisfaction frontier) found that every structural control built and progressively strengthened for `ci-gate` across twenty fix rounds — the step-level `if:` ban, the `continue-on-error` ban, the env key-set pin, the byte-for-byte run-line pin, the ordered step-key-set pin — stops at the `ci-gate` job block. `test`, the `needs` member whose pass/fail signal `ci-gate` exists to aggregate and whose failure is the entire point of the gate, is guarded only by 15 order-free, position-free substring presence checks with none of `ci-gate`'s structural rigor. None of the three HIGH findings this surfaced are live defects — CI at `ada50a34` is correct today — but each is a straightforward, undetected-by-twenty-rounds-of-review path to a silent false-green: skip the test step with `if: false`, neutralize its gates with `continue-on-error: true`, or relocate the existing `set +o pipefail` token to defeat the pipefail-detection assertions.

**Root cause.** Every one of the twenty fix rounds that hardened `ci-gate` was responding to a finding *about* `ci-gate` — the object under review was, each time, the thing being cited. `test` was never independently the subject of a hardening round because no finding ever named it directly; it was treated as trusted infrastructure the gate merely reads a result from, rather than as a peer attack surface. This is a structural version of "fix the finding, not the class" (the same shape as `TARGETED-FIX-ROUNDS-DO-NOT-CONVERGE`, one level up): the review process hardened the node most recently named, not the node most load-bearing for the property the whole guard family exists to protect. The aggregator (`ci-gate`) only ever reports what its `needs` members decided; if a `needs` member's own decision can be silently neutered, hardening the aggregator's aggregation logic is necessary but not sufficient.

**Prescription (actionable, applies to any guard family built around an aggregator + reported-to members):**
...
_Trigger: RESEARCH-VALIDATION+U1 (2026-08-08) -- a human-directed research pass validated GitHub Actions gating semantics against primary sources and surfaced U1, a needs-set completeness gap no adversarial pass had found in 50 passes._
_Tagged: [verification-methodology] [external-validation] [research] [ci-gate] [needs-set-completeness] [soh-dx-1] [codified]_
...
_Trigger: RESEARCH-VALIDATION+U1 (2026-08-08) -- a declined `git pull --ff-only` at an earlier resume produced a false "defect still live on develop" research finding; remediated by fast-forwarding local develop to `df203233`._
_Tagged: [state-integrity] [stale-artifact] [orchestrator-process] [housekeeping-discipline] [session-resume] [codified]_

### [codified] SANCTIONED-PATH-EXISTS-BEFORE-DECLARING-DEADLOCK: STATE.md's "no working write path" conclusion was false -- the guard's own error text named the fix

The SESSION WRAP burst (2026-08-09) recorded STATE.md as having no working write path and concluded that compaction itself was unreachable, because "compaction requires exactly the large write that is failing." All three write mechanisms had been tried and had each failed or been blocked in some way: `Write` failed on an output-size limit at STATE.md's then-current 112,071 bytes / 372 lines; `Edit` is correctly blocked by `verify-state-timestamp-refresh` for any diff that doesn't touch the frontmatter's `timestamp:` field; `Bash`-based writes are correctly blocked by `guard-state-bash-write.sh`, a PreToolUse hook that exists specifically because raw file I/O bypasses every STATE.md validator. Three blocked paths reasonably reads as "no path" -- but it wasn't. `guard-state-bash-write.sh`'s own error text states the sanctioned path verbatim: use the `Write` tool with content that advances `timestamp:`. That IS the blessed path, not a fourth blocked one. It had only ever failed because of the file's SIZE, an orthogonal constraint the burst's diagnosis conflated with the guard apparatus itself. The compaction skill's own design makes the deadlock avoidable in the first place: its final STATE.md write is a SLIM file by construction (this burst: 320 lines / ~37KB, well under the size threshold that had failed before), and its large EXTRACTION writes go to `.factory/cycles/cycle-001/*.md`, files which carry no guard hooks at all. The premise "compaction requires exactly the large write that is failing" was simply false -- compaction's whole design exists to avoid ever needing that large write again.
...
A count derived by grepping for a keyword and counting occurrences (`grep -o keyword | uniq -c`) is not the same operation as counting rows in a table whose status column happens to contain that keyword among other text — the former counts every textual mention (including mentions inside notes, cross-references, and prose describing a DIFFERENT row's status), the latter counts actual table rows. Applied to this project's drift-item and finding-status bookkeeping, the two methods diverged: 211 (keyword-occurrence count) vs. 188 (real row count) on the same underlying table, and two different agents, each re-deriving "the count" independently rather than re-reading a prior figure, arrived at two different WRONG splits of that same discrepancy — evidence that the failure mode is the measurement method itself, not a one-off transcription slip by a single agent.

**Disposition:** count rows by parsing table structure (or an equivalent row-boundary-aware method), never by counting keyword occurrences in free text, whenever the artifact being measured is a table with a status column. This is now a standing rule, applied with immediate effect (not deferred to a future story) — this same burst's own re-derived commit-trail and `#[test]`-count figures (`S-626-1.md` FIX ROUND 27) deliberately used exact structural matches (`git log --format='%h' -- <path> | wc -l`; a line-exact `#[test]` match, not a substring `grep -c`, which itself overcounted 27 real test functions to 34 by matching prose mentions of the string) as a direct application of this same principle, not merely a citation of it.

_Trigger: RESUME+WINDOW-54-55-56+CLASS-SWEEP (2026-08-09) -- recorded as drift item `MEASUREMENT-METHOD-PRODUCES-FALSE-CLAIM`, deferred with immediate-effect standing-rule disposition rather than a follow-up story per S-7.02._
_Tagged: [measurement-methodology] [row-counting] [re-derivation-discipline] [process-gap] [s-7.02] [codified]_

### [codified] RED-PROOF-EVERY-SEMANTICALLY-EQUIVALENT-SPELLING-NOT-JUST-THE-FIRST-ONE-TRIED: `0e61a2dc`'s RED proofs covered only the bare key spelling, and all three new call sites failed open on the other three

Commit `0e61a2dc` (this same burst) RED-proofed its two new guards against the bare, unquoted key spelling only — a scratch sibling workflow with a plain `name: CI Gate` line, a temporary `fromJSON()` rewrite of a plain `os:` list. Both RED proofs correctly demonstrated the guards fail before the fix and pass after. What neither RED proof exercised: `"name":`, `'name':`, and `name :` (space before colon) — three spellings PyYAML treats as byte-for-byte identical to the bare form, and all three of which defeated the new guards' bare `strip_prefix`/`starts_with` value-reads, exactly the root cause window 54/55/56 converged on three independent times. A RED proof that only reproduces the FIRST failure mode tried gives false confidence that the fix is complete when it has only been shown complete against one input shape out of (at minimum) four PyYAML-equivalent spellings already known to be relevant in this exact file — `extract_key_name_at_indent`, the file's own quote/whitespace-aware KEY matcher, already handles all four; the new guards' VALUE-reads did not, and no RED proof surfaced the gap because none was tried against those three additional spellings.

**Disposition:** a RED proof must cover every semantically-equivalent spelling of its target construct that a real parser treats as identical, not just the first (typically bare/unquoted) spelling that comes to mind — this is now the fifth documented condition for `RED-PROOF-REQUIRES-FOUR-CONDITIONS`. Deferred with a concrete target rather than fixed as a one-off: applies as a review-time checklist item at every future RED-proof requirement in this file's class of guard, starting with **S-CIGATE-3** (the durable YAML-parser fix that will eventually retire the need for line-based extractors, and their spelling-variant RED-proof burden, entirely).
...
### [codified] AN-AGGREGATE-IS-A-MEASUREMENT-DERIVE-IT-DONT-ESTIMATE-IT: the orchestrator supplied a finding-count aggregate it had not actually summed, in the closeout of the very burst that logged the same class of error twice already

The S-626-1-MERGE+ADV-P60-P61+BURST-CLOSE burst's own closeout recorded `ADV-P1-INDEX.md` as `442→452 total findings, +10 new, 6 actionable`. The correct arithmetic, sitting in that same commit's own Pass 60/61 catalog, was `442→456, +14 new, 9 actionable`: P60 = 2H+0M+3L+3I = 8, P61 = 0H+2M+2L+2I = 6 (re-rating one finding's severity after independent reproduction does not change the finding count), 8+6=14. The aggregate was supplied as a round-trip-plausible number rather than derived by actually adding the per-severity tallies already present. This is `MEASUREMENT-METHOD-PRODUCES-FALSE-CLAIM`'s THIRD instance, and it landed in the closeout of the burst that had just logged the second instance hours earlier — the same failure mode recurring within the same burst that was supposed to have just reinforced the standing rule against it.

What went right: a downstream agent dispatched to add itemized per-finding detail caught the mismatch by arithmetic. It did not silently "fix" the number using its own guess, and it did not fabricate detail for the six placeholder rows it had not been given real content for — it preserved the (wrong) authoritative number, flagged the discrepancy in an explicit reconciliation note, and stopped there. That is the correct behavior for an agent that finds a measurement it cannot independently verify: flag, don't silently override, don't fabricate to fill the gap.

**Disposition:** an aggregate (a total, a count, a sum across categories) is a measurement, not an estimate — derive it by actually performing the arithmetic over the source data, every time, even when the number "sounds right" or was supplied by an upstream authority. This applies as much to the orchestrator supplying dispatch-brief numbers as it does to any other measurement claim already covered by this drift item (env vars actually set, files actually changed). Separately: a downstream agent flagging an arithmetic or measurement mismatch against an upstream-supplied number is a correction to act on immediately, not noise to dismiss or route around — the flag itself is often the fastest, cheapest signal that a claim needs re-deriving before it becomes a permanent record.
...
**Disposition:** correcting a record is an edit to the record, not an exemption from the discipline the record is held to. Apply the same claim-accuracy review to a "just fixing the docs" PR that would apply to a PR touching the mechanism the docs describe — especially when the PR's whole premise is that the prior record was wrong, since that framing invites less scrutiny of the new text, not more. When a claim traces through a citation chain (primary source → research artifact → project record), re-fetch the primary source at the final hop rather than trusting the intermediate paraphrase's scope; each hop in this instance was individually reasonable, and the compounded result was still false.

_Trigger: PR675-MERGE+ADV-P675-CLOSE burst (2026-08-11, team-lead-dispatched) — targeted claim-accuracy review of a documentation-only PR (`CLAUDE.md` + `scripts/check-ci-gate.sh` comment/prose, zero non-comment lines) found 2 MEDIUM + 2 LOW, all amended pre-merge (`d2430a8a`); drift items `CORRECTION-PR-INTRODUCED-NEW-FALSE-CLAIMS` and `SCOPED-SOURCE-GENERALIZED-THROUGH-CITATION-CHAIN` recorded._
_Tagged: [ci-gate] [claim-accuracy] [citation-chain] [process-gap] [review-discipline] [codified]_

## PR675-MERGE+ADV-P675-CLOSE — A Doc-Only PR Warrants Review When Doc Comments Are Load-Bearing Controls (2026-08-11)

The dispatch reasoning behind ADV-P675 was explicit going in, not discovered after the fact: this repository has multiple places (`resolve_trusted_jq`'s `HONEST SCOPE` comment, the round-13 CLAUDE.md `uses:`-pinning scope note, the many rounds of `ci_gate_completeness.rs` doc-comment corrections recorded across this cycle) where a comment IS the control — no test enforces the property the comment describes; the comment is the only place the property's actual scope and residual risk are recorded for a future reader. The outcome (2 real MEDIUM findings in a 0-non-comment-line diff) confirms the premise: "docs-only" was not a signal to skip review, it was a signal to review the diff for exactly what it was diffing.

**Disposition:** whether a PR needs adversarial review should be a function of what the changed text is depended on for, not what file extension or diff shape it has. A one-line code change to an unused constant may warrant less scrutiny than a ten-line comment correction to a security-boundary rationale. This generalizes the existing project rule ("CI green and mergeable is not fully reviewed," logged at S-626-1-MERGE+ADV-P60-P61+BURST-CLOSE) one step further: "documentation-only" is not fully reviewed either, when the documentation is itself part of the control surface.
...
## A close-out burst that dies mid-response should reconcile the prior attempt's uncommitted work, not re-derive it from scratch

Three consecutive state-manager bursts attempting to close `S-MUTANTS-SCOPE-1` died on transient mid-response API errors before committing, each leaving real, correct F3–F7 artifact content sitting uncommitted in the `.factory/` worktree (`stories/S-MUTANTS-SCOPE-1.md` through its AC-count amendment, `stories/STORY-INDEX.md` through v1.5.90, `spec-changelog.md` entry `[1.3.182]`, the matching `specs/prd/cross-cutting.md` citation refresh, `code-delivery/S-MUTANTS-SCOPE-1/`, `demos/S-MUTANTS-SCOPE-1/`, and a partial edit to `drift-items-open-detail.md`). The recovery burst ran `git -C .factory status`/`git -C .factory diff` FIRST, verified each piece of pending content against what the close-out was supposed to record, and built the remaining work (Close-Out section, status flip, drift-item resolution, DEC, count reconciliation) on top of what was already correct on disk — rather than treating the dirty worktree as noise to discard or blindly re-writing files that were already right.

**Disposition:** a resilient burst-recovery protocol is: (1) verify worktree preconditions per the standing checklist; (2) `git status`/`git diff` to enumerate exactly what a prior failed attempt left uncommitted; (3) diff each pending file's content against what the current instruction asks for, rather than assuming it's stale or wrong; (4) reconcile (extend/complete) rather than overwrite verified-correct partial work; (5) commit everything in one atomic burst per the Single-Commit Burst Protocol. This closes cleanly without re-doing work that three prior attempts had already gotten right, and without risking data loss from an unnecessary full rewrite.

_Trigger: S-MUTANTS-SCOPE-1-CLOSED (2026-08-14), explicit orchestrator instruction naming the 3 prior failed runs and directing a resilient, idempotent reconciliation rather than a fresh rewrite._
_Tagged: [state-manager-resilience] [burst-recovery] [S-MUTANTS-SCOPE-1] [codified]_
...
## A fix instruction into a subtle matching/identity path needs a complete combination matrix in the same fix, not an incremental patch

S-605-1's Step-4.5 RMW (read-modify-write) remove-matching code accumulated a **fix-chain** across four consecutive rounds, each orchestrator fix instruction touching the same id/name matching logic without covering the full combination space, each introducing the next round's regression: R3 F1 wired the numeric `--component` value as `{"name":"<digits>"}` instead of `{"id":...}`; the R4 fix for that (MED-1) left a retained-by-name fallback path that re-emitted retained components by name, silently colliding with and dropping distinctly-id'd components sharing that name; the R5 fix for THAT (B-LOW-1) refactored to a `ComponentRef` collapse that looked complete but only handled the name-only and id-only cases, not id-bearing components being removed by name; R6's HIGH-1 caught the resulting regression — name-based removal silently no-op'd against live, id-bearing components fetched from Jira (a correctness defect that would have shipped as silent data non-removal). R6's fix was definitive specifically because it was scoped as a full combination matrix (id-present × name-present × remove-kind × add-set-relationship-to-remove-set) rather than another point patch, with a dedicated RMW test matrix covering every cell.

**Disposition:** when instructing a fix in an identity/matching path (name vs id vs some-combination, add vs remove, retain vs drop), the fix instruction must require the fixer to enumerate the full combination matrix up front and show test coverage for every cell — not describe only the one failing case the adversary happened to find. A point patch to the reported case, without the matrix, reliably ships the next cell's regression to the next adversary round. This is a further, concrete instance of the pre-existing `ORCHESTRATOR-FIX-INSTRUCTION-CAUSED-REGRESSION` drift item (now a 3-round fix-chain in one story, not just two isolated instances) — DEFERRED to F7 disposition as an engine/process-level fix (dispatch-instruction template requiring a stated combination matrix for any identity/matching-path fix).

_Trigger: S-605-1 Step-4.5 (2026-08-18), RMW remove-matching fix-chain R3→R4→R5→R6._
_Tagged: [process-gap] [orchestrator-fix-instruction] [identity-matching] [S-605-1] [regression-chain]_

## RMW test fixtures must mirror the live wire shape (id-bearing), or the fixture itself masks the bug it exists to catch

The R6 HIGH-1 silent-data-loss defect (name-based component removal no-op'ing against id-bearing live components) went undetected for five prior Step-4.5 rounds because the test fixture used to exercise the read-modify-write path (`issue_response_with_components`) emitted components as `{"name": n}` with **no `id` field** — an id-less shape live Jira never actually returns (Jira always includes `id` in a GET response). Every RMW test built on that fixture could therefore only ever exercise the name-only matching branch; the id-bearing branch the real bug lived in was structurally unreachable through the whole fixture family until R6 rebuilt it with ids present. This is the same root-cause shape as the previously-codified `FIX-576-DL` mock-vs-live drift (an id-typed-as-string-vs-integer mismatch between wiremock fixtures and live Jira) recurring in a different dimension (field-presence, not field-type) of the same general class: **a test double that is a strict subset of the real wire shape can make a whole class of bug permanently invisible to the tests meant to catch it.**

**Disposition:** any fixture standing in for a live read-modify-write response (component lists, or any other entity Jira returns with a server-assigned id) must include the id field by default, matching the live wire shape, not the minimal shape convenient to hand-author. Treat an id-less fixture for an id-bearing live resource as a fixture defect in its own right, not a simplification. DEFERRED to F7 as a general RMW-fixture-fidelity convention (candidate: a lint/convention note in test-fixture helper doc comments requiring id-bearing shapes for any resource Jira assigns ids to).

_Trigger: S-605-1 Step-4.5 (2026-08-18), R6 HIGH-1 masked for 5 rounds by an id-less `issue_response_with_components` fixture; recurrence of the FIX-576-DL mock-vs-live drift class._
_Tagged: [process-gap] [test-fixture-fidelity] [mock-vs-live-drift] [S-605-1] [FIX-576-DL-recurrence]_

## A read-only adversary dispatch without an explicit worktree-identity tuple can be misled by the orchestrator's own session-start git snapshot

S-605-1 Step-4.5 R1's Lens B (adversary, read-only) pass HALTED with a false "code absent / wrong worktree" claim. Root cause: the pass was misled by the SESSION-START git snapshot embedded in its dispatch context, which described the main repo checked out on `develop` — not the story's actual `.worktrees/S-605-1` checkout — so the adversary, reading the snapshot rather than independently confirming its own working directory, concluded the story's code didn't exist yet. This is a recurrence of the `S668-STEP45-DISPATCH-MISSING-IDENTITY-TUPLE` / `ORCHESTRATOR-DISPATCH-MISSING-WORKTREE-IDENTITY-TUPLE` process-gap class already tracked from S-668-1 and S-604-3. It was mitigated in-cycle (not left to recur silently for the rest of the story) by baking explicit worktree-path discipline plus an orchestrator-verified ground-truth HEAD SHA into every subsequent Step-4.5 adversary dispatch this story, which is why R1 was the only round to hit it.

**Disposition:** every adversary/reviewer dispatch that touches a story worktree must carry an explicit identity tuple — (worktree-abs-path, feature-HEAD-SHA, story-id, canonical-repo-root) — in the dispatch prompt itself, and the dispatched agent must be instructed to treat that tuple as authoritative over any session-start snapshot baked into its inherited context. This is now the fourth recorded instance of this pattern (S-668-1, S-604-3 near-miss, and now S-605-1 R1 as a full false-halt, not just a near-miss) — DEFERRED to F7 as an engine-level (`vsdd-factory`) fix to the adversary-dispatch template, not a `jira-cli` product defect.

_Trigger: S-605-1 Step-4.5 R1 (2026-08-18), Lens B false "code absent / wrong-tree" halt from a stale session-start git snapshot._
_Tagged: [process-gap] [worktree-identity] [adversary-dispatch] [S-605-1] [S668-STEP45-DISPATCH-MISSING-IDENTITY-TUPLE-recurrence]_

## Render/aggregation functions with per-key status branches need explicit per-branch coverage and a mutation-catch check on every silent-success setter

S-605-2's Step-4.5 window found the SAME class of defect four separate times against the SAME function, `render_bulk_component_results` (the per-key result-rendering/aggregation for the bulk `issue edit --component` path): R3 caught a partial-failure branch (MED) where a mutated `any_failed`-style flag could flip without any test failing; R6 caught the identical class on the inaccessible-key branch (LOW); R7 on the out-of-chunk-failed branch (LOW); R8 on duplicate-row/row-count accounting (LOW). In every instance the branch was reachable in normal Jira usage (not a defensive "shouldn't happen" arm), the code compiled and the happy-path tests stayed green, and only a targeted mutation-testing pass — flipping the branch's status literal or its `any_failed`-equivalent boolean — exposed that no test actually asserted the branch's specific output. This mirrors the pre-existing `STEP45-MISSED-CONTRACT-BUGS-PR-REVIEW-CAUGHT` and `LOOSE-CONTAINS-MASKS-BC-VERBATIM-MESSAGE-DRIFT` process-gap items in shape (a function with silently-untested branches surviving a "3/3 CLEAN" declaration) but is narrower and more mechanical: it is specifically about render/aggregation functions that fan out over a collection of per-key results and set a silent-success flag (`any_failed`, `all_ok`, a status enum) per branch.

**Disposition:** any render/aggregation function that iterates a per-key or per-item collection and derives an overall success/failure signal from individual branch outcomes needs (a) an explicit test asserting the exact rendered output for every reachable branch, not just the aggregate happy path, and (b) a mutation-catch check specifically targeting the silent-success setter in each branch (flip the branch's status literal or boolean and confirm a test fails). The HUMAN-ADJUDICATED bar this story settled on — bounded to normal-Jira-reachable branches, with cosmetic stream-routing and truly-unreachable defensive branches accepted as deferred, matching the pre-existing shipped `render_bulk_edit_results` sibling's bar — is a reasonable STANDING convention, not a one-off exception, and should be applied proactively (checked during implementation, not discovered four times per adversary round) to any future render/aggregation function of this shape. DEFERRED to F7 as a general render-coverage convention/lint candidate (a checklist item or a mutants.toml-scoped requirement targeting per-key status-setter branches in render functions), rather than opened as a standalone follow-up story this cycle.

_Trigger: S-605-2 Step-4.5 (2026-08-19), `render_bulk_component_results` silent-success mutation surfaces recurring at R3/R6/R7/R8._
_Tagged: [process-gap] [codified] [render-coverage] [S-605-2] [mutation-catch]_
