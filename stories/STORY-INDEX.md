...
total_waves: 4
status: complete
last_updated: 2026-06-18 (S-TESTTOOL-1 added; test-tooling hardening — cargo-mutants baseline scope + keyring-test gate; no BCs; 79→80; F3 COMPLETE)
...
| S-TESTTOOL-1 | Test-tooling hardening — cargo-mutants baseline scope (`src/api/jira/issues.rs` + `src/cache.rs` added to `examine_globs`) + keyring-test gate (`tests/auth_profiles.rs::global_profile_flag_targets_auth_status` gated behind `JR_RUN_KEYRING_TESTS=1`); CLAUDE.md roster doc-fallout (MAINT-MUTANTS-GLOBS-01, #526-F6-KEYRING-GATE) | — (no BCs; test/CI-quality changes only) | — | **draft** — F3 COMPLETE (2026-06-18); no deps; awaiting F4 dispatch | xsmall (2 SP) |
