...
total_waves: 4
status: complete
last_updated: 2026-06-16 (S-522 added; push_text/push_code CR normalization on generic ADF parser path; feature mode F3; BC-7.2.011 EC-11/INV-push-text-cr; 76→77)
...
| S-522 | Fix `push_text` / `push_code`: add `contains('\r')` fast-path guard + two-pass `replace("\r\n","\n").replace('\r',"\n")` normalization at both generic-path chokepoints; rename+de-ignore+invert `test_lone_cr_survives_pre_existing_492_oos`; flip `prop_492_arbitrary_string_holds_core_invariants` `strict_cr` false→true; add 3 new unit tests (closes #522) | BC-7.2.011 (EC-11 / INV-push-text-cr) | — | **ready** — F3 COMPLETE (2026-06-16); no deps; awaiting F4 dispatch | xsmall (1 SP) |
...
| S-522 | feature-followup (feature mode F3; push_text/push_code CR normalization on generic ADF parser path; BC-7.2.011 EC-11/INV-push-text-cr; 76→77; 2026-06-16; no deps) | /Users/zious/Documents/GITHUB/jira-cli/.factory/stories/S-522-adf-push-text-cr-normalization.md |
