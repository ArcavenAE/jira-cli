...
| S-MAINT-CR-005 | `OffsetPage::items()` underuse refactor (CR-005) — replace direct `.body.issues` / `.body.items` field accesses in `get_issue_types_for_project` with the `OffsetPage<T>` abstraction already used elsewhere in the codebase (see DRIFT-331-PAGINATION). Origin: 2026-06-19 maintenance sweep Bundle D. | — (no new BCs; refactor only) | — | **draft** — 2026-06-19 maintenance sweep; awaiting F1 dispatch | small |
