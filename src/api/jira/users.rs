...
impl JiraClient {
    ...
    /// Search for users assignable to issues in a project (before an issue exists).
    ///
    /// Uses the `/user/assignable/multiProjectSearch` endpoint which returns users
    /// eligible for assignment on the specified project.
