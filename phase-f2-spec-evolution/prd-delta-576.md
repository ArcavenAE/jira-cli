...
BC-2.7.002. All design decisions ratified by D-179.
...
| BC-3.9.003 | --public → servicedeskapi two-step; D-174 confirmation gate; --yes bypass |
...
| BC-3.9.008 | attachment delete → DELETE/id; 404 = exit 64 + body (D-168) |
...
| BC-3.9.014 | upload confirmation gate mechanics (THREE consumers: --public standalone, --replace-existing ≥1-match, combined; D-174 eprint!+read_line, NOT dialoguer) |
...
## Ratified Design Rulings (D-179)

All the following were ratified at the F1 gate (D-179):
...
- **D-174 confirmation gate**: `eprint!` + `io::stdin().lock().read_line()` is the ratified interactive mechanism. `dialoguer::Confirm` is forbidden (returns `Err(NotConnected)` on piped stderr before reading input).
- **D-168 precedent on delete**: 404 on a targeted DELETE = exit 64 + surface Jira body (NOT silent exit 0). Same ruling as comment delete (BC-3.5.004).
...
| P14-004 (LOW) | LOW | impact-boundary-576.md | APPLIED | §2.2 BC-3.9.008 row retro-annotated: "404 → exit 0" superseded by D-168 (shipped BC-3.9.008 = exit 64 + surface body). §2.3 NFR idempotency row retro-annotated similarly. |
...
| P37-002 (INFO) | INFO | prd-delta-576.md | APPLIED | BC-3.9.014 one-liner updated from `--public confirmation gate mechanics: eprint!+read_line, NOT dialoguer` to `upload confirmation gate mechanics (THREE consumers: --public standalone, --replace-existing ≥1-match, combined; D-174 eprint!+read_line, NOT dialoguer)`. |
...
| 10 | INFO-3: BC-2.7.012 download-scope comment | ACCEPTED-CARRIED | — | Multi-sentence rationale; folding to one sentence would lose the D-168 / read-vs-write divergence context |
