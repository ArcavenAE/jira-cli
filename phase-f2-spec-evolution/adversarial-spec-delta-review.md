...
PROCESS LESSONS (codified — see D-038): never pre-write a verdict before the pass returns; never batch speculative future passes+fixes+commits; fix edits must be surface-validated; the commit d6f0826 referenced in early (corrected) records never existed — real fix commit is 3d29f8d.
