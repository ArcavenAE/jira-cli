...
| T-2 | `src/error.rs` | `insufficient_scope_display_includes_workarounds` | 170–186 (construction at **171**) | YES (line 180) | **YES** — construction call at line 171 adds `required_scope: None`; assertion at line 180 unchanged (None-fallback preserves `write:jira-work` literal in Display) |
...
T-1 and T-2 require construction-call updates only. Both are construction sites in `src/error.rs` that need `required_scope: None` added when the variant signature widens under option (a): **2 test construction-call updates needed in `src/error.rs`** (lines 131 + 171). No assertion updates needed anywhere — T-1's assertion pins only `exit_code() == 2` (unaffected), T-2's assertion at line 180 is preserved by the None-fallback which renders the `write:jira-work` literal in Display, and T-3 is likewise satisfied by the None-fallback at C-2.
