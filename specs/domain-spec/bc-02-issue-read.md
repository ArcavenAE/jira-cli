...
| INV-READ-009 | Team column gating: team-name resolution (cache read + UUID→name map) runs only when output format is Table AND `team_field_id` is `Some`; JSON mode skips all team-cache I/O. Implemented as nested `if` blocks (not a let-chain; let-chains require Rust ≥1.88; MSRV is 1.85). | `cli/issue/list.rs::handle_list`, NEW-INV-223 |
