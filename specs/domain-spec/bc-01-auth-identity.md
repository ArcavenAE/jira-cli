...
| INV-AUTH-004 | OAuth state parameter: 32 bytes from `OsRng` → 64 hex chars. State is validated against CSRF at callback. A state mismatch → login error; keychain NOT written. | `api/auth.rs` (BC-1146) |
