...
**Display-sanitization cross-reference (SEC-576-011)**: the `<raw>` value in the degenerate-name warning (`"warning: using id as filename for attachment <AID> — original name '<raw>' could not be sanitized."`) is a server-supplied attachment filename and MUST be display-sanitized (all ASCII control characters 0x00–0x1F and 0x7F replaced with `?`) before writing to a TTY, per BC-2.7.011 display-sanitization requirement. RAW value retained in JSON mode (this warning is a hint, suppressed in JSON mode — no exposure vector in that path).
