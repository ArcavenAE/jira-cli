...
| Permission denied (EACCES / read-only FS; `ErrorKind::PermissionDenied` \| `ReadOnlyFilesystem`) | 1 | `Permission denied: cannot write to <dir>: <os_error>. Check directory permissions and try again.` (single mode; batch mode: per-file fail-soft per BC-2.7.008) |
