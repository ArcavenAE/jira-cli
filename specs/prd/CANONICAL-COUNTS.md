...
| bc-3-issue-write.md | 139 | 139 | YES |
...
| cross-cutting.md | 96 | 96 | YES |
| **Total individually-bodied** | **542** | — | — |
...
| bc-3-issue-write.md | 168 |
| bc-4-assets-cmdb.md | 32 |
| bc-5-boards-sprints.md | 36 |
| bc-6-config-cache.md | 44 |
| bc-7-output-render.md | 97 |
| bc-8-components.md | 28 |
| cross-cutting.md | 162 |
| **Sum** | **772** |

### Grand total

**Canonical grand total: 772** (+2 BC-X.16.001..002 added 2026-09-25 via cycle-014
`issue-triage-quickfixes` F2 spec evolution, human-approved F1 gate (D-378/D-379), issue #583 —
new `## BC-X.16: API Query Parameters` subsection in cross-cutting.md (160→162 cumulative,
94→96 individually-bodied): `jr api --query-param NAME=VALUE` repeatable flag — merge with an
existing `?`, repeated same-name params sent in order, single-pass percent-encoding, and
method-orthogonal application, plus a companion malformed-value error taxonomy (BC-X.16.002).
Same burst: BC-X.7.002 (cross-cutting.md) AMENDED in place — issue #862, `jr user list`'s
local-flag → global-flag → configured-default `--project` resolution order. Root cause:
`UserCommand::List.project` was a clap-required `String` field, so clap's own required-argument
validation rejected the invocation before any global-flag propagation could run, and
`handle_list` had no `Config`-default fallback either. COUNT-NEUTRAL;
BC-X.14.001/BC-X.14.003 (cross-cutting.md) AMENDED in place — issue #861, READ-SIDE ONLY per the
F1 human gate (D-378) — M1/M2 label-resolution fallback to `name` when `value` is absent, fixing
system-field (`priority`/`resolution` (when present on the Create/Edit screen)/`versions`/
`fixVersions`/`components`/`security`/`issuetype`) `"(unnamed)"` labels; the WRITE-side `find_option_match`/`resolve_option_value` fix
originally proposed for #861 was REMOVED from scope after a fresh-context audit at the F1 gate
refuted its reachability for system-typed fields (tracked as drift item
`FIELD-SYSTEM-TYPES-UNSUPPORTED`), COUNT-NEUTRAL. Same burst: BC-X.14.004 (cross-cutting.md)
gains one cross-reference row in its error-taxonomy table (the pre-existing empty-`<field>`
guard, citing BC-X.14.001 EC-X.14.001-15) — documentation-only, COUNT-NEUTRAL. Was 770 before this addition; prior: +1
BC-X.15.001 added 2026-09-17 via cycle-008
`oauth-surface-correctness` F2 spec evolution, ADR-0026 — new `## BC-X.15: OAuth Agile-Command
Error-Mapping` subsection in cross-cutting.md (159→160 cumulative, 93→94 individually-bodied):
`jr board`/`jr sprint` 401 auth-scheme-conditional call-site rewrite disambiguating scope-mismatch
vs. expired-token vs. (regression-guard) wrong-host, VP-OAUTH-GW-003, modeled on
`require_service_desk`/BC-X.8.006..007. Same burst: BC-4.2.001 (bc-4-assets-cmdb.md) AMENDED in
place — OAuth 3LO gateway-routing invariant + unified 7-call-site fix table, VP-OAUTH-GW-001,
COUNT-NEUTRAL (32/22 unchanged); BC-1.3.023 (bc-1-auth-identity.md) AMENDED in place — 7 granular
Jira-Software/Agile scopes added to `DEFAULT_OAUTH_SCOPES`, Teams scopes explicitly excluded,
VP-OAUTH-GW-002, COUNT-NEUTRAL (83/72 unchanged); BC-5.1.001 (bc-5-boards-sprints.md) gains a
cross-reference note, COUNT-NEUTRAL (36/18 unchanged). Was 769 before this addition; prior: +12
BC-3.3.013..015 + BC-3.4.033..037 + BC-3.8.019..022 added 2026-09-12 via cycle-012 `field-adf-autoconvert` F2 spec evolution — `--field NAME=VALUE` ADF auto-conversion on `issue edit` (platform), `issue create` (platform), and `issue create --request-type` (JSM); bc-3-issue-write.md 156→168 cumulative, 127→139 individually-bodied; was 757 before this addition; prior: +3 BC-1.6.048..050 added 2026-09-10 via cycle-007
...
`windows-correctness` F2 spec evolution, issues #759/#760, D-334, ADR-0021 (new) + ADR-0022 (new)
...
read path + distinct corrupt-file error, BC-1.4.037 versioned-envelope atomic file write,
BC-1.4.038 delete-both-backends, BC-1.4.039 honest-fail `DpapiFallbackFailed` backstop, BC-1.4.040
profile-name-as-filename path-traversal guard on the new secret file); API-token `cloud_id`
acquisition via unauthenticated `/_edge/tenant_info` closing A-PA-LOW-001 (BC-1.2.052 fallback-chain
acquisition, BC-1.2.053 mechanism-switch refresh-not-clear, BC-1.2.054 confirmed-unchanged
`base_url()`/`assets_base_url` auth-method gating); BC-1.4.028 amended in place (partial-state
error extended to check the DPAPI file before erroring), no separate count; in
bc-1-auth-identity.md (71→80 cumulative, 60→69 individually-bodied); bc-6-config-cache.md
unaffected by this pass (a planned BC-6.2.016 cross-reference amendment was deferred — pre-existing
TD-031 stable-anchor hygiene violation in that file blocks edits, unrelated to this cycle's diff);
was 733 before this addition; prior note: +2 BC-1.1.016 + BC-1.4.034 added 2026-09-01 via cycle-003
`auth-profile-dx` F2-gate FIX round, same day as the F2 spec evolution below — BC-1.1.016 closes
adversarial finding I-1 (non-interactive OAuth guard for explicit `--oauth`/implicit oauth-method
`refresh`); BC-1.4.034 formalizes the one-time re-login breaking-change contract BC-1.4.032's
no-copy redesign requires; BC-1.4.032/033 REDESIGNED in place (no-copy detect-and-instruct model,
D-315/ADR-0020 §Decision 2a) and BC-1.2.014 reworded in place (credentials-before-config-entry
ordering), no separate count; in bc-1-auth-identity.md (69→71 cumulative, 58→60
individually-bodied); was 731 before this addition; prior note: +12 BC-1.1.013..015 +
BC-1.2.048..051 + BC-1.4.031..033 + BC-1.6.047
added 2026-09-01 via F2 spec evolution, cycle-003 `auth-profile-dx` bundle, D-312..325, ADR-0020
(new) + ADR-0011 (amended) — per-profile API-token credential storage and one-time lazy migration
(D-315: BC-1.4.031/032/033), non-destructive `auth logout` + 4-step `auth remove` (D-322:
BC-1.2.013/014 amended), OAuth-default-at-creation + non-interactive/runtime-default regression
pins (D-313: BC-1.1.013/014/015), `auth refresh` mechanism-override removal (D-321: BC-1.2.051),
`--oauth` deprecation + new `--api-token` flag (D-323: BC-1.2.049/050), `auth list` ENV column +
JSON shape (D-314/324: BC-1.6.046 amended + BC-1.6.047); BC-1.1.009/010, BC-1.2.017, BC-1.4.025/
...
gains an additive `env: Option<String>` tag (D-314); BC-6.2.015 amended in place — ADR-0011
hard-fence un-defer (D-317), no separate count; in bc-6-config-cache.md (43→44 cumulative, 33→34
...
D-188's `--field`-alone platform-path exit-64 guard, D-310 registered 2026-08-26); `--field
...
Breakdown:
- 772 = sum of per-file `total_bcs` values (canonical; see per-file table above)
- 542 of 772 are individually-bodied (have a `#### BC-` heading)
...
- BC-X.4.009 (ADV-P1-029) is a `#### BC-` heading in cross-cutting.md; it is
  included in cross-cutting's `total_bcs: 162` and in the **772 sum**.
  Every later addition through cycle-014's +2 (BC-X.16.001..002) is likewise entirely
  individually-bodied, no range-collapsed entries — so the 230 range-collapsed figure above is
  ...
  after +2 BCs (BC-1.1.016 + BC-1.4.034 added to bc-1-auth-identity.md); BC-1.4.032/033
  REDESIGNED in place (no-copy detect-and-instruct model) and BC-1.2.014 reworded in place
  (credentials-before-config-entry ordering); no separate count for these amendments; was 731
  before this addition; prior note: F2 spec evolution, cycle-003 `auth-profile-dx` bundle: 731
  total after
...
| bc-06-config-cache.md | 43 | bc-6-config-cache.md | 44 | PENDING (L2 not bumped by the 2026-09-01 cycle-003 `auth-profile-dx` F2 delta — F2 touched L3 only; +1 BC BC-6.1.015 added D-314/ADR-0020 §4; prior: YES, bumped 2026-06-27; +1 BC-6.2.018 added cache warm-hit no-HTTP invariant) |
...
- Group 15 (Comment CRUD, SOH-COMMENT-CRUD-1 F2 2026-07-09/2026-07-10, D-168): H-NEW-COMMENT-001..H-NEW-COMMENT-005 (BC-3.5.005/008/004/010/003; issue #577) — +5
...
- ADR-0017: First multipart/streaming HTTP surface — reqwest multipart+stream features + tokio-util direct dependency (SOH-ATTACHMENTS-1 F2 D-179, 2026-07-15)
- ADR-0018: Component resolution, caching, delete-safety, and mutation-wire-shape strategy (component-mgmt F2 gate, D-278/279/280; issues #604/#605/#606/#608, 2026-08-15)
...
- ADR-0020: Per-Profile Credential Ownership, Environment Tagging, and OAuth-Default-at-Creation (auth-profile-dx F2 gate, D-312..316; cycle-003, 2026-09-01)
- ADR-0021: Windows OAuth Secret Storage — Keyring-First with DPAPI-Encrypted-File Fallback (windows-correctness F2 gate, D-334; cycle-004, issue #759, 2026-09-03)
