...
**Subject**: Boards & Sprints
**Behavior**: Column gating is conjunctive — all three conditions required (Table mode AND configured field AND ≥1 populated UUID). Affects `jr sprint current`, `jr board view`, and `jr issue list`. Implementation form differs by call site: `src/cli/board.rs::handle_view` and `src/cli/issue/list.rs::handle_list` (S-cycle13 let-chain conversion) fold the first two conditions into a single let-chain guard — `if matches!(output_format, OutputFormat::Table) && let Some(field_id) = team_field_id { … }` — with the third condition (`if uuids.iter().any(…)`) remaining a nested `if` inside that block; `src/cli/sprint.rs::handle_current` is unaffected by the let-chain conversion (its outer condition is already a `match` arm, not an adjacent `if`, so there is no `if`/`if let` pair to fold) and continues to express the Table case as a `match` arm (`OutputFormat::Table => {…}`), with the remaining two conditions as nested `if`s inside that branch.
**Postconditions**:
1. When `output_format` is `Table`, `team_field_id` is configured (`Some(_)`), and `uuids.iter().any(|u| u.is_some())` is `true` for the result set, `show_team_col` is `true`, the "Team" header column appears in the table output, and each row includes a team cell with one of three values: (a) the resolved team name string when `team_map.get(uuid)` hits; (b) the raw UUID string verbatim when `team_map.get(uuid)` returns `None` (cache miss); or (c) the literal `"-"` when the issue's own `team_id` is `None` — the mixed-set case where `uuids.iter().any(|u| u.is_some())` is true but this particular issue has no team field set.
**Trace**: Pass 3 BC-1138a/c (R4); `"-"` mixed-set arm (`src/cli/board.rs::handle_view`, `src/cli/issue/list.rs::handle_list`, `src/cli/sprint.rs::handle_current` — `None => "-".to_string()`) has no dedicated test fixture and is outside `examine_globs` in `.cargo/mutants.toml`
...
**Behavior**: Team column is omitted in table output when `team_field_id` is not configured (`None`) or when `team_field_id` is configured but no issue in the result carries a non-None team UUID. At `src/cli/board.rs::handle_view` and `src/cli/issue/list.rs::handle_list` (S-cycle13 let-chain conversion), the two outer conditions (`Table` output format and `team_field_id` configured) are folded into one let-chain guard, so the unconfigured-field case now falls through to that guard's single combined `else { Vec::new() }` branch — at `src/cli/board.rs::handle_view:~255` and `src/cli/issue/list.rs::handle_list:~787` — rather than a separate outer `if`; the no-populated-UUIDs case remains a distinct inner `else { Vec::new() }` branch nested inside the let-chain body — at `src/cli/board.rs::handle_view:~252` and `src/cli/issue/list.rs::handle_list:~784`. `src/cli/sprint.rs::handle_current` is unaffected by the let-chain conversion and retains two separate nested `if`/`else` branches: inner branch (no populated UUIDs) at `src/cli/sprint.rs::handle_current:~312`; outer branch (unconfigured field) at `src/cli/sprint.rs::handle_current:~315`. In every case `team_displays` ends up empty and `show_team_col` is `false`.
**Postconditions**:
...
**Trace**: Pass 3 BC-1138b/d (R4); S-cycle13 let-chain rewrite adds the combined-guard `else { Vec::new() }` branch for board view and issue list
...
**Confidence**: HIGH
**Source**: `tests/team_column_parity.rs::sprint_current_falls_back_to_uuid_when_team_not_cached` (no-suffix clause verified by the `.not()` assertion on the `(name not cached…)` suffix in this test; this test reaches `src/cli/sprint.rs::handle_current` only); `tests/team_column_parity.rs::test_board_view_falls_back_to_uuid_when_team_not_cached` (pins BC-5.3.003 postcondition for the `src/cli/board.rs` render site); `tests/cli_handler.rs::test_list_team_column_falls_back_to_uuid_when_cache_missing`
