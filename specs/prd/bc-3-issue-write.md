...
4. **Interactive prompts** — project key (interactive fallback), type, and summary; description is NOT prompted by `handle_create` (`--description`/`--description-stdin` flags only).
...
AC-13 (NEW) `test_platform_create_combined_empty_on_behalf_with_field_exits_64_combined_error` [mode: human]: EC-3.8.012-1 as test — `jr issue create --on-behalf-of "" --field a=b` (no `--request-type`) → combined-error string present (`stderr.contains("--field and --on-behalf-of are only valid with")`); exit 64; single-flag strings absent (per AC-3 assertion spec). Covers EC-3.8.012-1 sub-case where `--on-behalf-of ""` (empty string value) co-occurs with `--field`. S-639-1 F3 story deliverable.
AC-14 (NEW) `test_platform_create_empty_request_type_routes_jsm_not_bc_3_8_012` [mode: human]: EC-3.8.012-2 as test — `jr issue create --field a=b --request-type ""` → BC-3.8.016 fires (empty RT guard inside JSM path), NOT BC-3.8.012; exit 64; stderr does NOT contain `"--field is only valid with"` (the BC-3.8.012 guard string). S-639-1 F3 story deliverable.
...
AC-16 (NEW) `test_platform_create_on_behalf_empty_string_exits_64_013_error` [mode: human]: EC-3.8.013-1 as test — `jr issue create --on-behalf-of ""` (empty value, `--field` absent, no `--request-type`) → BC-3.8.013 fires; stderr contains `"--on-behalf-of is only valid with"` (single-flag string) AND does NOT contain `"--field and --on-behalf-of are only valid with"` (combined string); exit 64. S-639-1 F3 story deliverable.
