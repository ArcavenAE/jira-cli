...
**Completeness caveat:** This list covers the guard-relevant ordering only. Type and summary exit-64 fallbacks (`src/cli/issue/create.rs` ~:119 and ~:130) and `--markdown`→ADF conversion with BC-7.2.012 recursion-depth guard (~:163-169) occur within steps 4–5 and are intentionally elided. For the intersection of the BC-3.8.012 guard with `--markdown` + `--field`, see EC-3.8.012-5: the guard fires at step 2, so `--markdown` ADF conversion at step 5 is never reached on the guarded path.
