...
**CLI flags** (pinned for e2e surface guard): `<KEY>` (positional, required); `<FILE>...` (positional, repeatable, 1+); `--public`; `--internal`; `--yes`; `--replace-existing`; `--dry-run`; `--output json`; `--no-input`; `--profile <NAME>`; `--no-color`.
