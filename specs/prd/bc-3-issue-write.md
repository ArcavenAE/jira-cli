...
**SINGLE SOURCE OF TRUTH** for the guard-relevant ordering (authoritative for step numbering) in `handle_create` (platform branch). BC-3.8.012 (step 2) and BC-3.8.013 (step 2) reference this block rather than embedding copies. When changing any step, update ONLY this block.
...
**AC namespace note:** AC-1..AC-21 below are the S-639-1 acceptance-criteria targets to be authored verbatim into the S-639-1 story at F3; they SUPERSEDE S-383's same-numbered ACs (historical, do-not-implement). Until S-639-1 exists, THIS Trace is the authoritative AC source. Step-ordering statements in ACs (e.g., "guard fires at step 2 before project-key resolution at step 3") reference the Platform-Path Guard Ordering SSOT block (~:2971 — "**SINGLE SOURCE OF TRUTH** for the guard-relevant ordering (authoritative for step numbering) in `handle_create`"); that block is authoritative; per-AC statements are informational summaries only. Every negative assertion in an AC MUST name the invocation element that makes it falsifiable; structurally-unfalsifiable negatives are hygiene and must be labeled as such.
