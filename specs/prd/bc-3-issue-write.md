...
trace: |
  ...
  - F2 spec evolution, Field DX bundle (2026-08-25, issues #580/#578): +8 individually-bodied
    ...
    2026-08-25]` section added; DEC-310 (registered 2026-08-26, human-approved at the F2 gate;
    reverses DEC-188) recorded the governance reversal).
...
- EC-3.3.010-6a **[ADDED 2026-08-26, F2 adversary-convergence round-5, F-NEW-1 — governed set widened 5→9, ADR-0019 § "D2 correction (adversary F-NEW-1)"; COUNT CORRECTED 9→10, 2026-08-26, F2 adversary-convergence round-6]**: EC-3.3.010-6's five collision examples (`summary`/`description`/`issuetype`/`priority`/`components`) are NOT the complete governed set on `issue create` — the create-path guard covers THREE more static-key collisions (`labels`/`parent`/`assignee`) plus TWO resolved-id keys (`points` and `team` — DISTINCT `customfield_NNNNN` wire keys, each a separately governed member, NOT one combined "resolved-id category" as round-5 wrongly framed it), none of which existed in the original D2 execution. TEN wire keys total: 5 original Gate B keys + 3 new static keys + 2 new resolved-id keys:
  ...
  See ADR-0019 § "D2 correction (adversary F-NEW-1)" for the full ten-member table and rationale.
...
F-NEW-1; COUNT CORRECTED 9→10, 2026-08-26, F2 adversary-convergence round-6]** `create.rs`'s guard is CREATE-path-only and passes `detect_flag_field_overlap` a
TEN-member wire-key set (`summary`/`description`/`issuetype`/`priority`/`components`/`labels`/
`parent`/`assignee`, plus TWO SEPARATE resolved-id keys `--points`/`--team` — each a distinct
`customfield_NNNNN` wire key — via the `customfield_NNNNN` bypass form
...
- EC-3.4.017-16 (NEW, issue #578 F2, adversary pass-13 F-1): `jr issue edit KEY --priority
  ...
  **for `priority` and every other field this EC's own examples cover, all of which are members
  ...
  own set is TEN members **[COUNT CORRECTED 9→10, 2026-08-26, F2 adversary-convergence round-6]**
  ...
  resolved-id category — beyond this Gate B's five), per BC-3.3.010 EC-3.3.010-6a and ADR-0019 §
...
**Invariants**:
1. The `plannedChanges` field shapes are INTENTIONALLY SIMPLIFIED previews that do NOT match live-edit wire payloads (except the hint-path composed shapes per F-NEW-2 — see item 3 above) **[QUALIFIER ADDED 2026-08-26, F2 adversary-convergence round-6, LOW — this lead-in was not updated when round-5's F-NEW-2 hint-path exception was introduced at Postconditions — Common item 3 above, and over-generalized without it]**:
...
**Platform-vs-JSM asymmetry (load-bearing, do not conflate) — [CORRECTED 2026-08-26, S-578-3 implementation adjudication, supersedes the pre-implementation wording below, retained for audit trail]:** the byte-identical claim above (VP-578-007) holds on the **platform `issue create`/`issue edit` path only** (wire target `fields`), where `:option` uses the object-wrap `{"value": ...}` shape (BC-3.4.016) and bare uses BC-3.4.016's auto-detect dispatch — genuinely two different code paths that happen to converge on the same resolved id. On the JSM create path (`--request-type` set), the asymmetry runs the OTHER way: BC-3.8.008's amendment gives bare `kind: None` AND `:option` the SAME unconditional string-wrap (`{"cf": "V"}`) — `:option` **is** byte-identical to bare on JSM (bare-parity, VP-578-015), unlike on the platform path. It is the JSM-path `:id`/`:name`/`:asset` hints that diverge from bare on JSM (object/array wraps), and those three remain UNVERIFIED against live JSM (see BC-3.8.008 amendment CAVEAT, narrowed to `:id`/`:name`/`:asset`) — only `:option`'s bare-parity on JSM is settled, backed by VP-578-015's regression pin rather than by analogy.
**[Pre-implementation wording, superseded 2026-08-26, retained for audit trail]:** "the byte-identical claim above (VP-578-007) holds on the platform `issue create`/`issue edit` path only (wire target `fields`). On the JSM create path (`--request-type` set), BC-3.8.008's amendment gives bare `kind: None` a DIFFERENT wire shape (unconditional string-wrap, `{"cf": "V"}`) than `:option` (`{"cf": {"value": "V"}}`) — so `:option` is not byte-identical to bare on JSM. Confidence note (adversary pass-8 M-2 correction): the ASYMMETRY conclusion itself (bare ≠ `:option` on JSM) is confirmed — the two shapes are structurally different regardless of live verification. But the exact `{"cf": {"value": "V"}}` non-cascading shape shown here is (JSM wire shape UNVERIFIED — see BC-3.8.008 amendment CAVEAT): only the platform-path `fields` wire contract was CONFIRMed by research; the `requestFieldValues` shape is asserted by analogy, not verification." — this described a shape that was never implemented; S-578-3 implemented bare-parity for `:option` on JSM instead.
...
7. **[ADDED 2026-08-26, F2 adversary-convergence Pass 1, P1-001 — DECISION: ENFORCE_GATE, orthogonality ruling]** EC-3.4.027-1's entry-point `schema.type ∈ {"option", "option-with-child"}` gate and Invariant 6/EC-3.4.027-7's structural `children`-empty check are TWO DIFFERENT QUESTIONS, asked in strict sequence, and do not conflict with each other or with Invariant 6's "structural, not `schema.type`" framing:
   - **Question 1 (EC-3.4.027-1, entry gate): "Is this an option field AT ALL?"** — answered ONCE via a `schema.type` closed-set membership check, before any `allowedValues`/`children` inspection.
   - **Question 2 (Invariant 6 / EC-3.4.027-7, structural check): "Among fields ALREADY CONFIRMED option/option-with-child by Question 1, is THIS SPECIFIC matched parent entry cascading or plain?"** — answered structurally via `children.is_empty()`, NEVER by a second `schema.type` lookup, precisely because Question 1 already settled the field-level type question; re-deriving it from `schema.type` at this second point would be redundant, not merely non-idiomatic.
   Invariant 6's "not a `schema.type` lookup" language describes ONLY how Question 2 is answered — it says nothing about, and does not preclude, Question 1's own `schema.type` gate running first. A reviewer who reads Invariant 6 in isolation and concludes `schema.type` inspection is disallowed anywhere in this BC is over-generalizing a rule scoped to one specific structural distinction. Both checks are REQUIRED and coexist: EC-3.4.027-1 fires first (or not at all, if the field passes); Invariant 6/EC-3.4.027-7 fires only for fields that already passed EC-3.4.027-1, on a per-matched-entry basis.

**Edge Cases**:
- EC-3.4.027-1 **[MADE UNAMBIGUOUS 2026-08-26, F2 adversary-convergence Pass 1, P1-001 — see Invariant 7 below for the orthogonality ruling this EC depends on]**: `:option` hint on a resolved field whose `schema.type` is NOT `"option"` and NOT `"option-with-child"` → exit 64, no PUT/POST. This is an ENTRY-POINT type gate: it MUST run immediately after the field's `editmeta`/`createmeta` `schema.type` is known and BEFORE the `:option` composer inspects `allowedValues` or `children` at all (i.e. before EC-3.4.027-2/3/6/7's allowedValues/children-based checks, which all presuppose `schema.type` already passed this gate). Two sub-cases, both exit 64 via this same gate, but with different message content because they were already differently handled by the pre-existing bare-form dispatch:
  - **`schema.type` is `"array"` or `"any"`** (a type BC-3.4.015/BC-3.4.016 already treat as wholesale unsupported by `--field`, hinted or not): the `:option` composer MUST call the SAME code path BC-3.4.015 Step 4 already uses for this case — reuse EC-3.4.015-5's exact message/exit behavior, not a re-derived one. Pinned assertion convention (mirrors the existing `test_bc_3_4_015_*` tests): stderr contains the literal schema-type string (`"array"` / `"any"`).
  - **`schema.type` is anything else the bare form DOES support** (`string`/`number`/`date`/`datetime`/`user`, or any future scalar type BC-3.4.016 Step 4 dispatches on): the field is NOT unsupported in general — only unsupported for the `:option` hint specifically, since the hint declares an option-field intent the resolved type contradicts. This is a DISTINCT message from EC-3.4.015-5 (do not reuse "unsupported type" wording, which would misleadingly imply the field can't be set at all). Load-bearing substrings: `"is not an option field"` and the literal resolved `schema.type` string (e.g. `"schema type 'string'"`), naming both the field and its actual type, with a hint to drop `:option` and use the bare form instead. **This message-content split (reuse EC-3.4.015-5 vs. distinct hint-mismatch message) is itself load-bearing and must not be collapsed into one generic message** — the two sub-cases have different actionable remediation (drop `--field` entirely for array/any vs. drop `:option` specifically for string/number/date/datetime/user).
  - **Non-goal, explicitly**: this gate is a closed-set `schema.type` membership check only. It never inspects `allowedValues` content or `children` — an `option`/`option-with-child` field with empty/null `allowedValues` is NOT caught here (that is EC-3.4.016's existing "no configured option values" case, unaffected by this gate) and must not be conflated with it.
...
2. **[CORRECTED 2026-08-26, F2 adversary-convergence Pass 1, P1-006]** For system fields ALSO reachable via a dedicated named flag (currently only `priority`), the `--field NAME:name=VALUE` wire body MUST be BYTE-IDENTICAL to the dedicated flag's wire body for the same value — this is a consistency guarantee on OUTPUT, not an implementation-technique mandate. **This invariant does NOT require the two code paths to share a single wire-composition function call** — `--priority`'s dedicated-flag path and `--field NAME:name=VALUE`'s hint-composer path MAY compose the identical `{"name": "<VALUE>"}` shape via independent, textually-duplicated code, provided the two resulting JSON bodies are byte-for-byte equal (VP-578-010 asserts the OUTPUT equality, not any shared-function implementation detail). Prevents drift between two ways of setting the same field, verified at the wire-output boundary — not by inspecting whether a function was reused.
...
- EC-2 (`:asset` malformed `WORKSPACE:OBJECTID` shape) **[SCOPE NOTE ADDED 2026-08-26, F2 adversary-convergence round-3, F-A; SUB-CASE COUNT CORRECTED 2026-08-26, F2 adversary-convergence round-3, F-C — three sub-cases (EC-2a/b/c) was stale; EC-2d below is a fourth; HTTP-SCOPE CORRECTED 2026-08-26, F2 adversary-convergence Pass 1, P1-005]**: covers FOUR sub-cases, all exit 64, **no workspace-discovery `GET` and no `PUT`/`POST`** — this is narrower than a blanket "no HTTP" claim: the field-resolution `editmeta`/`createmeta` `GET` (BC-3.4.015/BC-3.3.010 Step 3, required to confirm the field is on the Edit/Create screen) has ALREADY occurred by the time this BC's `WORKSPACE:OBJECTID` parsing runs, per this BC's own Preconditions ("Same field-resolution preconditions as BC-3.4.015"). What these four sub-cases genuinely guarantee is that the value-shape check fires and exits 64 BEFORE the `:asset`-specific `get_or_fetch_workspace_id` discovery `GET` (BC-3.4.030's error taxonomy) and BEFORE any field-write `PUT`/`POST` — not before the issue's own field-resolution `GET`, which is unavoidable on this call site regardless of hint kind. **This is the ONLY `:kind` in this catalog whose empty-value form is a client-side exit-64** — and even then, for a STRUCTURAL reason, not a value-validation one: the `:asset` composer must build a `[{workspaceId, id, objectId}]` object-reference array, and an empty `objectId` gives it nothing to compose an `id`/`objectId` field from — there is no valid array to send, so `jr` rejects it before attempting to build one. Contrast EC-8/EC-9 below (`:id`/`:name` empty value), which are explicitly NOT exit-64, because `:id`/`:name` never compose a structured shape from `VALUE` — they wrap it verbatim (`{"id": ""}`/`{"name": ""}`), so an empty string is just another unvalidated value the server is free to accept or reject, per BC-3.4.028/029 Invariant 1's "server is sole validator" posture. Do not generalize EC-2a's exit-64 outcome to `:id=`/`:name=` — see BC-3.4.028 EC-3.4.028-3 / BC-3.4.029 EC-3.4.029-3 for the full rationale.
...
preserved — see Verification Property below); `Some(Option)` (non-cascading — including an
opaque `Parent>Child` literal produced when a cascading-shaped value is supplied, see
EC-3.8.008-1) → **[CORRECTED 2026-08-26, S-578-3 implementation adjudication — supersedes the
pre-implementation `{"value": ...}` object-wrap immediately below, retained for audit trail]**
the SAME unconditional string-wrap as `kind: None` — `build()`'s kind-aware dispatch places
`kind: None` and `Some(Option)` in ONE shared arm, both producing `{"cf": "V"}`. `:option` is
therefore bare-parity on this (JSM) path — byte-identical to the pre-existing, already-shipped,
VP-578-015-pinned bare-form wire shape — NOT the platform-path `fields`-target `{"value": ...}`
shape (BC-3.4.016/BC-3.4.027). This is a deliberate JSM-vs-platform divergence, not an
oversight: this BC's own pre-existing (pre-#578) Behavior paragraph above already documents
`requestFieldValues` as a flat NAME→string-JSON map for every resolved value, so there was never
an established JSM-side `{"value": ...}` convention for `:option` to preserve, and the
object-wrap shape below was never wire-verified in the first place (see the CAVEAT immediately
following, now narrowed to `:id`/`:name`/`:asset`). Bare-parity also avoids touching the shared,
already-tested `kind: None` dispatch arm — see BC-3.4.027's "Platform-vs-JSM asymmetry"
paragraph, corrected in the same pass, for the reciprocal update.
**[Pre-implementation wording, superseded 2026-08-26 by the correction above, retained for audit
trail]:** "`Some(Option)` → `{"value": ...}` (non-cascading)" — this shape was drafted by
analogy to the platform-path `fields` contract before implementation began; S-578-3 implemented
bare-parity instead (story AC-002 "bare/`:option` parity", test
`test_bc_3_8_008_build_kind_aware_dispatch_option_bare_parity`), and this BC is corrected to
match the shipped, tested-green behavior rather than the earlier draft.
**[CAVEAT, adversary pass-6 F3, EXTENDED adversary pass-16 MEDIUM-2; NARROWED 2026-08-26,
S-578-3 implementation adjudication — `:option` is now OUT OF SCOPE for this caveat, see the
correction above]: the `:id`/`:name`/`:asset`
`requestFieldValues` shapes described below (`{"id": ...}` / `{"name": ...}` /
`[{"workspaceId","id","objectId"}]`) are ALL UNVERIFIED against live JSM.** Research CONFIRMed
only the platform-path `fields` wire contract for these kinds
...
wire shape specifically for `:id`/`:name`/`:asset` (nor, historically, for `:option` before the
bare-parity correction above removed `:option` from needing wire verification at all —
bare-form JSON-string wrapping is `jr`'s own pre-existing, already-shipped behavior, not an
Atlassian wire contract being guessed at by analogy). Every one of `:id`/`:name`/
`:asset` on `requestFieldValues` is asserted here by analogy to the platform-path shape, not by
verification. `:asset` in particular is at least as likely to diverge as any hinted kind —
Assets
attribute payloads are the least standardized of the four across Atlassian's JSM vs platform
surfaces. Do not read `:id`/`:name`/`:asset` as settled fact merely because an earlier version
of this caveat's scope differed; the pre-pass-16 wording below is retained for audit trail only.
...
unverified status as the cascading shape below rather than being stated as settled fact." (This
pre-pass-16 quote describes the now-superseded `:option` object-wrap draft; retained verbatim
for audit trail, not current contract.)
...
- EC-3.8.008-3 **[ADDED 2026-08-26, F2 adversary-convergence round-6, LOW — explicit pin, previously only implied by transitivity; CORRECTED 2026-08-26, S-578-3 adversary Pass-3 finding ADV-S578-3-P3-001 — supersedes the undifferentiated "no separate JSM-specific pre-flight check" clause, retained below for audit trail]**: this EC's two example classes rest on two DIFFERENT mechanisms, not one shared mechanism — they must not be treated as a single undifferentiated claim.
  - **Unknown `:kind` TAG** (e.g. `--field cf:bogus=X`): rejected by `parse_field_kv`'s own closed-set `:kind`-TAG check (BC-3.4.031's catalog: `option`/`id`/`name`/`asset`), which fires on this JSM path (per the "Hint-kind uniformity" amendment above, `parse_field_kv` is the SAME shared parser on both call sites) BEFORE any `POST /rest/servicedeskapi/request` call, identically to the platform-path shape BC-3.4.031 documents. This half of the original claim is correct: `parse_field_kv` validates only the `:kind` TAG itself, uniformly and request-type-agnostically, as a direct consequence of running as a single shared parse pass before either path's request-body construction.
  - **Structurally malformed `:asset` VALUE** (e.g. `--field cf:asset=` — empty value; an empty workspace segment; an extra colon; a non-numeric objectId): NOT rejected by `parse_field_kv` — that parser validates only the `:kind` TAG and has no `:asset`-value-shape logic at all; `cf:asset=` parses successfully to `FieldValueSpec { kind: Asset, value: "" }`, a structurally valid parse. Malformed-`:asset`-VALUE rejection is performed by a SEPARATE, per-call-site L2 value-shape check: `field_resolve.rs::compose_asset_hint` on the platform path, and `jsm_create.rs::resolve_asset_field_l2` on this (JSM) path — each independently validating the `WORKSPACE:OBJECTID` shape before its own workspace-discovery call. **A JSM-specific L2 pre-flight check for malformed `:asset` values DOES exist and IS needed** — it is not, and never was, an emergent property of `parse_field_kv`'s single-pass parsing. See BC-3.4.030 EC-3.4.030-3 + BC-3.4.031 EC-2a/EC-2b/EC-2c/EC-2d for the value-shape catalog this JSM-path check mirrors.
  - **What is still true of the original claim:** the "rejected BEFORE any POST/workspace GET" guarantee holds for BOTH example classes — but by two different mechanisms (the shared `parse_field_kv` kind-tag catalog vs. the per-call-site L2 value-shape check), not by one shared parser-level mechanism as the superseded wording below implied.

  **[Pre-correction wording, superseded 2026-08-26 by ADV-S578-3-P3-001, retained for audit trail]:** "`parse_field_kv`'s shared unknown-`:kind` / malformed-`:asset` exit-64 catalog (BC-3.4.031) fires on this JSM path (per the "Hint-kind uniformity" amendment above, `parse_field_kv` is the SAME shared parser on both call sites) BEFORE any `POST /rest/servicedeskapi/request` call — i.e. a malformed hint on `--request-type` create (e.g. `--request-type RT --field cf:bogus=X`, or `--field cf:asset=` with a structurally invalid `:asset` value) exits 64 with zero HTTP POST, identically to the platform-path shape BC-3.4.031 documents. This is a direct consequence of `parse_field_kv` running as a single, request-type-agnostic parse pass before `JsmRequestBuilder::build()`/`handle_jsm_create` ever construct the request body — no separate JSM-specific pre-flight check is needed, and none exists." This is the reciprocal of the correction already applied to this story's AC-005 in v1.2 (adversary Pass-1 finding ADV-S578-3-P1-003), which corrected the same false premise on the story side; this BC-level correction closes the matching spec-side gap.

**Verification Properties**:
- VP-578-015: A bare (unhinted) `--field NAME=VALUE` on the JSM create path produces BYTE-IDENTICAL `requestFieldValues` wire output before and after this amendment — the kind-aware dispatch is purely additive for `kind: None`. Regression pin against `tests/issue_create_jsm.rs`'s existing `--field` wire-shape assertions (BC-3.8.005..007's `summary`/`description`/`priority`/`labels` keys, which sit in the same `rfv` map, are UNTOUCHED by this amendment). **[EXTENDED 2026-08-26, S-578-3 implementation adjudication]** This byte-identity claim also covers `Some(Option)` (non-cascading `:option`, including the opaque-literal cascading-attempt case, EC-3.8.008-1): `build()`'s dispatch places `kind: None` and `Some(Option)` in the SAME arm, so a `:option`-hinted pair produces the IDENTICAL `{"cf": "V"}` string-wrap a bare pair carrying the same value would — see the "Hint-kind uniformity" amendment's correction, above, which supersedes the amendment's original `{"value": ...}` draft for `:option`.
...
> **[AMENDED 2026-08-25 issue #578, DEC-310 (registered 2026-08-26, human-approved at the F2 gate; reverses DEC-188)]**
...
> decision entry — **DEC-310** (registered 2026-08-26, human-approved at the F2 gate; next
> sequential after the highest DEC number found across the ENTIRE `.factory/` tree, DEC-309 — a
> `grep -rohE "DEC-[0-9]{3}" .factory/` survey run during the F2 adversary-convergence pass,
> 2026-08-26; registered inline in spec prose per this repo's convention, since no centralized
> DEC registry file exists). **Correction (F2 adversary-convergence pass, C-M1):** an earlier
...
> full-tree maximum (DEC-309) yields DEC-310, not DEC-307. DEC-310 was registered 2026-08-26
> (human-approved at the F2 gate) via the existing inline-prose convention; a future pass MUST
> NOT silently reuse DEC-310 (or DEC-307, already taken) for an unrelated decision.
...
**Trace**: issue #578 item 2; DEC-310 (registered 2026-08-26, human-approved at the F2 gate, this amendment); supersedes DEC-188 (#639, 2026-07-25) for the `--field`-alone and combined-guard cases only; BC-3.3.010/011 (new resolution + error taxonomy this guard removal delegates to); BC-3.8.008 (JSM-path `--field` amendment, wire-target substitution note); BC-3.8.013 (unmodified, `--on-behalf-of` guard survives standalone)

[AMENDED 2026-08-25 issue #578 F2: DEC-188's --field-alone and combined guards REVERSED; --on-behalf-of guard (BC-3.8.013) unaffected; DEC-310 (registered 2026-08-26, human-approved at the F2 gate) recorded this reversal]
...
> **[CURRENT BEHAVIOR — effective 2026-08-25, issue #578]**
> Post-reversal (see BC-3.8.012's `[AMENDED 2026-08-25 issue #578, DEC-310 (registered 2026-08-26)]` note
...
issue #578, DEC-310 (registered 2026-08-26)]:** BC-3.8.012's combined check is REMOVED (see BC-3.8.012
