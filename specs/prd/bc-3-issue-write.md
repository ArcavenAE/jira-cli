...
**AC namespace note:** AC-1..AC-16 below are the S-639-1 acceptance-criteria targets to be authored verbatim into the S-639-1 story at F3; they SUPERSEDE S-383's same-numbered ACs (historical, do-not-implement). Until S-639-1 exists, THIS Trace is the authoritative AC source.
...
AC-12 (NEW) `test_platform_create_help_flags_requires_request_type_in_help`: `jr issue create --help` stdout contains the substring `"requires --request-type"` for the `--field` entry AND for the `--on-behalf-of` entry (per the verbatim strings in delivery item (d), both include this exact substring). Assert `stdout.contains("requires --request-type")` — this pins both flag entries simultaneously. Pins delivery item (d) help-string update. S-639-1 F3 story deliverable.
...
**Trace**: `tests/issue_create_jsm.rs` — see BC-3.8.012 Trace for full AC namespace note (AC-1..16 are S-639-1 targets; supersede S-383 ACs).
