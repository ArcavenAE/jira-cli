...
**Error taxonomy — bare `:asset=<objectId>` cold-cache workspace discovery failure [ADDED 2026-08-26, F2 adversary-convergence pass, B-LOW]**: the bare form's cold-cache `get_or_fetch_workspace_id` GET (`GET /rest/servicedeskapi/assets/workspace`, `src/api/assets/workspace.rs`) can itself fail; this BC pins that behavior explicitly (previously undocumented):
| Condition | Behavior |
|---|---|
...
None of these four outcomes is client-side pre-validated before the GET fires — this is a genuine HTTP round-trip on a cold cache (warm-cache reads never reach this code path at all, per VP-578-011). This taxonomy applies identically whether the triggering `:asset=<objectId>` hint appears on `issue edit --field` or `issue create --field` (BC-3.3.010) — the L2 handler on each path calls the same `get_or_fetch_workspace_id` function (ADR-0019 §2's "L2 resolves, `build()` only wraps" split).
...
- VP-578-022 **[NEW 2026-08-26, B-LOW]**: each row of the cold-cache workspace-discovery error taxonomy above is independently exercised via wiremock (403/404 → Assets-unavailable UserError exit 64; 200 + empty `values` → no-workspace UserError exit 64; 401 → standard auth-error mapping; 5xx/network error → standard API/network-error mapping), on BOTH the edit and create call sites.
