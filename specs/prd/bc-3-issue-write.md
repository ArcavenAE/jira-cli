...
**AC namespace note:** AC-1..AC-19 below are the S-639-1 acceptance-criteria targets to be authored verbatim into the S-639-1 story at F3; they SUPERSEDE S-383's same-numbered ACs (historical, do-not-implement). Until S-639-1 exists, THIS Trace is the authoritative AC source.
...
AC-6 (`test_jsm_create_with_field_and_request_type_does_not_fire_bc_3_8_012`) [mode: --output json] — NOT unchanged post-DEC-188; test body MUST be updated at F3. The original negative assertion `!stderr.contains("--field is ignored on the platform create path")` is now vacuously true (that substring no longer exists anywhere). Updated AC-6 contract: exit 0 AND `!stderr.contains("--field is only valid with")` AND `!stderr.contains("--field and --on-behalf-of are only valid with")` — pinning that the new pre-flight guard MUST NOT mis-fire on the JSM path (i.e., when `--request-type` IS present, BC-3.8.012 must remain silent).
