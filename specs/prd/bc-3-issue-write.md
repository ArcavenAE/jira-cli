...
- EC-3.8.012-9: `--field ""` (empty string VALUE) without `--request-type` → `field_pairs` is non-empty (clap stores `[""]`); guard fires on `!field_pairs.is_empty()` normally; exit 64. The guard fires on presence only — the value is never parsed at this stage. Non-goal: combined-flags `--output json` is deliberately not a separate test case — the JSON envelope shape is identical to the single-flag case (covered by AC-10); the combined-flags case is tested in human mode by AC-3.
...
AC-1 `test_platform_create_field_flag_exits_64_without_request_type` [mode: --output json] (renamed from
`test_platform_create_field_flag_emits_warning_without_request_type`):
test body MUST invert the prior `stdout.contains("PROJ-123")` assertion to `stdout.is_empty()` at F3;
AC-2 `test_platform_create_on_behalf_of_flag_exits_64_without_request_type` [mode: --output json] (renamed from
...
AC-7 `test_platform_create_malformed_field_without_request_type_exits_64` [mode: --output json] (renamed from
