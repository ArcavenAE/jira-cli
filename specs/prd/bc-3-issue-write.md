...
**VP-577-027**: EC-3.5.002-2 KEY URL-encoding pin — verify the request URL path segment is correctly encoded: `jr issue comment delete "MY KEY-1" --id 10001 --yes` against a wiremock mounted at `DELETE /rest/api/3/issue/MY%20KEY-1/comment/10001` → exit 0; the wiremock route receives exactly one hit (confirming the space was percent-encoded to `%20` before URL construction). A wiremock route mounted at the un-encoded path `MY KEY-1` receives zero hits. Test-writer note: `urlencoding::encode` is the normative crate; the key `MY KEY-1` is a synthetic fixture exercising the space-encoding path. (Added adversary pass-35 F-A3.)

**Trace**: F2 spec evolution (2026-07-09, DEC-168; adversary pass-17 F1 VP-577-022; adversary pass-35 F-A3 EC-3.5.002-2 KEY URL-encoding + VP-577-027; issue #577 SOH-COMMENT-CRUD-1)
...
**Trace**: F2 spec evolution (2026-07-09, DEC-168 ruling 3; adversary pass-2 MEDIUM-1 remediation; adversary pass-6 MEDIUM-1 JR_STDIN_IS_TTY seam; adversary pass-8 L1 seam-scope; adversary pass-11 F1 Term::stderr(); adversary pass-22 F4 delivery-task obligation added; adversary pass-35 F-A5 EC-3.5.003-3 dialoguer Err → JrError::Interrupted exit 130; issue #577 SOH-COMMENT-CRUD-1)
...
**VP-577-025**: `jr issue comment edit FOO-1 --id 10001 "Updated text" --internal` → exit 0; stderr contains `"(marked internal)"` (the human-channel echo marker from BC-3.5.005 Response 200 output). Wiremock mounts PUT returning 200. **Second variant (`--public --yes` path)**: `jr issue comment edit FOO-1 --id 10001 "Updated text" --public --yes` → exit 0; stderr contains `"(marked public)"`. Wiremock mounts PUT returning 200. Pins the `" (marked internal)"` / `" (marked public)"` human echo markers defined in BC-3.5.005 Response 200 output and EC-3.5.008-2 Confirm path. (Added adversary pass-32 F2.)
...
- **(1) `--internal` path**: `jr issue comment edit FOO-1 --id 10001 "Updated text" --internal --output json` → exit 0; stdout parses as JSON; `changed_fields["jsm_internal"]` equals `serde_json::Value::Bool(true)` — boolean, NOT the string `"true"`.
- **(2) `--public --yes` path**: `jr issue comment edit FOO-1 --id 10001 "Updated text" --public --yes --output json` → exit 0; `changed_fields["jsm_internal"]` equals `serde_json::Value::Bool(false)` — boolean, NOT the string `"false"`.
- **(3) default body-only path**: `jr issue comment edit FOO-1 --id 10001 "Updated text" --output json` → exit 0; `changed_fields` object does NOT contain the key `"jsm_internal"` at all — assert `stdout_json["changed_fields"].as_object().unwrap().contains_key("jsm_internal")` is `false` (key ENTIRELY ABSENT, not null, not false).
...
**Trace**: F2 spec evolution (2026-07-09, DEC-168 ruling 1; research verdict Claim 1 REFUTED-footgun; adversary pass-5 M6b marker (since resolved — PRESERVED verdict 2026-07-09); adversary pass-32 F2 VP-577-025 human-echo-marker pin; adversary pass-34 F-577-A VP-577-026 jsm_internal boolean-type + key-absence parse pin; adversary pass-35 F-A2 Other-4xx/5xx-except-401 + 401-auth-path clause; issue #577 SOH-COMMENT-CRUD-1)
...
**VP-577-028**: EC-3.5.008-4 `--yes` silent-no-op pin [ORCHESTRATOR RULING, fix round 40 — F2 human gate required before closing]: `jr issue comment edit FOO-1 --id 10001 "text" --yes` (WITHOUT `--public`) against a wiremock returning PUT 200 → exit 0; wiremock receives exactly one PUT hit (the `--yes` flag does NOT suppress the edit operation); stderr does NOT contain any error substring relating to `"--yes"` being unexpected or invalid. **Second variant (clap-level no-requires pin)**: `jr issue comment edit --help` output does NOT contain a `requires --public` constraint on `--yes` (string-level assertion against help text). (Added adversary pass-35 F-A4.)

**Trace**: F2 spec evolution (2026-07-09, DEC-168 open design point Option a; adversary pass-2 MEDIUM-1 + LOW-2 remediation; adversary pass-5 L1 VP-577-017; adversary pass-9 F1 EC-3.5.008-3 prescriptive + VP-577-017 second-variant; adversary pass-35 F-A4 EC-3.5.008-4 --yes silent-no-op (orchestrator ruling) + VP-577-028; F-A5 EC-3.5.008-5 dialoguer Err → JrError::Interrupted; issue #577 SOH-COMMENT-CRUD-1)
...
**Trace**: F2 spec evolution (2026-07-09, DEC-168 ruling 2 Option A; adversary pass-1 HIGH-1 + LOW-1; adversary pass-2 MEDIUM-2 + LOW-3 + MEDIUM-4 remediation; adversary pass-3 HIGH-2 + MEDIUM-4 + LOW-4 remediation; adversary pass-5 L2 VP-577-018; adversary pass-7 F3 VP-577-019; adversary pass-8 L3 VP-577-020; adversary pass-25 VP-577-020 mixed-case variant; adversary pass-32 F4 BC-3.4.011 removed from EC-3.5.012-5 item (a); adversary pass-35 F-A1 EC-3.5.012-5 items (f)+(g) README+CLAUDE.md migration obligations; issue #577 SOH-COMMENT-CRUD-1)
