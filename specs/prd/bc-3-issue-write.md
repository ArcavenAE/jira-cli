...
not per-value). **Implementation constraint (MUST NOT):** this guard MUST NOT be realized via clap `#[arg(requires = "request_type")]` — that attribute yields exit 2 pre-handler, violating SSOT step 2 and colliding with AC-15's clap-exit-2-precedence pin (see EC-3.8.012-8); the guard MUST be a hand-rolled check inside `handle_create` returning `JrError::UserError` (same model as `edit.rs`'s flag-guard pattern).
...
produces one error. **Implementation constraint (MUST NOT):** this guard MUST NOT be realized via clap `#[arg(requires = "request_type")]` — that attribute yields exit 2 pre-handler, violating SSOT step 2 and colliding with AC-15's clap-exit-2-precedence pin (see EC-3.8.012-8); the guard MUST be a hand-rolled check inside `handle_create` returning `JrError::UserError` (same model as `edit.rs`'s flag-guard pattern).
