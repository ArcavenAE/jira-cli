...
**Non-interactive path** (`--no-input` OR stdin is not a TTY): the gate is NOT presented; `jr` exits 64 immediately with the hint message. No servicedeskapi calls are issued; no DELETEs are issued.
