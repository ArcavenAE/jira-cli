...
- **403** (if surfaced by endpoint variant) → exit 64 + surface body per the primary clause above. **Carve-out**: when the 403 response body contains an OAuth-scope indicator (case-insensitive substring `"scope"`), the handler MUST re-wrap as `JrError::InsufficientScope` (exit 2, matching `jr`'s established OAuth re-consent UX at `client.rs` 401-scope branch and BC-3.8.014/015) instead of `UserError`. All other 403 causes route to exit 64 + surfaced body. Promoting 403-scope detection into `parse_error` (general-purpose, parallel to the 401 branch) is a **follow-up story candidate** (see v1.3.28 Follow-up Obligations). This 403 clause is inherited in full by BC-3.5.005 and BC-3.5.010.
...
- **403** (if surfaced by endpoint variant) → inherits BC-3.5.004's 403 clause in full, including the scope-indicator carve-out (`JrError::InsufficientScope` when body contains `"scope"`, exit 2; all other 403s exit 64 + surface body).
...
**Response 403** (if surfaced by endpoint variant) → inherits BC-3.5.004's 403 clause in full, including the scope-indicator carve-out (`JrError::InsufficientScope` when body contains `"scope"`, exit 2; all other 403s exit 64 + surface body).
...
**EC-3.5.012-5** (try_parse regression obligations, story-input scope): The `src/main.rs` `try_parse()` refactor required by EC-3.5.012-1 modifies the whole-CLI clap error path. The implementing story (S-577-1) MUST include regression-test obligations for the following surfaces: (a) BC-3.4.011 cross-hierarchy `--type` 400 hint; (b) BC-3.7.003/004 remote-link error paths; (c) BC-3.8.010 JSM create error paths; (d) `--help` snapshot tests; (e) `tests/e2e_cli_surface_guard.rs` SURFACE table — the existing single `comment` row MUST be replaced with four rows (one each for `comment add`, `comment delete`, `comment edit`, `comment view`) each carrying its own flag set; (f) `tests/e2e_live.rs` flat-form sweep per EC-3.5.012-2 (all `"issue", "comment"` call sites updated to `comment add` form in the same PR as the CLI refactor). Regression suites for each surface MUST pass unchanged post-refactor.
