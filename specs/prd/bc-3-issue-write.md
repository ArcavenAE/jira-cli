...
trace: |
  - Adversary pass-28 fix round (2026-08-25, issue #578, MEDIUM F-1): BC-3.3.010's Postconditions
    ...
    resolution behavior (realized in `.factory/specs/verification-delta/` by the formal-verifier
  ...
  - F2 spec evolution, Field DX bundle (2026-08-25, issues #580/#578): +8 individually-bodied
    ...
    2026-08-25]` section added; DEC-307 proposed to record the governance reversal).
...
- `--project <KEY>` resolved (flag or profile default) BEFORE this resolution step (guard ordering: JSM dispatch fork → [no pre-flight `--field` guard, per BC-3.8.012 reversal] → project-key resolution → field resolution → POST).
...
**Trace**: issue #578 (item 2); BC-3.4.015 Edge Cases (EC-3.4.015-1..8, substituted "Create screen" for "Edit screen"); BC-3.4.016 Edge Cases (EC-3.4.016-2/3/8); BC-3.3.010 (resolution algorithm this taxonomy attaches to)
...
[AMENDED 2026-08-25 issue #578 F2: DEC-188 --field echo-suppression qualifier reversed per DEC-307; --on-behalf-of suppression retained]
...
counterparts. See EC-3.4.017-16. This is EDIT-path-only, matching Gate B's existing scope;
BC-3.4.029 EC-3.4.029-2 documents the CREATE-path counterpart, where no Gate B guard exists
and last-wins applies instead.
...
- EC-3.4.017-16 (NEW, issue #578 F2, adversary pass-13 F-1): `jr issue edit KEY --priority
  ...
  EC-3.4.029-2 documents the CREATE-path counterpart of this same flag combination (no Gate
  B exists there — last-wins applies); this EC is the EDIT-path resolution, closing the
  contradiction between the two BCs.
...
**Invariants**:
1. **Bare form is permanent, not deprecated.** `--field NAME=VALUE` with no `:kind` suffix on an `option`-schema field continues to auto-detect via BC-3.4.016's existing label/id fuzzy-match dispatch, forever. Hints are additive/opt-in; they do not narrow or deprecate the unhinted path. (Resolves BA open question §5.2.)
...
- VP-578-008: Cascading parent>child composition produces the `{"value":...,"child":{"value":...}}` wire shape. Delimiter CONFIRMED — split on the FIRST literal `>` only, with `:id=` as the documented escape hatch for a display value containing a literal `>` (ADR-0019 §3, Accepted 2026-08-25). No longer PROVISIONAL.

**Trace**: issue #578 item 1; BC-3.4.016 (shared resolution logic); BC-3.4.026 (parser); `.factory/research/field-dx-feasibility-2026-08-25.md` claim 6
...
- EC-3.4.029-2 **[AMENDED 2026-08-25 issue #578 F2, adversary pass-13 F-1 fix]**: `--field priority:name=Medium` combined with `--priority Medium` in the same invocation, **on the CREATE path** (`jr issue create`, BC-3.4.014 — no Gate B mutual-exclusion guard exists on create) → LAST-WINS at the `fields` JSON object merge step (standard "later flag wins" `jr` convention); no special-case guard preventing both from being supplied. This is a permissive but consistent behavior on create, not a hard conflict — no exit 64. **This does NOT apply on the EDIT path.** On `jr issue edit`, the identical flag combination is NOT last-wins: BC-3.4.017 Gate B matches a hint-tagged `--field NAME:kind=VALUE` pair on its BARE NAME (the `:name` suffix is stripped before the map key is formed, per BC-3.4.026's bare-key rule), so `--priority Medium --field priority:name=Medium` fires Gate B identically to the bare `--field priority=Medium` form → exit 64, no HTTP. **Previous version (superseded, contradicted BC-3.4.017 Gate B by claiming universal last-wins with no path scoping):** "`--field priority:name=Medium` combined with `--priority Medium` in the same invocation → LAST-WINS at the `fields` JSON object merge step ...; no special-case guard preventing both from being supplied. This is a permissive but consistent behavior, not a hard conflict — no exit 64." See BC-3.4.017 EC-3.4.017-16 for the full hint × Gate-B interaction across all five system fields and every hint kind.
...
**Parsing rule**:
1. `VALUE` is split on the FIRST `:` (within the already-extracted `NAME:asset=VALUE` value portion — this is a SECOND colon-split, independent of the `NAME:kind` split in BC-3.4.026 step 2, operating on the post-`=` value string).
2. If a `:` is present: left segment = explicit `workspaceId`, right segment = `objectId`. If NO `:` is present: the entire `VALUE` is the `objectId`, and `workspaceId` is resolved via the EXISTING cached workspace-id lookup (`get_or_fetch_workspace_id`, per-profile, 7-day TTL, BC-4.2.001) — the SAME cache `jr assets search` already warms; no new cache family, no duplicate HTTP call if the cache is warm.
3. `objectId` MUST be non-empty and match `\d+` (numeric) — a non-numeric or empty `objectId` segment → exit 64 (BC-3.4.031 EC-3).
...
**Edge Cases**:
- EC-3.4.030-1: Bare `:asset=12345` (no `workspace:` prefix) → resolves `workspaceId` via the per-profile cache (warm) or `GET /rest/servicedeskapi/assets/workspace` (cold, then cached) — see BC-3.4.031 EC-2 for the malformed-shape catalog entry.
...
- VP-578-012: `objectId`-only malformed shapes (non-numeric, empty) exit 64 before any HTTP call — property test per the F1 delta analysis §3 recommendation (parallels `prop_sanitize_attachment_filename_no_path_traversal`, VP-576-001).
...
- EC-2 (`:asset` malformed `WORKSPACE:OBJECTID` shape): covers three sub-cases, all exit 64, no HTTP:
  ...
  - EC-2c: `:asset=:12345` (workspace segment empty, colon present) → treated as a malformed EXPLICIT-workspace form (NOT the bare-objectId form, since a colon IS present) → exit 64, "workspace segment cannot be empty when ':' is present; omit the workspace prefix entirely to use the cached workspace id".
- EC-3 (`:asset` non-numeric `objectId`): `--field cf:asset=abc` (bare form, non-numeric) or `--field cf:asset=ws:abc` (explicit form, non-numeric objectId segment) → exit 64, "objectId must be numeric".
...
**Verification Properties**:
- VP-578-013: Every edge case above (EC-1, EC-2a/b/c, EC-3, EC-5) is exercised by a dedicated unit test asserting exit 64, zero HTTP, and the documented load-bearing substring.
...
no longer exits 64 [reversed 2026-08-25 issue #578 DEC-307: `--field` no longer exits 64 — resolves via
...
BEFORE helper resolution calls (`resolve_team_field`, `resolve_assignee_by_project`). BC-3.8.012's `--field`-alone exit-64 guard was reversed 2026-08-25 issue #578 DEC-307 — `--field` no longer exits 64 and instead resolves via createmeta per BC-3.3.010/BC-3.8.012.

---

#### BC-3.8.001: `issue create --request-type <NAME|ID>` dispatches to `POST /rest/servicedeskapi/request`; platform POST body, JSON response, and exit code unchanged when `--request-type` absent (unless `--on-behalf-of` present — exits 64 per BC-3.8.013; `--field` no longer exits 64 — reversed 2026-08-25 issue #578 DEC-307: resolves via createmeta per BC-3.3.010/BC-3.8.012)
...
**SINGLE SOURCE OF TRUTH** for the guard-relevant ordering (authoritative for step numbering) in `handle_create` (platform branch). BC-3.8.013 (live step-2 guard) and BC-3.8.012 (step-2 `--field` guard REMOVED per the 2026-08-25 DEC-307 reversal; references this block for removal/historical context) reference this block rather than embedding copies. When changing any step, update ONLY this block.

Short guard-ordering reference for `src/cli/issue/create.rs::handle_create` when `--request-type` is absent. The BC-3.8.013 `--on-behalf-of` pre-flight guard fires at **step 2**, immediately after the dispatch fork, **before** project-key resolution:
...
2. **BC-3.8.013 pre-flight guard** — `--on-behalf-of` present → `JrError::UserError` exit 64. Zero HTTP. No interactive prompts. No project-key resolution has run yet. (`--field` no longer triggers this guard — see BC-3.8.012's 2026-08-25 reversal; a bare `--field` now falls through to steps 3–6 and resolves via `createmeta`, per BC-3.3.010.)
...
4b. **`--field` createmeta field resolution** — per BC-3.3.010: when one or more `--field NAME=VALUE` flags are present, resolves each via `GET /rest/api/3/issue/createmeta/{project}/issuetypes/{issueTypeId}` (project+issueType-scoped fields endpoint, matching BC-3.3.010 step 3 / ADR-0019 §1 — NOT the bare `.../issuetypes` list endpoint), validates, and merges the resolved value(s) into the create POST body — same machinery as `issue edit --field` (BC-3.4.015/016), source substituted. This is a distinct step from, not a substitute for, the `--field`-triggered `get_issue_types_for_project` name→id lookup described in BC-3.3.010 step 3 (adversary pass-16 LOW-2): that lookup DOES hit the bare `.../issuetypes` list endpoint, once per invocation, to resolve `issueTypeId` before this step's project+issueType-scoped fields fetch runs. The "NOT the bare list endpoint" clause above describes only THIS step's own endpoint choice, not a claim that the list endpoint goes unused on the `--field` create path. Runs AFTER the step-2 `--on-behalf-of` guard and step-3 project-key resolution (both are hard prerequisites: `createmeta` is scoped to the resolved project, and the step-2 guard — when it fires on `--on-behalf-of` — short-circuits before this step is ever reached, per H-NEW-PREFLIGHT-003), and BEFORE the step-6 platform POST. Absent `--field`, this step is a no-op.
...
> **[AMENDED 2026-08-25 issue #578, DEC-307 (proposed — flagged for orchestrator to register)]**
...
> decision entry — proposed ID **DEC-307** (next sequential after the highest DEC number found
> in the live spec tree, DEC-306; NOT yet registered in any formal DEC log, since this repo has
> no centralized DEC registry file — DEC numbers are assigned inline in spec prose by
> convention, per `grep -rohE "DEC-[0-9]{3}"` survey performed during this F2 pass). The
> orchestrator/state-manager should register DEC-307 in whatever mechanism supersedes ad hoc
> inline assignment, and MUST NOT let a future pass silently reuse DEC-307 for an unrelated
> decision.
...
**Trace**: issue #578 item 2; DEC-307 (proposed, this amendment); supersedes DEC-188 (#639, 2026-07-25) for the `--field`-alone and combined-guard cases only; BC-3.3.010/011 (new resolution + error taxonomy this guard removal delegates to); BC-3.8.008 (JSM-path `--field` amendment, wire-target substitution note); BC-3.8.013 (unmodified, `--on-behalf-of` guard survives standalone)

[AMENDED 2026-08-25 issue #578 F2: DEC-188's --field-alone and combined guards REVERSED; --on-behalf-of guard (BC-3.8.013) unaffected; DEC-307 proposed to record this reversal]
...
> **[CURRENT BEHAVIOR — effective 2026-08-25, issue #578]**
> Post-reversal (see BC-3.8.012's `[AMENDED 2026-08-25 issue #578, DEC-307 (proposed)]` note
...
issue #578, DEC-307 (proposed)]:** BC-3.8.012's combined check is REMOVED (see BC-3.8.012
