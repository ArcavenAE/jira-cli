...
any helper HTTP calls (`resolve_team_field`, `resolve_assignee_by_project`).
...
**Test Note**: Test assertions use `stderr.contains(...)`, not `==`, to accommodate the "Error: " prefix prepended at the `src/main.rs` error-rendering site. With `--output json`, the error JSON is written to **stderr** (not stdout) — parse stderr as JSON; assert `code == 64` and `error` contains the guard message substring. Key order is unspecified in the spec (serde_json with BTreeMap serializes alphabetically: `code` before `error`); output is compact, not pretty-printed. Implementers: reuse `tests/json_error_shape.rs::assert_json_error_envelope` per repo convention.
**Edge Cases**:
