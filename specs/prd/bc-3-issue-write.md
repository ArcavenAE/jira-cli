...
AC-10 (NEW) `test_platform_create_field_without_request_type_json_error_shape`: `jr issue create --field a=b --output json` (no `--request-type`) → exit 64; stderr (NOT stdout) contains `{"error":"…","code":64}` with `code` field equal to `64` and `error` field containing `"--field is only valid with"`; stdout is empty. Pins `--output json` error routing on the guarded path. S-639-1 F3 story deliverable.
