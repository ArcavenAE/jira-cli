...
assert `jr issue create --field a=b --field c=d` (two `--field` occurrences) produces stderr byte-identical to the single-`--field` case — the guard checks `!field_pairs.is_empty()` (any non-empty Vec), so n>1 occurrences produce ONE error with no field-name enumeration and no count suffix. AC-5 may be folded into AC-1 with a note if the assertion is identical;
