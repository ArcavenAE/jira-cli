...
AC-3 `test_platform_create_both_inverse_flags_exit_64_combined_error` (renamed from
`test_platform_create_both_inverse_flags_emit_independent_warnings`):
assert combined string present: `stderr.contains("--field and --on-behalf-of are only valid with")`; AND both single-flag strings absent: `!stderr.contains("--field is only valid with")` (combined has "are only valid with", single has "is only valid with" — no shared distinguishing substring) AND `!stderr.contains("--on-behalf-of is only valid with")` (combined has "--on-behalf-of are only valid with", single has "--on-behalf-of is only valid with");
AC-5 `test_platform_create_field_idempotent_one_error_per_logical_flag` (renamed from
...
AC-8 (NEW) `test_platform_create_field_with_helpers_exits_64_zero_http`: `jr issue create --project PROJ --type Task --summary "test" --field a=b --team X --to me` (no `--request-type`) — invocation is complete enough to reach helper HTTP + POST if the guard were absent; → exit 64; `stderr.contains("--field is only valid with")`; wiremock `expect(0)` on ALL endpoints (team-resolution, assignable-user search, issue POST) — zero-HTTP discriminating proof that the guard fires before any helper HTTP call. (CMDB field discovery via `get_or_fetch_cmdb_fields` is post-POST/JSON-only per step 7 of the Guard Ordering; it is not a discriminating mock for this test.) S-639-1 F3 story deliverable.
...
AC-12 (NEW) `test_platform_create_help_flags_requires_request_type_in_help`: `jr issue create --help` — assert TWO per-flag scoped occurrences: (i) find the stdout line(s) describing `--field` and assert that line contains `"requires --request-type"`; (ii) find the stdout line(s) describing `--on-behalf-of` and assert that line contains `"requires --request-type"`. A single `stdout.contains("requires --request-type")` is insufficient — it passes even when only one flag's help text is updated. Implementation: split stdout by newlines; for each flag, locate the first line whose content includes the flag name (`"--field"` / `"--on-behalf-of"`); assert the located line contains `"requires --request-type"`. Alternatively, assert `stdout.matches("requires --request-type").count() == 2`. Pins delivery item (d) help-string update. S-639-1 F3 story deliverable.
