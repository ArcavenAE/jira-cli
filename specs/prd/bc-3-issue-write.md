...
not per-value). **Implementation constraint (MUST NOT):** this guard MUST NOT be realized via clap `#[arg(requires = "request_type")]` — that attribute yields exit 2 pre-handler, violating SSOT step 2 and falsified by AC-1/AC-2/AC-16 (under a `requires` realization, the exit-64 assertion and guard-string contains assertions in those ACs fail; AC-15 is NOT a falsifier — it passes either way); the guard MUST be a hand-rolled check inside `handle_create` returning `JrError::UserError` (same model as `edit.rs`'s flag-guard pattern).
