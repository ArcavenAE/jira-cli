...
invocation: `jr issue create --field a=b --on-behalf-of X` (no `--request-type`); exit 64; assert combined string present: `stderr.contains("--field and --on-behalf-of are only valid with")`; `stdout.trim().is_empty()`; AND both single-flag strings absent: `!stderr.contains("--field is only valid with")` (combined has "are only valid with", single has "is only valid with" — no shared distinguishing substring) AND `!stderr.contains("--on-behalf-of is only valid with")` (combined has "--on-behalf-of are only valid with", single has "--on-behalf-of is only valid with"); REGRESSION PIN: `!stderr.contains("is ignored on the platform create path")` — per Removal postcondition (:3036); NOT vacuous: this invocation class previously emitted the old warn string, so the pin proves it is absent post-DEC-188; S-639-1 F3 story deliverable.
