...
- **(EC-11) Context-aware CR normalization — `push_text` / `push_code` chokepoints (issue #522):** **(INV-push-text-cr)** `AdfBuilder::push_text` inspects the current block context (`self.stack.last()`) and normalizes CR characters CONTEXT-DEPENDENTLY before building any ADF text node. The three-way dispatch is:
  ...
  3. **All other contexts** (heading, paragraph, listItem, taskItem, tableCell, blockquote, panel, inline marks, footnote definitions, inline HTML): `\r\n` → space, lone `\r` → space. This mirrors the treatment of `Event::SoftBreak` (→ space), preserving INV-1 ("no raw `\n` in any non-codeBlock text node"): converting `\r` to `\n` in a heading or paragraph would CREATE a raw `\n` in a non-codeBlock text node (a Jira HTTP 400 hazard); converting to space is safe and semantically closest to a soft line break.

  `AdfBuilder::push_code` (inline code spans — always inline context, never codeBlock): lone `\r` → space. This is **defense-in-depth only**: pulldown-cmark §6.3 converts carriage returns inside inline code spans to spaces before emitting `Event::Code`, so a lone `\r` is not normally reachable via the `markdown_to_adf` public path. The guard exists for direct `push_code` call sites and against future parser changes.

  **INV-1 preservation rationale:** The context-aware rule ensures: (a) no `\r` survives into any text node (the no-raw-CR invariant); (b) no raw `\n` is introduced into a non-codeBlock text node (INV-1). A uniform `\r`→`\n` rule would satisfy (a) but VIOLATE (b) for headings, paragraphs, and all non-codeBlock contexts — the exact defect that triggered the F5 adversarial finding.
  ...
  **Empirical minimal repros (commit 7968d66):**
  ...
  **AC (tests: `test_push_text_normalizes_lone_cr_in_heading_and_code_block`; `test_push_text_normalizes_lone_cr_in_fenced_code_block`; `test_push_text_crlf_two_pass_ordering_deterministic`; `test_push_code_normalizes_lone_cr_in_inline_code`):**
  ...
  - `prop_492_arbitrary_string_holds_core_invariants` calls `assert_no_raw_newline_in_text_nodes` WITHOUT a `strict_cr` parameter (the `strict_cr` parameter was removed; the `\r` check is now unconditional in the helper). No `\r` character survives into any text node from any block type on the generic parser path.
