...
**Reverse path:** `adf_to_text` renders `hardBreak` nodes as `\n` regardless of context. A round-trip through `adf_to_text` therefore reconstructs the LINE STRUCTURE of the original source (minus the trimmed trailing whitespace) with `\n` separators between lines. This is line-structure-lossless for non-URL-bearing content: multi-line structure and `\n\n` interior gaps round-trip correctly. HOWEVER, the round-trip is NOT byte-identical for CR-bearing inputs: `\r\n` (CRLF) and lone `\r` inputs normalize to `\n` in step 3 (EC-1), so a CRLF input round-trips back as LF-only output. Only LF-only inputs are byte-identical on round-trip (minus trimmed trailing whitespace). Exception: a line containing a bare URL at a valid autolink boundary is split into multiple nodes by step 7 (autolink pass); round-trip renders as `[text](url)` not the bare URL literal (lossy for URL-bearing lines — same class as all `autolink_bare_urls` transforms). The `hardBreak`-separator approach is HIGH-confidence per the ADF schema (`hardBreak` is explicitly schema-sanctioned for intra-paragraph line breaks). The exact rendering fate of the OLD raw-`\n`-in-text output from pre-#492 code (whether Jira returned HTTP 400, silently stripped `\n`, or collapsed the break) is live-sandbox-deferred and does NOT gate this fix — the fix is mandated by the ADF schema regardless.
