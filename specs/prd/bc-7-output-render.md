...
- **(EC-11) Generic-path CR normalization — `push_text` / `push_code` chokepoints (issue #522):** **(INV-push-text-cr)** Every call to `AdfBuilder::push_text` and `AdfBuilder::push_code` normalizes the input string for CR characters BEFORE building any ADF text node. If the input contains `\r`, `\r\n` is replaced with `\n` first, then lone `\r` is replaced with `\n`; the normalized string is used to build the text node. If the input contains no `\r`, no allocation is performed. This normalization applies to ALL block types that route through the generic parser path — heading, paragraph, listItem, taskItem, tableCell, blockquote, panel, inline marks, footnote definitions, and inline HTML — because all of these ultimately reach `push_text` for their `Event::Text` / `Event::InlineHtml` content and `push_code` for their `Event::Code` (inline code span) content. This extends INV-1's CR-free guarantee from block HTML only (Algorithm B, EC-9) to the full `markdown_to_adf` generic parser path, closing the pre-existing out-of-#492-scope gap documented in the `strict_cr=false` proptest comment. It is the lone-CR sibling of the raw-`\n` invariant already established in this BC. Mirrors CommonMark §2.3 (lone CR is a line ending). The `NodeKind::HtmlBlock` end arm (Algorithm B) performs its own CR normalization independently in step 3 and is NOT affected by this change (that handler never calls `push_text` — it builds the content array directly). Fix site: `src/adf.rs::AdfBuilder::push_text` and `src/adf.rs::AdfBuilder::push_code`. **Minimal repros (issue #522):** `markdown_to_adf("# x\ry")` → heading text nodes must contain no `\r`; `markdown_to_adf("\ta\r")` → codeBlock text node must contain no `\r`; `` markdown_to_adf("`a\rb`") `` → inline code span text node must be `"a\nb"` (not `"a\rb"`). **AC (tests: `test_push_text_normalizes_lone_cr_in_heading_and_code_block`; `test_push_text_normalizes_crlf_in_paragraph`; `test_push_code_normalizes_lone_cr_in_inline_code`):** After fix, `assert_no_raw_newline_in_text_nodes` with `strict_cr=true` holds for all `markdown_to_adf` outputs including inputs containing lone `\r` or `\r\n`; the previously `#[ignore]`d regression test `test_push_text_normalizes_lone_cr_in_heading_and_code_block` (renamed from `test_lone_cr_survives_pre_existing_492_oos`) has its assertions INVERTED (from `any(|t| t.contains('\r'))` to `all(|t| !t.contains('\r'))`) and its `#[ignore]` removed; the proptest `prop_492_arbitrary_string_holds_core_invariants` argument is flipped from `strict_cr=false` to `strict_cr=true` with an updated comment. No `\r` character survives into any text node from any block type on the generic parser path.

**Trace**: `src/adf.rs::AdfBuilder::end` (NodeKind::HtmlBlock arm — fix site, issue #492); `src/adf.rs::AdfBuilder::push_text` (generic-path CR normalization at chokepoint — fix site, issue #522; normalizes `\r\n`→`\n` then lone `\r`→`\n` before text-node construction; covers heading, paragraph, listItem, taskItem, tableCell, blockquote, panel, inline marks, footnote definitions, inline HTML — all block types on the generic parser path); `src/adf.rs::AdfBuilder::push_code` (inline code span CR normalization at chokepoint — fix site, issue #522; same normalization for `Event::Code` path); `src/adf.rs::trim_leading_trailing_hardbreaks` (step 5b — reused from taskItem path); `src/adf.rs::autolink_bare_urls`; `src/adf.rs::find_bare_url_spans` (boundary-char check — `char::is_whitespace()` OR one of `* _ ~ (`; the predicate is open, not a fixed char set: Unicode whitespace chars beyond space/tab also qualify); `src/adf.rs::AdfRenderer::render_node` (`"hardBreak"` arm — emits `\n`); `src/adf.rs::AdfRenderer::finish` (document-global `self.output.trim_end()`; load-bearing for EC-10 round-trip byte-identity contract); `src/adf.rs::is_empty_block_container` (NOT involved for paragraph — paragraph excluded from REQUIRES_CONTENT set); `src/adf.rs::tests` (`test_convert_block_html_is_preserved_as_literal_text` — pre-existing single-line; `test_convert_multiline_block_html_preserves_interior_newlines` — REPLACED by issue #492 to assert hardBreak-segmented nodes, not raw `\n`-in-text; `test_multiline_block_html_round_trips_through_adf_to_text` — NEW multi-line round-trip (EC-5 extended); `test_block_html_comment_only_behavior` — NEW comment-only block (DOCUMENT-AS-IS pin, EC-3); `test_block_html_bare_url_gets_link_mark` — NEW #473 autolink interaction, asserts both boundary-form link AND href-form non-link (EC-4); `test_block_html_crlf_interior_no_dangling_cr` — NEW CRLF normalization (EC-1); `test_block_html_consecutive_blank_lines_produce_double_hardbreak` — NEW lossless interior `\n\n` (EC-6); `test_block_html_leading_blank_line_no_leading_hardbreak` — NEW step-5b leading-hardBreak elimination (EC-8); `test_block_html_lone_cr_interior_produces_single_hardbreak` — NEW lone-`\r` old-Mac normalization (EC-9); `test_block_html_trailing_whitespace_final_line_not_byte_identical` — NEW trailing-whitespace final-line round-trip lossiness (EC-10); `test_block_html_all_empty_block_emits_no_paragraph` — NEW all-empty block early-return (EC-7); `test_block_html_round_trips_through_adf_to_text` — single-line round-trip (EC-5); `test_block_html_interior_line_url_split_preserves_hardbreaks` — interior-line URL autolink split preserving flanking hardBreaks (EC-4); `test_push_text_normalizes_lone_cr_in_heading_and_code_block` — NEW generic-path lone-CR in heading + codeBlock (EC-11, issue #522); `test_push_text_normalizes_crlf_in_paragraph` — NEW generic-path CRLF in paragraph (EC-11, issue #522); `test_push_code_normalizes_lone_cr_in_inline_code` — NEW push_code lone-CR in inline code span (EC-11, issue #522)); `docs/specs/adf-block-html.md` (created by issue #492); issue #489 (original block-HTML preservation); issue #492 (interior-newline hardBreak fix); issue #522 (generic-path push_text / push_code CR normalization — closes the out-of-#492-scope lone-CR gap on the generic parser path)
