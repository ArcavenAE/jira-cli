...
#### BC-7.2.011: `markdown_to_adf` preserves block-level HTML as literal text in a `paragraph` with interior newlines represented as `hardBreak` nodes — never as raw `\n` inside a text-node string; `adf_to_text` round-trips losslessly at the line-structure level for non-URL-bearing content (LF-only inputs byte-identical **except a final line that is or ends in whitespace, which `adf_to_text`'s document-final `trim_end()` strips on the reverse path**; CRLF/lone-`\r` normalize to `\n`)
...
**Reverse path:** `adf_to_text` renders `hardBreak` nodes as `\n` regardless of context. A round-trip through `adf_to_text` therefore reconstructs the LINE STRUCTURE of the original source with `\n` separators between lines. This is line-structure-lossless for non-URL-bearing content: multi-line structure and `\n\n` interior gaps round-trip correctly. HOWEVER, the round-trip is NOT byte-identical in three cases: (1) CR-bearing inputs — `\r\n` (CRLF) and lone `\r` normalize to `\n` in step 3 (EC-1), so a CRLF input round-trips back as LF-only output; (2) URL-bearing lines — a bare URL at a valid autolink boundary is split and rendered as `[text](url)` not the bare URL literal (EC-4); (3) a final line that is or ends in whitespace — `AdfRenderer::finish()` in `src/adf.rs` (~L2188-2189) performs a document-global `self.output.trim_end()` after rendering all nodes, which strips any trailing whitespace characters (including those preserved by step 2 into the last text node) AND the `\n` emitted by their preceding `hardBreak`. This `trim_end()` is document-global: it only affects the very end of the rendered output, so interior trailing-whitespace content (where the block-HTML paragraph is not the document-final node) is NOT affected. The asymmetry is deliberate: step 2 deliberately preserves trailing non-newline whitespace into the last text node (the forward ADF is correct for non-document-final nodes); `finish()`'s `trim_end()` then strips it on the reverse path for the document-final node. Counterexample (EC-10): handler input `<div>x</div>\n   ` (LF-only, final line is spaces) → forward ADF `[text("<div>x</div>"), hardBreak, text("   ")]` (step 2 preserved `"   "`) → round-trip via `adf_to_text` renders as `"<div>x</div>\n   \n"` → `finish().trim_end()` → `"<div>x</div>"`. The trailing-whitespace line and its hardBreak are destroyed — NOT byte-identical. For byte-identical round-trips, LF-only inputs whose final line ends in a non-whitespace character are required. The `hardBreak`-separator approach is HIGH-confidence per the ADF schema (`hardBreak` is explicitly schema-sanctioned for intra-paragraph line breaks). The exact rendering fate of the OLD raw-`\n`-in-text output from pre-#492 code (whether Jira returned HTTP 400, silently stripped `\n`, or collapsed the break) is live-sandbox-deferred and does NOT gate this fix — the fix is mandated by the ADF schema regardless.
