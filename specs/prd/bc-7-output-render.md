...
2. All trailing `\r` and `\n` characters are trimmed from the concatenated result. This handles both Unix (`\n`) and Windows (`\r\n`) line endings and any number of trailing blank lines. INTERIOR newlines are NOT affected at this step.
3. The trimmed string is split on all line-ending variants (`\r\n`, `\r`, `\n`) into a sequence of segments. This normalizes all interior line endings; no `\r` character survives into any segment.
...
7. After `finish()`, `autolink_bare_urls` processes each `text` node in the paragraph independently; bare `http(s)://` URLs within block HTML receive `link` marks on the containing text node exactly as they would in any other paragraph (see EC-4).
