...
- **(EC-8) Empty task item (`- [ ]` with no text)**: **After EC-16 inline-flattening is applied** (which runs first — see EC-16 ordering note), `"taskItem"` is in `is_empty_block_container`'s prune set; an empty `taskItem` is pruned before output. pulldown-cmark emits `TaskListMarker(false)` followed by no `Text` events → the `taskItem` has empty content and is pruned. A task item containing only whitespace text (e.g., `- [ ]   `) is treated identically — the whitespace-only text node is trimmed and if the resulting content array is empty, the `taskItem` is pruned. A task item containing only a `hardBreak` node and no other content is likewise pruned. **Note: the hardBreak-only prune is a DELIBERATE PRODUCT CHOICE, not schema-forced — a lone `hardBreak` is schema-valid ADF inline content. It is pruned because it carries no semantic content as a task item body; this choice should not be "corrected" by a future implementer.** The prune criterion covering all three cases (evaluated after EC-16 flattening): `taskItem.content` is empty OR contains only structurally-empty inline content (whitespace-only text nodes and/or bare hardBreaks with no substantive text).
