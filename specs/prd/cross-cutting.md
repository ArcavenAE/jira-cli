...
**Behavior**: The `test_claude_md_citations_resolve_to_real_files` test in `tests/claude_md_citations.rs` reads `CLAUDE.md` via `include_str!("../CLAUDE.md")`, extracts every backtick-quoted token that (a) starts with a develop-tracked directory prefix (`src/`, `tests/`, `docs/`, `.github/`, `scripts/`) AND (b) has a recognized file extension (`.md`, `.rs`, `.sh`, `.toml`, `.yml`, `.yaml`), normalizes tokens per BC-X.13.002 (glob skip, symbol-form strip, line-ref strip, trailing-punct trim), then asserts `Path::new(root).join(&citation).exists()` for each remaining path. ALL `.factory/` prefixes are EXCLUDED — `.factory/` is git-ignored and lives in a separate orphan-branch worktree that is ABSENT from the CI checkout; dead-citation coverage for `.factory/` paths is handled by the maintenance doc-drift sweep, NOT this guard. On failure, the assertion message lists EVERY dead path (not just the first) with the canonical message format below. The test passes green on the current `develop` HEAD (zero dead citations) and fails deterministically when any newly-cited path does not exist.
...
- Exit 0; test passes
- Every backtick token matching the in-scope grammar (`src/...`, `tests/...`, `docs/...`, `.github/...`, `scripts/...`) resolves to a real file
...
- The message includes the "Fix the citation or restore the file." instruction plus the auto-exclusion note
...
- VP-CITE-001: `extract_path_citations` grammar — unit + proptest coverage of in-scope detection and all normalization/exclusion rules; no false positives on documented edge cases. See `verification-delta-DEAD-CITATION-CI.md` §VP-CITE-001.
...
e. **Punctuation trim (leading and trailing)**: applied symmetrically and **greedily** (repeated until no trimable character remains) after the above strips.
   - *Leading*: strip a single leading `(` or `[` from the token. This allows `(src/adf.rs)` → after leading-`(` removal → `src/adf.rs)` → trailing-`)` check applies next.
   - *Trailing — plain set (greedy)*: trim trailing `.`, `,`, `;`, `:` characters **repeatedly** until none remain. This handles compound cases such as `(src/adf.rs).` (paren-plus-period) and `src/config.rs,;` — all plain punct comes off before the bracket/paren balance check runs.
   - *Trailing — unbalanced `)` (after plain-set removal)*: trim a trailing `)` ONLY if it is **unbalanced** across the whole token — defined as `count('(') < count(')')` after any leading-`(` strip (faithful mirror of `src/adf.rs::trim_url_extent` whole-token parenthesis balance). This makes `path1,` in comma-delimited `Detail: path1, path2` correctly yield `path1` (H-1/H-2); `(src/adf.rs)` → leading `(` stripped → `src/adf.rs)` → one `)`, zero `(` → unbalanced → trailing `)` stripped → `src/adf.rs` → checked.
   - *Trailing — unbalanced `]` (after plain-set and `)` removal)*: trim a trailing `]` ONLY if it is **unbalanced** — defined as `count('[') < count(']')` after any leading-`[` strip, mirroring the `)` rule. This closes the **`[`/`]` asymmetry (LOW-1)**: `[docs/x.md]` → leading `[` stripped → `docs/x.md]` → one `]`, zero `[` → unbalanced → trailing `]` stripped → `docs/x.md` → passes dir-prefix + extension filter → CHECKED. Without this rule, the trailing `]` survives and the extension filter rejects the token (`.md]` is not `.md`), silently excluding a valid citation — a latent false-negative.
   - *Double-wrap (greedy outer-pair stripping)*: `((src/x.rs))` — first pass: leading `(` stripped → `(src/x.rs))`, trailing plain-punct loop no-ops (no `.`, `,`, `;`, `:`), then trailing `)` check: two `)`, zero `(` → unbalanced → trailing `)` stripped → `(src/x.rs)`; second pass of the same rule applied to the now-outer token: leading `(` stripped → `src/x.rs)`, trailing `)` unbalanced → stripped → `src/x.rs` → checked. The rule is applied as many outer-pair iterations as needed until the token no longer starts with `(` or `[`. **One canonical rule, deterministic**: remove the outermost `(…)` or `[…]` if the brackets are unbalanced after leading-strip, repeating until stable.
...
- EC-CITE-024 (mixed trailing punct, LOW-2): Token is `(src/adf.rs).` — leading `(` stripped → `src/adf.rs).`; plain-punct greedy loop strips trailing `.` → `src/adf.rs)`; trailing `)` check: `count('(')` = 0, `count(')')` = 1 → unbalanced → stripped → `src/adf.rs` → checked. Demonstrates that plain-punct stripping runs BEFORE the bracket balance check and multiple punct chars all come off.
- EC-CITE-025 (double-wrap, LOW-3): Token is `((src/x.rs))` — first iteration: leading `(` stripped → `(src/x.rs))`; trailing `)` check: two `)`, zero `(` → unbalanced → stripped → `(src/x.rs)`; second iteration: leading `(` stripped → `src/x.rs)`; trailing `)` check: one `)`, zero `(` → unbalanced → stripped → `src/x.rs` → checked. The greedy outer-pair rule handles arbitrarily nested wraps deterministically.
...
**Behavior**: The guard's dir-prefix filter (BC-X.13.002 step f) recognizes ONLY `src/`, `tests/`, `docs/`, `.github/`, and `scripts/` as develop-tracked directories. `.factory/` is NOT in this list. As a result, ALL `.factory/` citations — regardless of sub-path (`specs/`, `research/`, `holdout-scenarios/`, `cycles/`, or any other) — are excluded by the dir-prefix filter and never reach the `Path::exists()` check.
...
**Postconditions**:
- ANY CLAUDE.md citation starting with `.factory/` (e.g., `.factory/specs/prd/bc-3-issue-write.md`, `.factory/research/S-3.03-wave3-verification.md`, `.factory/holdout-scenarios/H-001.md`) does NOT cause the guard to fail — it is excluded by dir-prefix filter before any existence check
- The guard only fails for dead citations in develop-tracked directories (`src/`, `tests/`, `docs/`, `.github/`, `scripts/`)
...
**Canonical Test Vectors** (for `extract_path_citations` dir-prefix filter unit tests):

| Path | Excluded by dir-prefix? | Rationale |
|------|------------------------|-----------|
| `.factory/specs/prd/bc-3-issue-write.md` | YES | `.factory/` not in develop-tracked prefix set |
| `.factory/holdout-scenarios/H-001.md` | YES | `.factory/` not in develop-tracked prefix set |
| `.factory/cycles/cycle-01.md` | YES | `.factory/` not in develop-tracked prefix set |
| `.factory/research/S-3.03-wave3-verification.md` | YES | `.factory/` not in develop-tracked prefix set |
