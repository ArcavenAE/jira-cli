...
| BC-X.13.002 | Canonical normalization pipeline (a)–(e): (a) glob/brace-glob skip (`*`,`{`,`}`); (b) unified normalize fixpoint — sub-steps repeat until stable: (1) symbol-form strip (`::…`); (2) line-ref strip (`:~NN`/`:NN`); (3) strip one leading `(`/`[`; (4) greedy plain-punct strip (`.`,`,`,`;`,`:`); (5) unbalanced `)` trim; (6) unbalanced `]` trim; (c) dir-prefix filter (`src/`,`tests/`,`docs/`,`.github/`,`scripts/`; ALL `.factory/` excluded); (d) extension filter; (e) Path::exists() check — merged fixpoint eliminates ordering-class false-negatives on paren-wrap+line-ref tokens (F-PASS6-01) | — (DEAD-CITATION-CI F2) | tests/claude_md_citations.rs::extract_path_citations (new, with inline unit tests) | HIGH |
