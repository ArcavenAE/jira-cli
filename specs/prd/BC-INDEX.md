...
sections:
  ...
  - bc-7-output-render.md (91 BCs cumulative; 45 individually-bodied)
...
## Section 7: Output Rendering (bc-7-output-render.md) — 91 BCs cumulative; 45 individually-bodied
...
| BC-7.2.012 | `markdown_to_adf` and `adf_to_text` enforce `MAX_ADF_DEPTH = 256` (inclusive rejection boundary); inputs exceeding the limit return exit 64 with message "input nesting too deep (max 256)" rather than causing stack overflow (CWE-674); depth check applied before recursing into children; both forward path (markdown input) and reverse path (Jira ADF responses) enforced; `text_to_adf` is non-recursive and exempt | SEC-001 | src/adf.rs recursion guards; tests/adf_recursion_depth.rs; src/adf.rs::tests | HIGH |
| BC-7.2.013..057 | Additional ADF contracts (range-collapsed from bc-7 body) [range-collapsed; not individually-bodied] | BC-1106..1117 | src/adf.rs::tests | HIGH |
