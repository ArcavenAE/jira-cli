...
last_updated: 2026-07-23  # FIX-F5-006 F5-R1-007 fix round: BC-3.9.006 row updated — step-2 network branch split from 5xx; transport/network → NetworkError, no retry hint; spec v1.3.100; BC count unchanged (657); BC-INDEX v6.38. Previous: PRE-F4-UNICODE-DISPLAY-SANITIZATION: BC-2.7.008/BC-2.7.010/BC-2.7.011/BC-3.9.015/BC-3.9.017 rows updated — display-sanitization character set extended (Unicode bidi/separator/NEL); cross-ref rows point to BC-2.7.011 primary clause; spec v1.3.94; BC count unchanged (657); holdout count 100 (unchanged); VP count 35 (unchanged); BC-INDEX v6.37
index_version: v6.38
source_pass: 3
sections:
...
| BC-3.9.006 | temporaryAttachmentId ~1h TTL; step-2 HTTP errors → retry hint "Temporary attachment IDs may have expired. Try the upload again." (4xx excl. 401/403 → exit 64; 401 → exit 2; 403 → exit 1; 5xx → exit 1 `JrError::ApiError`); step-2 transport/network error → `JrError::NetworkError`, exit 1, `"Could not reach <host> — check your connection"`, **no retry hint** (parity with step-1 transport mapping); no Cloud error-string pattern-matching (P2-2); no ID caching or reuse; FIX-F5-006 (F5-R1-007) | — (SOH-ATTACHMENTS-1 F2; FIX-F5-006) | src/api/jsm/attachments.rs (pending S5) | MEDIUM-HIGH |
