...
last_updated: 2026-07-25  # SOH-DX-1 DEC-188: BC-3.8.012/013 superseded (warn+proceed → pre-flight exit-64); spec v1.3.107; BC count unchanged (657); BC-INDEX v6.45. Previous: WAVE-576-05 F5-R12-001: BC-X.8.010 EC-X.8.010-2 + stale_healed per-command-not-per-file DOCUMENT-AS-IS note; 0 new BCs; spec v1.3.106; BC count unchanged (657); BC-INDEX v6.44.
index_version: v6.45
source_pass: 3
sections:
...
| L3 BC ID | Summary | Pass 3 BC ID | Source | Confidence |
|---|---|---|---|---|
| BC-3.8.001 | `issue create --request-type <NAME\|ID>` dispatches to `POST /rest/servicedeskapi/request`; platform POST body / JSON response / exit code unchanged when `--request-type` absent (unless `--field`/`--on-behalf-of` present → exits 64 per BC-3.8.012/013 DEC-188) | — (issue #288 F2) | tests/issue_create_jsm.rs; src/cli/issue/create.rs | HIGH |
