...
| BC-7.2.011 | `markdown_to_adf` preserves block-level HTML as literal text in a `paragraph`; interior newlines → `hardBreak` nodes (never raw `\n` in text-node string); trailing `\r\n` trimmed; `autolink_bare_urls` applies to resulting text nodes; `adf_to_text` round-trips losslessly for non-URL-bearing content (LF-only inputs byte-identical **except a final line that is or ends in whitespace, which `adf_to_text`'s document-final `trim_end()` strips on the reverse path**; CRLF/lone-`\r` normalize to `\n`); supersedes #489 raw-`\n`-in-text behavior (defect, issue #492) | issue #492 | src/adf.rs::AdfBuilder::end (HtmlBlock arm); src/adf.rs::autolink_bare_urls; src/adf.rs::AdfRenderer::finish (~L2188-2189); src/adf.rs::tests | HIGH |
