...
## Section 1: Auth & Identity (bc-1-auth-identity.md) — 69 BCs cumulative; 58 individually-bodied [BC-1.1.013..015 + BC-1.2.048..051 + BC-1.4.031..033 + BC-1.6.047 added 2026-09-01 cycle-003 `auth-profile-dx` F2 spec evolution, DEC-312..325, ADR-0020/ADR-0011 amended; BC-1.1.009/010, BC-1.2.013/014/017, BC-1.4.025/027/029, BC-1.6.046 amended in place]

### 1.1 OAuth Flow & Profile Resolution (15 BCs: BC-1.1.001..015) [BC-1.1.013..015 added 2026-09-01 cycle-003 `auth-profile-dx` F2 — OAuth-default-at-creation + non-interactive regression pins + runtime-default-unchanged pin, DEC-313]
...
### 1.2 Profile Lifecycle Management (11 BCs: BC-1.2.013..018, BC-1.2.047..051) [BC-1.2.048..051 added 2026-09-01 cycle-003 `auth-profile-dx` F2 — no-per-command-auth-switch invariant, `--oauth` deprecation, new `--api-token` flag, `auth refresh` override removal, DEC-313/321/323]
...
| BC-1.2.014 | `auth remove <name>` performs four-step delete: config entry, cache directory, OAuth tokens, API-token credential pair [AMENDED 2026-09-01 cycle-003 — 4th step added for the new per-profile API-token pair, DEC-315/DEC-322] | BC-014-R | src/cli/auth.rs; src/cache.rs:~82-88 | HIGH |
...
### 1.4 Token Keychain Layout (9 BCs: BC-1.4.025..033) [BC-1.4.031..033 added 2026-09-01 cycle-003 `auth-profile-dx` F2 — per-profile API-token keychain functions, one-time lazy migration, partial-state handling, DEC-315]
...
| BC-1.4.032 | One-time lazy migration of shared flat `email`/`api-token` → `<default>:email`/`<default>:api-token`, mirroring `load_oauth_tokens`'s pattern [NEW 2026-09-01 cycle-003, DEC-315] | — | ADR-0020 §Decision 2; src/api/auth.rs::load_oauth_tokens (pattern mirrored) | HIGH |
| BC-1.4.033 | Partial-state handling for the per-profile API-token pair (one of `email`/`api-token` present, the other absent) → `Err`, actionable message [NEW 2026-09-01 cycle-003, DEC-315] | — | ADR-0020 §Decision 2 item 4; src/api/auth.rs::load_oauth_tokens partial-state pattern (mirrored) | HIGH |
