...
| L3 BC ID | Summary | Pass 3 BC ID | Source | Confidence |
|---|---|---|---|---|
| BC-X.13.001 | Every in-scope backtick-quoted path citation in CLAUDE.md (develop-tracked dirs: `src/`, `tests/`, `docs/`, `.github/`, `scripts/`; recognized extension) resolves to a real on-disk file; ALL `.factory/` paths excluded (absent from CI checkout); guard fails with canonical message listing ALL dead references | — (DEAD-CITATION-CI F2) | tests/claude_md_citations.rs::test_claude_md_citations_resolve_to_real_files (new) | HIGH |
| BC-X.13.002 | Canonical normalization pipeline (in order): (a) glob/brace-glob skip (`*`,`{`,`}`); (b) symbol-form strip (`::fn`); (c) line-ref strip (`:~NN`/`:NN`); (d) section-ref excluded by whitespace tokenization; (e) single-fixpoint punctuation trim — sub-steps: (1) strip one leading `(`/`[`; (2) greedy plain-punct strip (`.`,`,`,`;`,`:`); (3) unbalanced `)` trim; (4) unbalanced `]` trim — repeat until stable; (f) dir-prefix filter; (g) extension filter — no false positives on any of these forms | — (DEAD-CITATION-CI F2) | tests/claude_md_citations.rs::extract_path_citations (new, with inline unit tests) | HIGH |
