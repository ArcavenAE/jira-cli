...
sections:
  ...
  - cross-cutting.md (151 BCs cumulative; 85 individually-bodied)
...
## Section 1: Auth & Identity (bc-1-auth-identity.md) — 58 BCs cumulative; 47 individually-bodied
...
| BC-1.1.009 | `auth login --profile <new>` creates profile even when profile doesn't yet exist | BC-009 | tests/auth_profiles.rs:~241-280 | HIGH |
| BC-1.1.010 | `auth login --profile X` succeeds even when JR_PROFILE points to absent profile | BC-010 | tests/auth_profiles.rs:~290-332 | HIGH |
...
### 1.2 Profile Lifecycle Management (7 BCs: BC-1.2.013..018, BC-1.2.047)

| L3 BC ID | Summary | Pass 3 BC ID | Source code | Confidence |
|---|---|---|---|---|
| BC-1.2.013 | `auth logout` deletes only `<profile>:oauth-access-token` and `<profile>:oauth-refresh-token` | BC-013-R | src/api/auth.rs:~24-32, 88-97 | HIGH |
| BC-1.2.014 | `auth remove <name>` performs three-step delete: config entry, OAuth tokens, cache directory | BC-014-R | src/cli/auth.rs; src/cache.rs:~82-88 | HIGH |
...
| BC-1.2.017 | `auth login --profile X` against `JR_PROFILE=ghost` succeeds creating profile X | BC-029 (R1) | tests/auth_profiles.rs:~282-333 | HIGH |
...
| L3 BC ID | Summary | Pass 3 BC ID | Source code | Confidence |
|---|---|---|---|---|
| BC-1.4.025 | `default` profile lazy-migrates legacy flat OAuth keys; non-default profiles never inherit | BC-023-R | src/api/auth.rs:~111-169 | HIGH |
...
| BC-1.4.027 | Per-profile keychain keys: `<profile>:oauth-access-token` / `<profile>:oauth-refresh-token` | BC-1153 (R4) | src/api/auth.rs:~24-32 | HIGH |
| BC-1.4.028 | `load_oauth_tokens` errors on PARTIAL state (one token present, other missing) | BC-1156 (R4) | src/api/auth.rs:~1249-1269 | HIGH |
| BC-1.4.029 | `load_oauth_tokens("sandbox")` does NOT inherit legacy flat keys | BC-1158 (R4) | src/api/auth.rs:~1323-1341 | HIGH |
...
| BC-1.6.046 | `auth list` table snapshot: 4 columns, active profile with `* ` prefix | BC-1115 (R4) | src/cli/snapshots/jr__cli__auth__tests__list_table_snapshot.snap | HIGH |

---

## Section 2: Issue Read (bc-2-issue-read.md) — 114 BCs cumulative; 72 individually-bodied [BC-2.7.001..012 added 2026-07-15 SOH-ATTACHMENTS-1 F2 DEC-179; BC-2.2.032 + BC-2.3.039 added 2026-08-13 issue #668 F2 duedate feature; BC-2.1.018..022 + BC-2.3.040 added 2026-08-15 issue #606 F2 component-management bundle]

### 2.1 JQL Composition (22 BCs: BC-2.1.001..022) [BC-2.1.018..022 added 2026-08-15 issue #606 F2; BC-2.1.006/007 amended in place, no separate count]
...
| BC-2.1.006 | No project AND no filters AND no `--jql` → exit 64 listing all 14 filter sources (13→14, `--component` added 2026-08-15 issue #606) | BC-129 (R1) | src/cli/issue/list.rs:~344-351 | HIGH |
| BC-2.1.007 | `build_filter_clauses` emits in stable order: assignee, reporter, status, open, team, recent, asset, component, date filters (`component` inserted after `asset` 2026-08-15 issue #606) | BC-130 (R1); BC-1093 (R4) | src/cli/issue/list.rs:~613-649 | HIGH |
...
### 2.2 Issue List Behavior (15 BCs: BC-2.2.018..032)
...
### 2.3 Issue View (9 BCs: BC-2.3.032..040) [BC-2.3.040 added 2026-08-15 issue #606/#605/#604/#608 F2 — shared Component.id prerequisite]
...
## Section 3: Issue Write (bc-3-issue-write.md) — 144 BCs cumulative; 115 individually-bodied [BC-3.9.001..014 added 2026-07-15 SOH-ATTACHMENTS-1 F2 DEC-179; BC-3.9.015..020 added 2026-07-15 adversary pass-1 round B; BC-3.4.022..025 added 2026-08-15 issues #604/#605/#608 F2 component-management bundle]
...
### 3.8 JSM Request Create + Platform-Path Pre-flight Guards + Auth-Conditional 401 Hints (17 BCs: BC-3.8.001..017) [Added 2026-05-18 issue #288; BC-3.8.010 added F1d pass-01; BC-3.8.011 added F1d pass-01; BC-3.8.012..013 added 2026-05-19 issue #383 F2; BC-3.8.014..015 added 2026-05-19 issue #384 F2; BC-3.8.016..017 added 2026-05-20 issue #385 F2; BC-3.8.002/010/011 modified 2026-05-20 issue #385 F2; BC-3.8.012..013 amended 2026-07-25 DEC-188 (warn→exit-64 pre-flight)]

| L3 BC ID | Summary | Pass 3 BC ID | Source | Confidence |
|---|---|---|---|---|
| BC-3.8.001 | `issue create --request-type <NAME\|ID>` dispatches to `POST /rest/servicedeskapi/request`; platform POST body, JSON response, and exit code unchanged when `--request-type` absent (unless `--field`/`--on-behalf-of` present — exits 64 per BC-3.8.012/013) | — (issue #288 F2) | tests/issue_create_jsm.rs; src/cli/issue/create.rs | HIGH |
...
| BC-3.8.012 | `--field` on platform path without `--request-type` exits 64 pre-flight — **[AMENDED DEC-188 2026-07-25]** ~~emits one stderr warning; platform POST proceeds~~ → pre-flight `JrError::UserError` exit 64 BEFORE any HTTP when `--field` present without `--request-type`; ONE error regardless of `--field` count; combined error when both `--field` + `--on-behalf-of` present | — (issue #383 F2; amended SOH-DX-1 DEC-188) | tests/issue_create_jsm.rs (platform-path inverse-flag tests inverted) | HIGH |
| BC-3.8.013 | `--on-behalf-of` on platform path without `--request-type` exits 64 pre-flight — **[AMENDED DEC-188 2026-07-25]** ~~emits one stderr warning; platform POST proceeds~~ → pre-flight `JrError::UserError` exit 64 BEFORE any HTTP when `--on-behalf-of` present without `--request-type`; combined error governed by BC-3.8.012 when both flags present | — (issue #383 F2; amended SOH-DX-1 DEC-188) | tests/issue_create_jsm.rs | HIGH |
...
| BC-8.4.005 | Client-side resolver case-insensitivity agrees with JQL's case-insensitive component-name matching (`ExactMultiple` path, no false Ambiguous) | — (BC-X.10.003, precedent BC-2.1.015, issues #604/#605/#606/#608) | src/cli/issue/helpers.rs::resolve_component (pending F4) | MEDIUM |

---

## Section X: Cross-Cutting Utilities (cross-cutting.md) — 151 BCs cumulative; 85 individually-bodied [BC-X.8.010 added 2026-07-15 SOH-ATTACHMENTS-1 F2 DEC-179; BC-X.13.007 added 2026-08-05 FIX ROUND 12 S-626-1 issue #626; BC-X.10.001 amended in place 2026-08-15 issue #604/#605/#606/#608 F2 — resolve_component caller citation, no count change]
