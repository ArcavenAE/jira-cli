...
| BC-3.9.015 | `attachment delete <AID>` interactive confirmation gate: `eprint!+read_line` (DEC-174, mirrors BC-3.5.003); non-interactive exit 64 + --yes hint; --yes bypasses; cancel `{"cancelled":true,"deleted":false}` (no id key); metadata-fetch GET before prompt; EOF → exit 130 | — (SOH-ATTACHMENTS-1 adversary pass-1 R2) | src/cli/issue/attachments.rs (pending S4) | HIGH |
...
| BC-3.9.019 | `attachment delete --older-than <duration>`: --issue KEY required; duration via `src/duration.rs` (same family as worklog); `created` ISO 8601 compared client-side via `chrono`; invalid duration exit 64; bulk-delete JSON `{"deleted":true,"count":N,"ids":[...]}` via `output::render_json` | — (SOH-ATTACHMENTS-1 adversary pass-1 R1) | src/cli/issue/attachments.rs (pending S4) | HIGH |
| BC-3.9.020 | `attachment delete --dry-run`: multi-attachment paths list affected IDs without mutation; JSON `{"dryRun":true,"ids":[...],"attachments":[{id,filename}]}` via `output::render_json`; single-ID --dry-run = stderr hint + exit 0 (no-op, no gate); --yes with --dry-run = DEC-169 silent no-op | — (SOH-ATTACHMENTS-1 adversary pass-1 R1) | src/cli/issue/attachments.rs (pending S4) | HIGH |
