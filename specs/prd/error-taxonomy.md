...
| **Message format** | Lead line: `CLAUDE.md cites file paths that do not exist on disk:` — then one `  <path> (line N)` per dead reference — then `Fix the citation or restore the file.` — then `Note: .factory/, glob, and symbol-form tokens are auto-excluded.` |
| **Actionability** | Each dead citation is listed on its own line, prefixed with two spaces, followed by ` (line N)` for actionability. The developer can: (a) restore the deleted/renamed file, (b) update the CLAUDE.md citation to the new path. There is NO allowlist — `.factory/` paths are always excluded by the dir-prefix filter; `.factory/` citations never trigger this error. |
| **False-positive risk** | LOW when the guard is correctly implemented per BC-X.13.002 (glob/brace-glob skip, suffix strip, trailing-punct trim, dir-prefix filter) and BC-X.13.003 (all `.factory/` excluded). A false positive means a token was incorrectly classified as in-scope; the fix is a suffix-strip update in `extract_path_citations`. |
