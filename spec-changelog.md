...
delivered SERIALLY (D-381), A → C → B (STORY-A #862, then STORY-C #583, then STORY-B #861, each
rebased on the previous) — see `cycle-manifest.md`'s dated amendment note. The entry is MINOR
...
| VP-API-QP-001 | BC-X.16.001: the separator a merge adds is `?` (no `?` in the pre-`#` part, even if it ends in `&`), none (query component empty or `&`-terminated), or `&` (otherwise, including when the component itself ends in a literal `?`). Prefix and `#fragment` are preserved. | proptest (separator oracle) + pinned unit examples |
...
| `README.md` | UPDATED (F4) | `jr user list --project FOO` row shows `--project` as optional with its configured-default fallback (STORY-A); `jr api <PATH>` row gains `-q`/`--query-param NAME=VALUE` (STORY-C); `jr field options <NAME>` row (~L346) rewords "Enumerate a custom field's allowed options" to "Enumerate a field's allowed options" (system-typed fields are covered too, not just custom fields) (STORY-B) |
...
| `docs/specs/cargo-mutants-policy.md` | UPDATED (F4, split per story) | §Scope gains one bullet per story, each added in the PR that introduces its functions, per this doc's own convention: STORY-A adds `` `src/cli/user.rs` — `resolve_user_list_project` ``; STORY-C adds `` `src/cli/api.rs` — `append_query_params`, `parse_query_param` ``. Delivery is serial (A → C → B, D-381); per-story F4 timing adopted by D-382. STORY-A bumps the count line 32 → 33 and STORY-C 33 → 34, each adding its own newest-first row to the policy's `## Changelog` table |
| `.cargo/mutants.toml` | UPDATED (F4, split per story) | `examine_globs` gains `src/cli/user.rs` (STORY-A) and `src/cli/api.rs` (STORY-C), each added in the PR that introduces its functions (additions approved D-378/D-379; per-story F4 timing adopted by D-382) |
| `.factory/specs/architecture/*` | UNCHANGED | F1 found no structural change, so these files are not touched this delta |

- **Affected stories:** S-580-1 (delivered, pointer note only — anchors BC-X.14.001..004; not
  rewritten). F3 (incremental story decomposition) is the next phase for this cycle; no story
  files exist yet for STORY-A/B/C.
- **Affected tests:** F4 adds tests for the new VPs in `src/cli/api.rs`, `src/cli/user.rs` and
  ...
  `JR_AUTH_HEADER`), per `.factory/cycles/cycle-014/phase-f2-spec-evolution/verification-delta.md` §2 (`Config::load_inner`'s two `JR_`-reading
  ...
  `src/cli/user.rs` (STORY-A) and `src/cli/api.rs` (STORY-C), approved D-378/D-379, per-story F4
  timing D-382. As part of STORY-A, `UserCommand::List.project`'s
...
- **Migration needed:** Partial — three `jr user list` behavior changes are user-visible and
  ...
  before global-value propagation ever ran, and now resolves `FOO` and succeeds. #861 is a
  non-breaking, user-visible output fix: `jr field options` now emits a real `label` string
  for system-field options instead of `null`/`(unnamed)`, and the `--value` filter now also
  matches those option names. It belongs in the product CHANGELOG as a fix. #583's
  `--query-param` flag is additive.
...
human **TIGHTENING decision** (D-345; mechanism finalized by the architect as Option (a),
...
previously-shipped behavior. No governance-flagged reversal (unlike the [2.0.0]/D-310 entry
...
**reverses BC-3.8.012's prior behavior**: D-188 (S-639-1) had made `--field`/`--on-behalf-of`
without `--request-type` exit 64 pre-flight; that guard is now removed for `--field` (governance
decision **D-310**, registered 2026-08-26 (human-approved at the F2 gate) — see Impact
...
behavioral contract (D-188 → D-310; `--field` alone on `issue create` used to exit 64
...
resolution and D-310's registration both remain as recorded elsewhere in this entry). This
remains a **distinct axis from CLI/product semver** — CLAUDE.md's own history separately labels
D-188's *original introduction* "breaking change" in product/CLI terms (S-639-1,
...
"MAJOR version bump determination" block above for the full rationale: the D-188 → D-310
...
to `v2.0.0 (MAJOR)` throughout, **APPLIED 2026-08-26** under this human delegation. D-310 itself
is **REGISTERED** (see below and the Impact Assessment table); both the spec-version classification
and D-310's registration are now settled, not open.
...
**been APPROVED (2026-08-26)**: D-310 is now **REGISTERED** and the F-3 JSM collision-guard
...
- `bc-3-issue-write.md` (NEW): BC-3.3.010 — `issue create --field` extended to the non-JSM
  ...
  `--field` hint exits 64 pre-POST). **BC-3.8.012 — REVERSED in place**: D-188's `--field`-alone
  and combined platform-path exit-64 pre-flight guard removed; `[D-188 BEHAVIOR, superseded]`
  retained inline for audit trail; governance decision **D-310, REGISTERED 2026-08-26
  (human-approved at the F2 gate)** records the reversal formally (renumbered from the
  initially-proposed D-307, propagation to D-310 completed across all citing files).
...
| Decisions Log (`STATE.md` Decisions of Record) | **REGISTERED** | **D-310** (reverses D-188) — authored by product-owner, **REGISTERED 2026-08-26** (human-approved at the F2 gate); STATE.md Decisions Log flipped by state-manager this burst |
...
| F2 human gate (Step 8) | **APPROVED** | Human approved 2026-08-26, delivering 4 decisions: gate APPROVED (F2→F3); spec version v2.0.0 (MAJOR); F-3 RESOLVED (JSM retain last-wins); D-310 REGISTERED. See `STATE.md`/`cycles/cycle-002/burst-log.md` Burst 8. |
...
- **Migration needed:** Product/CLI-level — yes, for the D-188 reversal specifically (a script
...
`component-mgmt`, D-278/D-279/D-280; issues #604/#605/#606/#608, #607/#609 deferred).
...
- `bc-8-components.md` (NEW FILE): 28 BCs across 4 subdomains — Component Read & CRUD (8.1,
  BC-8.1.001..008), Component Delete Safety (8.2, BC-8.2.001..008, D-279), Component Rename
...
- `bc-3-issue-write.md` (NEW): BC-3.4.022 — single-key `issue edit --component` native
  ...
  integer `componentId` (D-280; live-smoke-test-gated per `FIX-BULK-TRANSITION-001`
...
4 changes are body-only amendments — same rule applied at [1.3.164] (SOH-DX-1/D-188 breaking
...
- `bc-3-issue-write.md` (AMENDED, REVERSAL under D-274): BC-3.4.021 — Invariant 3 REVERSED:
  ...
  and never read stdin (asserted "correct, not a bug" pre-D-274). New additive
  ...
  correcting an inaccurate assumption in the original bug report). Pre-D-274 text retained
...
exit-64 promotion (SOH-DX-1/D-188, issue #639: `--field`/`--on-behalf-of` on `issue create`
...
| Artifact | Change Type | Notes |
|----------|-------------|-------|
| `.factory/specs/prd/bc-3-issue-write.md` | MODIFIED | BC-3.4.021 amended (reversal under D-274); frontmatter trace entry added; `total_bcs`/`definitional_count` UNCHANGED (111 individually-bodied — amend-in-place, no new heading) |
...
- D-274 (supersedes BC-3.4.021 Invariant 3; STATE.md record owned by state-manager)
...
- `.factory/specs/prd/bc-3-issue-write.md` (MODIFIED): version retarget at 4 `[AMENDED 2026-07-25 SOH-DX-1 D-188 #639]` banners (~:539, ~:3049, ~:3137, ~:3149); `_Last updated` footer prepended with v1.3.169 entry.
...
### Summary
SOH-DX-1 F2 round-46 trail-anachronism remediation: 5 LOW findings corrected. F1 (LOW): v1.3.114 frontmatter trail entry — `tests/common/assertions.rs ~:76` restored to `tests/common/fixtures.rs ~:76` in the F16-03 clause (unintended `replace_all` sweep from v1.3.144 F46-003; the trailing "moves to fixtures.rs on promotion" was not swept so the sentence named two different targets). F2 (LOW): v1.3.108 frontmatter trail entry — `tests/common/assertions.rs` restored to `tests/common/fixtures.rs` in the `assert_json_error_envelope` promotion directive (same unintended sweep; BC-INDEX.md trail still correctly preserved `fixtures.rs` for this event). F3 (LOW): v1.3.137 frontmatter trail entry — `tests/common/assertions.rs` restored to `tests/common/fixtures.rs` in the `write_profile_config` fixture contract description (same unintended sweep; BC-INDEX.md trail still correctly preserved `fixtures.rs` for this event). F4 (LOW): spec-changelog.md [1.3.144] F46-003 scope statement corrected from vague "replace_all on all promotion-target path references" to "(9 sites: 5 spec body + 3 historical trail entries + 1 footer)" with note that the 3 trail-entry sweeps were unintended anachronisms corrected in [1.3.145]. F5 (LOW): bc-3-issue-write.md v1.3.144 frontmatter trail entry F46-003 clause corrected from "replace_all on promotion-target path (5 sites)" to "(9 sites: 5 spec body + 3 historical trail entries + 1 footer)" with note that the 3 trail-entry sweeps were unintended anachronisms corrected in [1.3.145] (in-round residual of F4; TWIN-ARTIFACT-SWEEP class — second of two mirroring artifacts now aligned). Governing principle: a version-trail entry is an immutable audit record of what was true at that version; mechanical `replace_all` must never rewrite historical trail entries. BC count unchanged (140/111).

### Changed
- `.factory/specs/prd/bc-3-issue-write.md` (MODIFIED): v1.3.114 trail: `tests/common/assertions.rs ~:76` → `tests/common/fixtures.rs ~:76` in F16-03 first clause (F1). v1.3.108 trail: `tests/common/assertions.rs` → `tests/common/fixtures.rs` in assert_json_error_envelope promotion directive (F2). v1.3.137 trail: `tests/common/assertions.rs` → `tests/common/fixtures.rs` in write_profile_config fixture contract description (F3). v1.3.144 trail: F46-003 clause "(5 sites)" → "(9 sites: 5 spec body + 3 historical trail entries + 1 footer)"; unintended-anachronism note added referencing [1.3.145] (F5). `last_updated` 2026-07-26 → 2026-07-27. v1.3.145 trail entry prepended.
- `.factory/spec-changelog.md` (MODIFIED): [1.3.144] F46-003 scope corrected to "(9 sites: 5 spec body + 3 historical trail entries + 1 footer)" + unintended-anachronism note referencing [1.3.145] added (F4). [1.3.145] entry prepended.
...
### Summary
SOH-DX-1 D-188 round-46 adversary-pass corrections (#639): F46-001 (MED): AC-2 and AC-7 gain the explicit would-otherwise-succeed clause ("invocation is would-otherwise-succeed — `mount_platform_create_stubs` MUST be called so the platform POST can complete if the guard is absent; only a would-otherwise-succeed run makes `stdout.trim().is_empty()` a genuine DISCRIMINATING assertion rather than HYGIENE"), matching the six sibling ACs and closing the drop-the-stub incentive for these two `--output json` mode tests. F46-002 (LOW): both `[CURRENT BEHAVIOR]` Behavior block body-range labels reworded from the "excluded terminal case" phrasing to "BEFORE all pre-POST helper HTTP (steps 3–5) and BEFORE the platform POST (step 6) — see Platform-Path Guard Ordering block above" (replace_all, 2 sites) — kills the "excluded = reachable" misreading. F46-003 (LOW): promotion target for `assert_json_error_envelope` and `write_profile_config` changed from `tests/common/fixtures.rs` to `tests/common/assertions.rs` (new module registered in `tests/common/mod.rs`; keeping `fixtures.rs` pure-JSON); replace_all on promotion-target path (9 sites: 5 spec body + 3 historical trail entries + 1 footer); the 3 trail-entry sweeps were unintended anachronisms corrected in [1.3.145]; convention note "fixtures.rs convention" → "assertions.rs convention" and `pub mod assertions;` in `tests/common/mod.rs` registration note added to both Test Notes (replace_all). BC count unchanged (140/111).
...
- `.factory/spec-changelog.md` (MODIFIED): LOW-2: [1.3.110]–[1.3.114] Summary lines each append "(D-188 ratified 2026-07-25)". [1.3.115] entry added.
...
### Summary
SOH-DX-1 D-188 round-16 adversary-pass corrections (#639): Self-contradiction in AC-3 resolved (postcondition wins — regression pin replaces removal instruction); AC-1/2/5/7 regression pins added; AC-14 false-green fixed (--project PROJ + positive BC-3.8.016 assertion); AC-13/16/17/18/19 per-AC discrimination notes; Outputs/Effects file cite corrected; SSOT steps 3/4 deduped; AC-11 re-anchored to step 3. BC count unchanged (140/111). (D-188 ratified 2026-07-25)
...
SOH-DX-1 D-188 round-13 adversary-pass corrections (#639): AC-8 citation corrected (F13-01); AC-11 false-green hardened with explicit MUST NOT preconditions (F13-02); AC-12 whitespace-normalization prescribed before count (F13-03); AC-9 second precondition added (F13-04). (D-188 ratified 2026-07-25)
...
SOH-DX-1 D-188 round-12 adversary-pass corrections (#639): output-mode annotation per AC (F12-05); SSOT step 4a (`--description-stdin` blocking read) added between step 4 and step 5 (F12-06); helper-promotion directive extended with stale doc-comment fix note (LOW-1); BC-INDEX section 3.8 header "superseded"→"amended" (LOW-2). (D-188 ratified 2026-07-25)
...
SOH-DX-1 D-188 round-11 adversary-pass corrections (#639): combined error string parenthetical trimmed (F11-01); BC-3.8.013 prose hedged (F11-01); AC-12 dual-assertion re-spec (F11-02); delivery item (d) FIRST-LINE-ONLY (F11-03); vacuity rationale at Removal postcondition (F11-04); AC-17/18/19 added covering EC-3.8.012-5/7/9 (F11-05); BC-INDEX BC-3.4.014 D-188 qualifier (F11-06); spec-changelog [1.3.107] Type MINOR→PATCH (process-gap).
...
- `.factory/specs/prd/BC-INDEX.md` (MODIFIED): BC-3.4.014 row Summary extended with D-188 qualifier matching body text: "Echo fires only when neither `--field` nor `--on-behalf-of` is present without `--request-type`; if either JSM-only flag is present without `--request-type`, exit 64 fires per BC-3.8.012/013 before field echo is reached" (F11-06). `last_updated` updated to v1.3.109.
...
SOH-DX-1 D-188 round-10 spec-text corrections (#639): BC-3.8.013 error string parenthetical removed; AC-12 renamed + help-text pin; ADR-0014 three-site enumeration; AC namespace note added; `assert_json_error_envelope` promotion directive finalized.
...
SOH-DX-1 D-188 amendment (#639): BC-3.8.012 and BC-3.8.013 behavior break — warn-and-proceed (exit 0) superseded by pre-flight `JrError::UserError` exit 64 when `--field`/`--on-behalf-of` are present without `--request-type`; product-breaking change; ships v0.7.0-dev.1. AC-1..16 specified (AC-1..7 inverted/re-specified; AC-8..16 new test deliverables); EC-3.8.012-1..9 + EC-3.8.013-1 added; Platform-Path Guard Ordering SSOT block added; section 3.8 retitled from "Inverse Warnings" to "Pre-flight Guards"; BC-3.8.001/BC-3.3.001 qualified with D-188 caveat; delivery phasing corrected (F4 obligations land same PR as code change).

### Changed

- `.factory/specs/prd/bc-3-issue-write.md` (MODIFIED): BC-3.8.012 amended — warn-and-proceed (exit 0) → pre-flight `JrError::UserError` exit 64; Platform-Path Guard Ordering SSOT block added (prompt-list corrected: description removed; off-by-one fixed: steps 2–7); EC-3.8.012-1..9 added; EC-3.8.013-1 added; AC-1..16 specified — AC-1..7 inverted/re-specified (including AC-4/6 vacuity fixes), AC-8..16 new test deliverables (AC-8 zero-HTTP discriminator; AC-9 no-project/TempDir; AC-10 JSON error shape; AC-11 interactive-TTY; AC-12 --help string pin; AC-13..16 EC coverage); removal postcondition + stdout postcondition added (both BCs); asymmetry rationale direction corrected (two-direction explicit contrast); delivery phasing corrected to F4-same-PR obligations; BC-3.8.013 Trace additions (AC-3/AC-4/AC-16 cross-refs); `assert_json_error_envelope` promoted to `tests/common/fixtures.rs` (F3 deliverable). BC-3.8.001 H1 qualification added "(unless `--field`/`--on-behalf-of` present — exits 64 per BC-3.8.012/013)". BC-3.3.001 amendment notes updated with D-188 timeline. Section 3.8 heading retitled to "Pre-flight Guards". BC-3.4.014 precondition D-188 qualifier appended. Frontmatter version-log entries added. BC count unchanged (140/111).
- `.factory/specs/prd/BC-INDEX.md` (MODIFIED): Section 3.8 heading retitled; BC-3.8.001 Summary qualified with D-188 caveat; BC-3.8.012/013 rows marked [AMENDED D-188 2026-07-25] with new summary; BC-3.3.001 row amended 2026-05-19 annotation scoped with past-tense date range (2026-05-19..2026-07-25); `index_version` v6.44 → v6.45; `last_updated` updated.
...
- `.factory/phase-f1-delta-analysis/impact-boundary-576.md` (MODIFIED): R3.12 section added (--replace-existing ≥1-match confirmation gate ruling; D-180 precedent basis; gate mechanics; spec impact summary).
...
GAP-R15-001 terminology sync in EC-3.5.003-3 + EC-3.5.008-5 (bc-3-issue-write.md): stale "dialoguer::Error" terminology replaced with ratified D-174 mechanism language (`io::stdin().lock().read_line()` returning `Ok(0)` (EOF) or `Err(_)` (IO error) → `JrError::Interrupted` exit 130). No behavioral change — exit 130 on EOF/interrupt was always the contract; only the mechanism terminology changed.
...
F2 spec evolution for SOH-ATTACHMENTS-1 (issues #576 + #585). Adds 27 new individually-bodied BCs: Section 2.7 (Attachment Read, 12 BCs), Section 3.9 (Attachment Write, 14 BCs), and BC-X.8.010 (serviceDeskId cache). Ratified design per D-179.
...
- `.factory/specs/prd/bc-3-issue-write.md` (MODIFIED): Section 3.9 Attachment Write added (BC-3.9.001..014) — platform upload POST (X-Atlassian-Token, streaming, no client-size cap, 413/400), JSM default (internal by default P2-4a), --public two-step (D-174 confirmation gate), --internal two-step (OQ-9 non-JSM silent no-op), --public non-JSM exit 64, temporaryAttachmentId TTL, post-upload echo (P2-3c deferred S5), attachment delete (D-168/BC-3.5.004 precedent), JSON output shapes, error taxonomies, confirmation gate (eprint!+read_line, NOT dialoguer). `total_bcs: 120 → 134`, `definitional_count: 91 → 105`.
...
| VP count | 30 (unchanged) |
| Holdout count | 88 (unchanged) |
| ADR reference | D-179 (F1 gate approval); ADR-0017 Accepted 2026-07-15 (`.factory/specs/architecture/decisions/ADR-0017-first-multipart-streaming-http-surface.md` — multipart/streaming HTTP surface) [CONS-576-007 correction: was "planned", ADR exists Accepted on same date] |
...
F2 gate closure D-170 fix round 49 for SOH-COMMENT-CRUD-1 bundle (issue #577). Human-ruled: no full re-convergence needed — mechanical mirror of ratified items (f)/(g). Two delivery obligations added to EC-3.5.012-5 as items (h) and (i). VP count unchanged at 30.

### Changes

- `.factory/specs/prd/bc-3-issue-write.md` (MODIFIED):
  Gate closure (D-170 consistency-audit ruling): EC-3.5.012-5 extended with two new items appended after item (g):
  ...
  Both items carry attribution note: "(added at F2 gate closure per D-170 consistency-audit ruling, 2026-07-11)".
...
delete/edit/view`). D-168 human gate approved 2026-07-09. Adds 11 new BCs
...
| Design decision | --public always-confirm (Option a; D-168 open point resolved; recorded in BC-3.5.007) |
...
Post-fix micro-BC for SOH-BUGS-1 bundle (D-165, human-approved as recommended).
...
approved and 3-clean-converged story (S-WIN-6, F3 gate APPROVED D-080/D-082/D-084).
...
S-WIN-3 F4 implementation-confirmed deny.toml scope reconciliation (D-082 follow-on).
...
- **F-WIN3-IMPL-102 (transitive-deny-scope):** The D-082 correction had documented a single
...
ADR-0016 date remains 2026-06-13 (already set by D-082 amendment — no further date bump
...
(D-082/083 passes A/B/C) was already complete before F4 began; no new adversarial
...
| Governing decision | — | D-082 follow-on (F4 implementation-confirmed) | STATE.md |
...
reconciliation within an already-approved story (S-WIN-3, F3 gate RE-AFFIRMED D-084).
...
D-082/083 already validated the full skip requirement. Standard per-story convergence
...
D-082 pre-F4 external-claim verification corrections propagated to F3-converged artifacts.
...
F3-convergence + D-082 corrections. ADR-0016 date bumped to 2026-06-13 (effective amendment
...
| Stories total | 74 | 74 | 0 |
| Governing decision | — | D-082 | STATE.md decisions log |
...
- Phase F5 adversarial re-review (scoped to D-082 corrections)
...
F7 post-merge spec-example sync — AC-1 example assertion strings updated multi-word→single-token to match shipped impl (D-074 F3 wrap fix). `contains("Section Header")` → `contains("Header")`, `contains("link text")` → `contains("link")`, `contains("code snippet")` → `contains("snippet")`, `contains("nested blockquote text")` → `contains("blockquote")`. Added one-line note that single-token assertions resist comfy-table `ContentArrangement::Dynamic` cell-wrap. AC-2's `adf_contains_text("nested blockquote text")` (raw ADF JSON check) is unchanged. No BC/NFR change (594/41).
