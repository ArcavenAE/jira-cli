...
- **403-scope detection in parse_error**: promoting the 403-body `"scope"` substring check into `parse_error` as a general-purpose branch (parallel to the existing 401-scope branch) is a follow-up story candidate — reduces duplication vs. per-handler carve-out at BC-3.5.004/005/010.
