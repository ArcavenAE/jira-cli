...
<!-- STATE.md SIZE BUDGET (2026-09-05, cycle-004 Burst 19 -- F6 COMPLETE close-out, F6->F7 transition;
     line count refreshed same burst after splitting the F7 pre-gate step row into
     "input-hash drift check" (DONE) + "consistency audit" (PENDING) once the drift check ran):
     318 lines (wc-l) -- this burst compacts the prior burst's (Burst 18) F5-convergence close-out
     narrative and records the F6 targeted-hardening outcome as a single COMPLETE summary now that
     the fix PR (PR #775) has landed clean, plus the F7 pre-gate input-hash drift check's
     Drift/Standing-Items and maintenance-item entries added later this same burst -- the
     per-dimension hardening detail lives in cycles/cycle-004/burst-log.md Burst 19 and
     phase-f6-hardening/cycle-004/summary.md (which itself cites, rather than duplicates,
     kani-results.md, fuzz-results.md, mutation-results.md, security-scan-results.md, and
     code-delivery/FIX-F6-1-pr-review.md); the input-hash drift check's own report lives at
     phase-f7-convergence/cycle-004/input-hash-drift.md (cited, not duplicated); the superseded
     v3.70 Session Resume Checkpoint is archived to cycles/cycle-004/session-checkpoints.md.
     soft-target 200; hard cap 500; margin from soft-target = 118 lines OVER the soft target
     (documented, ongoing known deviation across cycles-002/003/004, not a blocker); margin from actual
     (hard cap) = 182 lines of headroom remain before the hard cap of 500. RECOVERY CONTEXT:
...
| **Current Phase** | Feature Mode cycle-004 (`windows-correctness`) -- **Phase F7 (delta convergence), IN PROGRESS** -- F6 fully complete: Kani/fuzz justified-skip, mutation gap found+partially-closed, security CLEAN, regression GREEN. F7 pre-gate input-hash drift check DONE (no gate-blocking drift). cycle-001, cycle-002, and cycle-003 remain CLOSED, historical. |
...
| F7-DELTA-CONVERGENCE (cycle-004) | **IN PROGRESS** | — | 5-dimensional convergence check on the delta + full regression + REQUIRED manual Windows-11 smoke gate + final human gate | Pre-gate input-hash drift check DONE this burst (no gate-blocking drift; detail `phase-f7-convergence/cycle-004/input-hash-drift.md`). Remaining: pre-gate consistency audit, 5-dimensional delta convergence check on the F6-hardened delta (`develop` @ `024de4d8`), full-codebase regression, the REQUIRED manual Windows-11 smoke gate (human reproduces #759 on real Windows 11), and the final human F7 convergence gate. | counts unchanged (742/55/106/172); F7 in progress |

## Current Phase Steps (cycle-004, Phase F7 delta convergence — in progress)
...
| F7 pre-gate input-hash drift check | **DONE (this burst)** | Factory-wide scan (215 tracked artifacts). No gate-blocking drift: 2 cycle-004 `[live-state]`-sentinel files are by-design non-blocking; 9 real-hash cycle-004 F1-F3 artifacts drift as one traceable cascade rooted in expected F4-F6 implementation changes to their declared `src/`/`STATE.md` inputs (already independently re-validated by F4/F5/F6) — pure-recompute candidates, not re-opened for content review. 165 factory-wide STALE artifacts confirmed as the pre-existing `F7-GATE-SYSTEMIC-INPUT-HASH-DRIFT-BOOKKEEPING` standing pool, untouched. Full detail: `phase-f7-convergence/cycle-004/input-hash-drift.md`. |
| F7 pre-gate consistency audit | **PENDING — NEXT** | Fresh-context `consistency-validator` audit of the cycle-004 delta, ahead of the 5-dimensional convergence check. |
...
**cycle-004 note (Burst 19, this burst):** DEC-341 (F6 COMPLETE) recorded above. The F7 pre-gate input-hash drift check ran this burst with no gate-blocking result (detail `phase-f7-convergence/cycle-004/input-hash-drift.md`). The next open decision points are: (1) the F7 pre-gate consistency audit + 5-dimensional delta convergence check, then (2) the REQUIRED manual Windows-11 smoke gate, then (3) the final F7 human gate → release.
...
- **CYCLE-004-INPUT-HASH-HYGIENE** (new, this burst) — 9 cycle-004 F1-F3 delta artifacts (`delta-analysis.md`, `architecture-delta.md`, `decomposition-manifest.md`, `S-cycle4-windows-docs.md`, `conflict-report.md`, `dependency-graph-extended.md`, `wave-schedule.md`, both `wave-*-holdout-scenarios.md`) have a stale `input-hash` — a single traceable cascade rooted in expected F4-F6 implementation changes to their declared `src/`/`STATE.md` inputs, not a content defect (F4/F5/F6 already independently re-validated the delivered code against these specs). Pure-recompute candidates (`compute-input-hash --update`), owned by business-analyst/architect/story-writer respectively, not state-manager. Non-blocking for the F7 gate. Detail: `phase-f7-convergence/cycle-004/input-hash-drift.md`.
...
8. **F7 pre-gate input-hash drift check ran and completed**, later this same burst: factory-wide scan of 215 tracked artifacts found no gate-blocking drift for cycle-004 — 2 `[live-state]`-sentinel files are by-design; 9 real-hash F1-F3 delta artifacts drift as one traceable cascade from expected F4-F6 implementation activity (already re-validated by those phases), flagged as a non-blocking hash-refresh candidate (`CYCLE-004-INPUT-HASH-HYGIENE`); the 165-artifact factory-wide standing pool was confirmed and left untouched. Report: `phase-f7-convergence/cycle-004/input-hash-drift.md`.

**NEXT ACTION on resume (exact, in order):**
1. Dispatch the **F7 pre-gate fresh-context consistency audit** (`consistency-validator`) on the cycle-004 delta (the input-hash drift check is already DONE, see above).
...
- **CYCLE-004-INPUT-HASH-HYGIENE** (new, this burst) — 9 cycle-004 F1-F3 delta artifacts need an `input-hash` refresh (pure recompute, no content change) after the F4-F6 implementation cascade; see `Constraints Carried Forward` above and `phase-f7-convergence/cycle-004/input-hash-drift.md`. Non-blocking for F7.
