...
project: jira-cli
mode: brownfield
current_step: "SOH-DX-1 F2 ADVERSARY GRIND IN PROGRESS (2026-07-26): 23 rounds complete (pass-1 8F 4H/4M → fix-1 → ... → pass-22 2F 0H/2M CONVERGING (4→2) → fix-22 → pass-23 3F 1H/2L REGRESSION (2→3) + 1 out-of-delta obs; HIGH: BC-INDEX index_version field stale by 4 (machine/prose drift, v6.46→v6.50 healed); LOWs: anchor refresh; pub fn directive; out-of-delta obs: prd/README 603-vs-657 (repaired opportunistically DEC-158); novelty LOW — first LOW-novelty pass; 9th process-gap: prd/README.md unguarded count surface; spec v1.3.121 + [1.3.121]; piecewise CLEAN; 3 guards green. ZERO consecutive CLEAN; need 3 STRICT. NEXT: pass-24. D-chain cite D-27893 latest brownfield. trajectory-tail →5→4→2→3."
...
<!--
STATE.md SIZE BUDGET (2026-07-26 update: SOH-DX-1 F2 ADVERSARY GRIND ROUND 23):
...
Hard cap: 500 lines. Prior: 239 lines. Net delta: +1 line (PG-009 drift item added; PP row rotated: pass-18-adversary archived, pass-23-adversary added; CPS row rotated: pass-19-adversary archived, pass-23-adversary added; all 5 trajectory-tail sites updated).
-->
...
| **Last Updated** | 2026-07-26: SOH-DX-1 F2 ADVERSARY GRIND — 23 rounds complete (pass-1 8F→...→pass-22 2F 0H/2M CONVERGING (4→2)→pass-23 3F 1H/2L REGRESSION (2→3); HIGH: BC-INDEX index_version field stale by 4 (machine/prose drift, v6.46→v6.50 healed); LOWs: anchor refresh; pub fn directive; out-of-delta obs: prd/README repaired opportunistically DEC-158; novelty LOW (first LOW-novelty pass); 9th process-gap: prd/README.md unguarded count surface; spec v1.3.121 [1.3.121]; piecewise CLEAN; 3 guards green). ZERO consecutive CLEAN; need 3 STRICT. NEXT: pass-24. trajectory-tail →5→4→2→3. Prior: rounds 1-22 complete; fix-22: ALL KEPT clauses exclusion-form (class-kill per adversary process suggestion); AC-1 presence-only/--no-input notes; EC-2 whitespace variant; adversary verdict 'contract layer appears converged'; spec v1.3.120. trajectory-tail →3→5→4→2 |
| **Current Phase** | Feature Mode SOH-DX-1 ACTIVE — F2 ADVERSARY GRIND IN PROGRESS (2026-07-26). Rounds 1-23 complete; trajectory 8→8→8→7→6→4→3→7→6→5→6→6→4→2→5→5→4→3→3→5→4→2→3; REGRESSION (2→3 pass-22→pass-23); 1 HIGH; novelty LOW (first LOW-novelty pass); index_version machine-field drift class. ZERO consecutive CLEAN passes (need 3 STRICT per DEC-189). NEXT: pass-24 → continue grind → human gate after STRICT convergence. |
...
BC-INDEX v6.50 / VP-INDEX v0.82 / STORY-INDEX v1.5.41 / ARCH-INDEX v0.16

SOH-DX-1 ACTIVE — F2 ADVERSARY GRIND IN PROGRESS 2026-07-26: 23 rounds complete; trajectory 8→8→8→7→6→4→3→7→6→5→6→6→4→2→5→5→4→3→3→5→4→2→3; REGRESSION (2→3 pass-22→pass-23); 1 HIGH (BC-INDEX index_version machine-field drift); novelty LOW — first LOW-novelty pass; piecewise CLEAN after fix round 23; ZERO consecutive CLEAN (need 3 STRICT per DEC-189). spec v1.3.121; BC-INDEX v6.50; STORY-INDEX v1.5.41 (S-383 superseded banner). NEXT: pass-24 → continue grind → human gate after STRICT convergence. trajectory-tail →5→4→2→3
...
| SOH-DX-1 (issues #639+#627+#626) | F2 ADVERSARY GRIND IN PROGRESS (2026-07-26). 23 rounds complete; trajectory 8→8→8→7→6→4→3→7→6→5→6→6→4→2→5→5→4→3→3→5→4→2→3; REGRESSION (2→3 pass-22→pass-23); 1 HIGH; novelty LOW (first LOW-novelty pass); ZERO consecutive CLEAN (need 3 STRICT per DEC-189). NEXT: pass-24. | 3 stories: S-639-1 (HIGH/breaking), S-627-1 (LOW), S-626-1 (LOW/MED). BC-3.8.012/013 superseded; spec v1.3.121; BC-INDEX v6.50; STORY-INDEX v1.5.41. v0.7.0-dev.1 target. trajectory-tail →5→4→2→3 |

## Session Resume Checkpoint

| Field | Value |
|-------|-------|
| Date | 2026-07-26 (SOH-DX-1 F2 ADVERSARY GRIND — rounds 1-23 complete; trajectory 8→8→8→7→6→4→3→7→6→5→6→6→4→2→5→5→4→3→3→5→4→2→3; REGRESSION (2→3 pass-22→pass-23); 1 HIGH (BC-INDEX index_version machine-field drift); novelty LOW — first LOW-novelty pass; ZERO consecutive CLEAN; NEXT: pass-24. DEC-189 STRICT criterion in force.) |
| Position | Feature Mode SOH-DX-1 F2 ADVERSARY GRIND IN PROGRESS (2026-07-26). develop @ e72b0166. v0.6.0-dev.11 released (tag @ 34d2f795). Spec v1.3.121; BC 657/holdouts 100/VP 35; AC 85 (AC-1..21 in S-639-1); BC-INDEX v6.50; STORY-INDEX v1.5.41. F2 adversary rounds 1-23 complete; NEXT: pass-24 (adversary convergence, 3-clean-pass STRICT minimum per DEC-189). |
| Convergence counter | SOH-DX-1 F2 ADVERSARY GRIND 2026-07-26. 23 passes complete; trajectory 8→8→8→7→6→4→3→7→6→5→6→6→4→2→5→5→4→3→3→5→4→2→3; REGRESSION (2→3 pass-22→pass-23); 1 HIGH; novelty LOW (first LOW-novelty pass); ZERO consecutive CLEAN (need 3 STRICT per DEC-189). Prior cycle: SOH-ATTACHMENTS-1 CYCLE FULLY CLOSED 2026-07-25 (DEC-186; F5 STRICT CONVERGED 14r/8 PRs; F6 PASS; F7 5/5 PASS). trajectory-tail →5→4→2→3 |
| In flight / On resume | F2 adversary grind in progress. No in-flight worktrees (fix burst rounds 1-23 complete, piecewise CLEAN, all .factory edits committed in this burst). Resume at pass-24. |
...
| Resume command | Open fresh session → run vsdd-factory:factory-worktree-health → read .factory/STATE.md → dispatch F2 adversary pass-24 via /vsdd-factory:next-step (same fresh-context prompt shape; artifacts: bc-3-issue-write.md §3.8, BC-INDEX §3.8/§3.4, spec-changelog [1.3.121], S-383 banner, delta-analysis). DEC-189 STRICT criterion (3 consecutive CLEAN required; any delta-attributable finding resets window). Signal: pass-23 novelty LOW (first LOW-novelty pass) — index_version machine-field drift class; all 3 findings fixed in-round. |

## RESUME PLAN (cold-start, self-contained)

Step 1 (BLOCKING): Run vsdd-factory:factory-worktree-health. Then read .factory/STATE.md (this file).

Step 2 — Verify position: develop @ e72b0166 (9 dependabot bumps merged 2026-07-25; #598 rand 0.10.1→0.10.2 was final merge after auto-rebase). v0.6.0-dev.11 released (tag @ 34d2f795). SOH-DX-1 F2 ADVERSARY GRIND IN PROGRESS (2026-07-26). F2 rounds 1-23 complete; trajectory 8→8→8→7→6→4→3→7→6→5→6→6→4→2→5→5→4→3→3→5→4→2→3; REGRESSION (2→3 pass-22→pass-23); 1 HIGH; novelty LOW; ZERO consecutive CLEAN. No in-flight worktrees. Counters: BC 657, NFR 42, ADR 17, Stories 117, Holdouts 100, VP 35, AC 85 (AC-1..21). Spec v1.3.121. BC-INDEX v6.50. STORY-INDEX v1.5.41. Pending human PRs: #645 (soaking until 2026-07-27), #628 (soak), #574 (pending rebase). DO NOT close #429.

Step 3 — SOH-DX-1 F2 ADVERSARY GRIND IN PROGRESS (2026-07-26): 23 adversary rounds complete; trajectory 8→8→8→7→6→4→3→7→6→5→6→6→4→2→5→5→4→3→3→5→4→2→3; REGRESSION (2→3 pass-22→pass-23); 1 HIGH (BC-INDEX index_version machine-field drift, v6.46→v6.50 healed); novelty LOW — first LOW-novelty pass; piecewise CLEAN after round 23; ZERO consecutive CLEAN (need 3 STRICT per DEC-189). spec-changelog [1.3.121] registered (also [1.3.120]; [1.3.119]; [1.3.118]; [1.3.117]; [1.3.116]; [1.3.115]; [1.3.114]; [1.3.113]; [1.3.112]; [1.3.111]; [1.3.110]; [1.3.109]; [1.3.107] Type MINOR→PATCH corrected); STORY-INDEX v1.5.41 (S-383 superseded); AC surface AC-1..21 (unchanged rounds 20-23). 9 process-gaps ledgered (SOH-DX-1-PG-001..009; PG-009 added round 23: prd/README.md unguarded 9th count surface). DEC-189 STRICT criterion: 3 consecutive CLEAN required; any delta-attributable finding resets the window. NEXT: dispatch pass-24 adversary via /vsdd-factory:next-step. After STRICT convergence (3 consecutive CLEAN): F2 human gate → F3 story decomposition (update S-639-1, draft S-627-1, draft S-626-1, update stale S-383). S-626-1: replacement SHA fa04a145 re-verify at F4. ENGINE IPs (5) to vsdd-factory after this cycle closes.
