...
|-------|-------|
| **Date** | 2026-06-02 |
| **Position** | **JSM E2E expansion at F2-COMPLETE / F3-PENDING.** DEC-064. develop HEAD = afa12570 (no code merged for JSM-E2E; factory-artifacts has the planning trail). 59 stories / 41 NFRs / 585 BCs. No active worktrees. |
| **Convergence counter** | BC: 585. NFR: 41. Stories: 59. develop HEAD: afa12570. jira-e2e env: JR_E2E_ISSUE_TYPE_ALT=Bug. DRIFT-331-PAGINATION: log-only (deferred). |
| **Key artifacts** | Brainstorm: `.factory/planning/brainstorming-report-jsm-e2e.md`. F1 delta: `.factory/phase-f1-delta-analysis/jsm-e2e-expansion/delta-analysis.md`. F2 spec CANONICAL: `docs/specs/jsm-e2e-coverage.md` (main checkout, untracked). Snapshot: `.factory/phase-f2-spec-evolution/jsm-e2e-coverage.md`. |
| **CRITICAL — L-459-1** | `docs/specs/jsm-e2e-coverage.md` is UNTRACKED in the main checkout. A verbatim snapshot is preserved at `.factory/phase-f2-spec-evolution/jsm-e2e-coverage.md`. F4 MUST commit the spec onto the feature branch (copy from snapshot or main-checkout file) before adversary pass — do NOT branch F4 off origin/develop and lose it. |
| **Rollout pending** | Set `JR_E2E_JSM_PROJECT=EJ` as an environment variable in the `jira-e2e` GitHub Environment (part of F4/rollout; already wired in e2e.yml; no workflow change needed). |
...
| **Standing context** | assign-by-query #458 + fork-safe E2E #459 both CLOSED+LIVE-GREEN. JR_E2E_ENABLED=true repo var set. JR_E2E_ISSUE_TYPE_ALT=Bug in jira-e2e env. Do NOT close #429 (human decision, DEC-029). OQ-5 open. E2E-PG-4 remote-link round-back sub-gap open (blocked: no `jr remote-link read`). |
| **Next step** | F3: create story S-JSM-E2E-1 (~3 SP, zero-src, 7 scenarios per DEC-064). F4: deliver tests/e2e_live.rs (7 gated tests), tests/e2e_cli_surface_guard.rs (4 SURFACE rows), docs/specs/e2e-live-jira-testing.md §4/§8, CLAUDE.md E2E note. Commit F2 spec onto feature branch before adversary (L-459-1). |
| **Resume prompt** | `Read .factory/STATE.md. JSM E2E expansion: F2-COMPLETE / F3-PENDING (DEC-064). develop HEAD = afa12570 (no JSM-E2E code merged). factory-artifacts HEAD = git -C .factory log -1 --format='%h'. CRITICAL (L-459-1): docs/specs/jsm-e2e-coverage.md is UNTRACKED in main checkout; snapshot at .factory/phase-f2-spec-evolution/jsm-e2e-coverage.md; F4 MUST commit spec onto feature branch before adversary pass. Story to create: S-JSM-E2E-1 (~3 SP, zero-src, 7 scenarios). Rollout pending: set JR_E2E_JSM_PROJECT=EJ in jira-e2e GitHub Environment (env-scoped var, already wired in e2e.yml). Deferred: --on-behalf-of (2nd customer), 401 scope hint (scope-stripped token). jira-e2e env: JR_E2E_ISSUE_TYPE_ALT=Bug. 59 stories / 41 NFRs / 585 BCs. Do NOT close #429 (DEC-029). OQ-5 open. E2E-PG-4 remote-link round-back open.` |
