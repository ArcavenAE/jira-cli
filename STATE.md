...
project: jira-cli
mode: brownfield
current_step: "D-chain cite D-453 latest brownfield (unchanged). trajectory-tail →1→3→0→2 (unchanged this burst). cycle-004 F1 delta-analysis APPROVED at human gate (DEC-335): 4-story scope (dpapi-storage-fix + honest-fail-message [bundled], windows-docs, cloud_id-correctness incl A-PA-LOW-001). Windows-validation = F4 CI spike + REQUIRED F7 manual gate. Advancing to F2 spec evolution. develop @ 42e92b46."
...
cycle_004_status: "windows-correctness -- F1 delta-analysis APPROVED at human gate 2026-09-03 (DEC-335); Phase F2 spec-evolution IN PROGRESS. 4-story scope: dpapi-storage-fix + honest-fail-message (bundled into one release), windows-docs, cloud_id-correctness (incl. A-PA-LOW-001). Windows-validation: F4 CI spike (windows-latest DPAPI feasibility) + REQUIRED F7 manual Windows smoke gate. windows-vs-windows-sys dependency decision deferred to F2 ADR."
activation_head: "42e92b46"
activation_version: "v0.7.0-dev.4"
---

<!-- STATE.md SIZE BUDGET (2026-09-03, cycle-004 F1-APPROVED burst / Burst 2):
     271 lines (wc-l) -- shrank 6 lines from the prior burst's 277: one
     Phase Progress row flipped to APPROVED plus one new F2-SPEC-EVOLUTION
     row, a reset (shorter) Current Phase Steps table, one new DEC-335 row,
     updated Convergence Status/Concurrent Cycles/Constraints Carried
     Forward/Drift-Standing prose recording the F1 human-gate outcome
     (4-story scope, Windows-validation plan accepted, windows-vs-windows-sys
     deferred to F2), and a replaced Session Resume Checkpoint; no new
     structural section added.
     soft-target 200; hard cap 500. margin from soft-target = 71 -- 71 lines
     OVER the soft target of 200 (documented, ongoing known deviation across
     cycles-002/003/004, not a blocker). margin from actual (hard cap) = 229
     lines of headroom remain before the hard cap of 500.
     This burst records cycle-004 (`windows-correctness`) Phase F1's human
     gate: the human reviewed the architect's F1 delta-analysis
     (`cycles/cycle-004/phase-f1-delta-analysis/delta-analysis.md`, 549
     lines, §13 open questions) and APPROVED it -- standard F1->F7 route,
     with the scope EXPANDED from the #759+#760 bundle at cycle-open (Burst
     1) to a 4-story decomposition: `dpapi-storage-fix` (#759 durable fix --
     keyring-first + user-scope DPAPI-encrypted-file fallback for oversized
     OAuth access+refresh tokens, atomic) + `honest-fail-message` (#759
     backstop -- accurate `keyring::Error::TooLong` message + explicit
     dangling-grant revoke), which the human explicitly bundled into ONE
     release rather than sequencing independently; `windows-docs` (#760);
     and `cloud_id-correctness` (human-added -- fetch+persist `cloud_id` via
     `GET /_edge/tenant_info` on API-token login, which ALSO closes the
     cycle-003-carried-forward `A-PA-LOW-001` standing item). The human
     accepted the F1 report's Windows-validation plan as proposed: F4 spikes
     whether `windows-latest` CI can exercise DPAPI, and a manual smoke test
     on real Windows is a REQUIRED (not optional) F7 gate. Two items were
     explicitly deferred/confirmed rather than decided now: the
     `windows`-crate-vs-`windows-sys` dependency choice moves to a F2 ADR;
     `clear_all_credentials` stays untouched (test-only, pre-existing
     do-not-reintroduce warning). `phase` frontmatter -> F2; `current_step`
     updated; `cycle_004_status` updated to reflect F1 APPROVED / F2 next /
     4-story scope. New DEC-335 records the human's F1-gate approval
     (collision-checked: highest prior ID was DEC-334, no collision).
     Counts unchanged: 733 BCs / 41 VPs / 106 holdout scenarios / 168
     stories -- F1 does not create/modify specs; that begins at F2. All
     cycle-001/002/003 Drift/Standing items are carried forward verbatim.
     One full-content Write, no Edit chain (DEC-247). Hygiene: this burst
     ALSO commits the F1 artifacts (`delta-analysis.md`, `affected-files.txt`)
     plus the previously-untracked `research/win-oauth-keychain-blob-limit-
     2026-09-03.md` and modified `research/RESEARCH-INDEX.md` (left
     uncommitted at Burst 1) -- resolving that hygiene note. The three
     OTHER pre-existing-dirty files unrelated to any cycle
     (`regression-state.json`, `sidecar-learning.md`, the modified
     `S-cycle3-env-tag` demo gif) remain explicitly NOT staged this burst. -->
...
| **Last Updated** | trajectory-tail →1→3→0→2 (unchanged). F1 HUMAN-GATE burst (2026-09-03): cycle-004 (`windows-correctness`) F1 delta-analysis **APPROVED** (DEC-335) -- standard F1→F7 route, 4-story scope (`dpapi-storage-fix` + `honest-fail-message` bundled, `windows-docs`, `cloud_id-correctness` incl. `A-PA-LOW-001`). Advancing to Phase F2 spec evolution. `develop` unchanged at `42e92b46`. cycle-001/002/003 remain CLOSED. |
| **Current Phase** | Feature Mode cycle-004 (`windows-correctness`) -- **Phase F2 (spec evolution), IN PROGRESS** (F1 APPROVED at human gate, DEC-335). cycle-001, cycle-002, and cycle-003 remain CLOSED, historical. |
...
| **F2-SPEC-EVOLUTION (cycle-004)** | **IN PROGRESS** | 2026-09-03 | (feature-mode spec evolution, no human gate at entry) | Incremental PRD/architecture/VP delta for the 4-story scope approved at F1 — not yet dispatched as of this burst. | counts TBD |

## Current Phase Steps (cycle-004, Phase F1→F2 transition)

| Step | Status | Notes |
|------|--------|-------|
| F1 delta-analysis (architect) report delivered | DONE | `delta-analysis.md` (549 lines) + `affected-files.txt` — impact boundary, 3-story (+1 contingent) decomposition, Windows-validation plan, 6 open questions for the gate |
| F1 human gate | APPROVED | DEC-335: standard F1→F7 route; 4-story scope confirmed (`cloud_id-correctness` added, subsuming `A-PA-LOW-001`); Windows-validation plan (F4 CI spike + required F7 manual gate) accepted as proposed; `windows`-vs-`windows-sys` deferred to F2; `clear_all_credentials` confirmed out of scope |
| cycle-004 STATE.md update (F1 APPROVED → F2) | DONE | One full-content Write; DEC-335 recorded; Phase Progress/Current Phase Steps/Convergence Status/Drift-Standing updated; F1 artifacts + research docs committed to factory-artifacts |
| F2 spec evolution dispatch | NEXT | PRD/architecture/VP delta for the 4-story scope — dispatch `/vsdd-factory:phase-f2-spec-evolution` |

(Prior cycle-004 Burst-1 steps — human triage, pre-cycle research pass, DEC-334, scaffolding bootstrap, F1 dispatch — archived to `cycles/cycle-004/burst-log.md` Burst 1.)
...
**cycle-004 note (Burst 2, this burst):** DEC-335 is the F1 human-gate approval, following DEC-334's cycle-open authorization at Burst 1. No F2 spec-evolution findings exist yet — those land once F2 is dispatched.
...
| cycle-004 burst history | `cycles/cycle-004/burst-log.md` (Burst 1 = cycle OPENED — DEC-334 recorded, #759+#760 bundle confirmed, F1 delta-analysis dispatched; Burst 2 = F1 human gate APPROVED — DEC-335 recorded, 4-story scope confirmed [`cloud_id-correctness` added], advanced to F2 spec evolution) |
...
| cycle-004 research grounding | `research/win-oauth-keychain-blob-limit-2026-09-03.md` (pre-cycle research validating the #759 fix strategy; committed to factory-artifacts this burst alongside `research/RESEARCH-INDEX.md`) |
...
**cycle-004 (F1 APPROVED at human gate, Burst 2, this burst):** Human approved the architect's F1 delta-analysis at the human gate (DEC-335): standard F1→F7 route; 4-story scope confirmed — `dpapi-storage-fix` + `honest-fail-message` (#759, bundled into one release) + `windows-docs` (#760) + `cloud_id-correctness` (human-added, fetch+persist `cloud_id` via `GET /_edge/tenant_info` on API-token login). **`A-PA-LOW-001` (stale `cloud_id` survives an oauth→api_token switch) is now IN-SCOPE for cycle-004, addressed directly by story 4 `cloud_id-correctness`** — no longer merely "overlapping" #760's docs scope as noted at cycle-open (Burst 1); see the updated bullet near the bottom of this section. Windows-validation plan ACCEPTED: F4 spikes whether `windows-latest` CI can exercise DPAPI; a manual smoke test on real Windows is a REQUIRED F7 gate, not optional. `windows`-crate-vs-`windows-sys` dependency choice DEFERRED to a F2 ADR (architect flagged — keyring 3.6.3 pulls `windows-sys` 0.60.2 transitively; the bare `windows` crate is currently ABSENT from `Cargo.lock`, so adding it is a new top-level dependency with a `cargo-deny` multiple-versions interaction to weigh). `clear_all_credentials` confirmed out of scope, left untouched (test-only, pre-existing do-not-reintroduce warning). **Hygiene note RESOLVED this burst:** the previously-uncommitted `research/win-oauth-keychain-blob-limit-2026-09-03.md` and modified `research/RESEARCH-INDEX.md` are committed to factory-artifacts this burst alongside STATE.md and the F1 artifacts.
...
- **A-PA-LOW-001** (stale `cloud_id` survives oauth→api_token switch → `jr assets` sends Basic auth to the OAuth gateway → 401 instead of a clean error) — **now IN-SCOPE for cycle-004, addressed by story 4 `cloud_id-correctness` (F1 human gate, DEC-335); see the cycle-004 note above. No longer a standalone future-maintenance item once cycle-004 ships.**
