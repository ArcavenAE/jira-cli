...
project: jira-cli
mode: brownfield
current_step: "IDLE. E2E EDGE-CASE AUDIT COMPLETE (record-only, 2026-06-27). 2 new MEDIUM drift items (E2E-EDGE-CASE-GAPS-2026-06-27 + MISSING-BC-SUBCLAUSE-PATTERN). DEC-137 recorded. develop @ 5ab4e0f (no code changes). Awaiting direction."
...
| **Last Updated** | 2026-06-27: E2E edge-case coverage audit (record-only) — 2-part static audit (read/infra + write/state); DEC-137; E2E-EDGE-CASE-GAPS-2026-06-27 + MISSING-BC-SUBCLAUSE-PATTERN (both MEDIUM) tracked. develop unchanged @ 5ab4e0f. |
| **Current Phase** | Phase 3 — IDLE (E2E edge-case audit complete; v0.6.0-dev.7 shipped). BC 603. NFR 42. ADR 16. Stories 92. Holdouts 70. |
...
| **DEAD-CITATION-CI F1–F7 + RELEASED v0.6.0-dev.6** | **CYCLE CLOSED** | **2026-06-20** | **Full VSDD F1–F7; 8+ defects caught; PRs #544/#545/#546; release.yml 27851891146 SUCCESS; 10 assets/5 targets. DEC-125/126/127/128/129/130.** | develop @ dbe8625 == v0.6.0-dev.6. Stories 83→91. |
...
| **E2E EDGE-CASE AUDIT — COMPLETE (record-only) 2026-06-27** | **COMPLETE** | **2026-06-27** | **2-part static audit (read/infra + write/state); 27 read + ~70 write edges assessed; 5 HIGH + 13 MED + 11 LOW gaps identified. Key finding: live E2E is happy-path-by-design; edge-case/error-injection/ADF-body-shape coverage belongs at wiremock/holdout tiers. Recurring missing-BC-sub-clause pattern identified as gating dependency. Gaps tracked as E2E-EDGE-CASE-GAPS-2026-06-27 + MISSING-BC-SUBCLAUSE-PATTERN (both MEDIUM). DEC-137.** | develop unchanged @ 5ab4e0f. No code merged. |
...
| **E2E EDGE-CASE AUDIT COMPLETE (record-only)** — 2-part static audit: read/infra surface (27 read commands; 2H/4M/4L net-new gaps) + write/state surface (~70 edges; 5H/13M/11L gaps). Key finding: live E2E is happy-path-by-design; edge-case/ADF-body/forced-error coverage belongs at wiremock/holdout tiers. Recurring missing-BC-sub-clause pattern identified as gating dependency for holdout-tier coverage. Gaps tracked: E2E-EDGE-CASE-GAPS-2026-06-27 + MISSING-BC-SUBCLAUSE-PATTERN (both MEDIUM). DEC-137. | state-manager | COMPLETE | develop unchanged @ 5ab4e0f. No code merged. |
...
| E2E-EDGE-CASE-GAPS-2026-06-27 | E2E coverage | Gap inventory from 2-part static E2E edge-case audit (2026-06-27). **Offline CLI tier (cheap):** G-EDIT-FIELD-LABEL-GUARD (--field + --label mutual-exclusion exit-64 offline), G-EDIT-FIELD-C1-BULK (--field on bulk-key set C-1 guard exit-64 offline), extend #526 error-shape JSON contract to `issue changelog` / `queue view` / `requesttype list`. **Wiremock tier:** G-ADF-INV1-INLINE-HTML (BC-7.2.011 INV-1 inline-HTML multi-line via body-capture), G-H2 partial_match no-HTTP short-circuit, G-MOVE-BULK-NONIDEMPOTENT (forced-400 path). **Holdout tier (blocked on missing BC):** G-ADF-FOOTNOTE (#472, needs BC sub-clause — see MISSING-BC-SUBCLAUSE-PATTERN). Infra edges (429/Retry-After, 401 auto-refresh, JRACLOUD-95368/71293) are ALREADY GREEN at wiremock tier and MUST NOT be re-created live. Reports: `.factory/research/e2e-edge-case-audit-2026-06-27-read.md`, `.factory/research/e2e-edge-case-audit-2026-06-27-write.md`. Cross-reference: E2E-PG-4, HOLDOUT-COVERAGE-GAPS-2026-06-25, CACHE-COVERAGE-GAPS-2026-06-27. DEC-137. | MEDIUM | TRACKED DEFERRAL |
| MISSING-BC-SUBCLAUSE-PATTERN | spec/process | Recurring blocker: ADF markdown→ADF behaviors (#471/472/474/483/489/492/522/473), cache D2 warm-hit no-HTTP, and read error-channel/partial_match behaviors are shipped with full test coverage and CLAUDE.md Gotchas entries but LACK a dedicated BC sub-clause — breaking the holdout authoring anchor chain (broken-anchor class). Candidate spec-first pass: author BC sub-clauses for ADF footnote (#472), ADF markdown-minor (#474), ADF GFM-alerts (#483), ADF block-HTML (#489), ADF INV-1 (#522), ADF bare-URL autolink (#473), cache-hit-no-HTTP (D2), and read error-channel/partial_match to unblock the wiremock/holdout backlog. Cross-reference: CACHE-COVERAGE-GAPS-2026-06-27 (no-HTTP BC prerequisite), HOLDOUT-COVERAGE-GAPS-2026-06-25 (holdout anchors), E2E-EDGE-CASE-GAPS-2026-06-27 (holdout-tier blocked items). DEC-137. | MEDIUM | TRACKED DEFERRAL |
...
- develop @ **5ab4e0f** (LOCAL == origin/develop, PR #561 squash-merged 2026-06-27). Tag v0.6.0-dev.7 pushed on 342987f (activation_head). activation_head/version: 342987f / v0.6.0-dev.7.
...
**Step 3 — IDLE. E2E EDGE-CASE AUDIT COMPLETE (record-only). CACHE-COVERAGE HIGH GAPS SHIPPED. D4 CLOSED. v0.6.0-dev.7 SHIPPED. Present open items to human, await direction.**
- **E2E EDGE-CASE AUDIT (2026-06-27):** Static 2-part audit (read/infra + write/state). Key insight: live E2E is happy-path-by-design; edge-case coverage belongs at wiremock/holdout tiers. E2E-EDGE-CASE-GAPS-2026-06-27 + MISSING-BC-SUBCLAUSE-PATTERN tracked (both MEDIUM). DEC-137. Reports: `.factory/research/e2e-edge-case-audit-2026-06-27-read.md`, `.factory/research/e2e-edge-case-audit-2026-06-27-write.md`.
...
- **PR #557 (MERGED @ c70d8a7):** dependabot: softprops/action-gh-release 3.0.0→3.0.1. DEC-133 DEPENDABOT-ACTION-SOAK standing policy now in effect.
...
- **RELEASE-CI-NETWORK-FLAKE** — LOW, open. Consider cargo-fetch retry in release.yml.
...
|-------|-------|--------|----------|-------|
| #561 | test(cache): pin per-profile cache isolation and fields.json self-heal | **CLOSED — squash-merged → develop @ 5ab4e0f (2026-06-27)** | LOW | Cache-coverage P1/P2. 8 unit tests: 6 per-profile isolation (BC-6.2.009) + 2 fields.json self-heal (BC-6.2.011). Code review: 1 MED + 2 LOW (all fixed pre-merge). 15/15 CI green incl. Windows/mutation/coverage. Admin squash-merge (DEC-128). Open PRs (action needed): NONE. |
