...
project: jira-cli
mode: brownfield
current_step: "ADF-CODE-MARK-EXCLUSIVITY (issue #571): F2 spec delta + 5 adv passes / 4 fix rounds. BC-INDEX v6.12, VP-INDEX v0.82, STORY-INDEX v1.02, ARCH-INDEX v0.16. D-chain cite D-27893 latest brownfield. BC 612. Holdouts 83. STREAK 1/3. trajectory-tail →4→5→5→0."
...
<!--
  ...
  Hard cap (500 lines) margin from soft-target = 500 - 268 = 232; margin from actual = 500 - 268 = 232 (D-446(c) dual-margin form). 268 lines (wc-l).
  Hard cap: 500 lines.
-->
...
| **Last Updated** | 2026-07-07: ADF-CODE-MARK-EXCLUSIVITY F2 complete — prd-delta-571.md + verification-delta-571.md authored; 5 adversarial passes / 4 fix rounds; BC 611→612 (BC-7.2.015 ADD + BC-7.2.007 EC-2 MODIFY); holdouts 82→83 (H-NEW-ADF-010 calls A-E incl. JSM Call E); spec v1.3.25; STREAK 1/3; trajectory-tail →4→5→5→0. DEC-157. Pass 6 dispatched. Prior: CITATION-GUARDS CYCLE FULLY CLOSED (DEC-156); PR #592 @ 0d8a8a5; BC 611; Stories 102. |
| **Current Phase** | Phase 3 — **ADF-CODE-MARK-EXCLUSIVITY F2 IN PROGRESS** (issue #571). Spec delta: prd-delta-571.md (BC-7.2.015 ADD + BC-7.2.007 EC-2 MODIFY, H-NEW-ADF-010 calls A-E) + verification-delta-571.md; spec v1.3.25. 5 adversarial passes / 4 fix rounds complete. STREAK 1/3. Pass 6 (spec-ecosystem coherence) dispatched. BC **612**. NFR 42. ADR 16. Stories **102**. Holdouts **83**. |
| **Next Phase** | Pass 6 (spec-ecosystem coherence lens) in progress. After pass 6: if CLEAN → streak 2/3; if findings → fix round 5 + pass 7. Target: 3 consecutive clean diverse-lens passes (STANDARD criterion, DEC-153 precedent). Then F3 story decomposition. |
...
<!-- archived: Phase 0–2 + Feature cycles 2026-05-04..2026-07-06 + CITATION-GUARDS STORY B F4 DELIVERED row (archived this burst) -->
...
| **ADF-CODE-MARK-EXCLUSIVITY F2 SPEC DELTA AUTHORED — BC 611→612 (2026-07-07)** | **COMPLETE** | **2026-07-07** | **prd-delta-571.md: BC-7.2.015 ADD (bandable BC-7.2.016..058) + BC-7.2.007 EC-2 MODIFY (allowlist framing). verification-delta-571.md. H-NEW-ADF-010 calls A-E. spec v1.3.25. BC-INDEX + CANONICAL-COUNTS updated. 3 guard scripts exit 0.** | BC 612. Holdouts 83. develop @ 0d8a8a5 UNCHANGED. |
...
| **pass-5 adversary (ADF-CODE-MARK-EXCLUSIVITY F2 — evaluator-simulation/ground-truth lens, 2026-07-07)** | **IN_PROGRESS** | **2026-07-07** | **CLEAN — 1 LOW-informational VA observation, non-resetting per DEC-153. STREAK 1/3. Pass 6 (spec-ecosystem coherence) dispatched.** | trajectory-tail →4→5→5→0. STREAK 1/3. Pass 6 in flight. |

## Current Phase Steps

<!-- Keep last 5 rows only. Archive older rows to cycles/cycle-001/burst-log.md. -->
<!-- archived: CITATION-GUARDS STORY B F3 CONVERGED + CITATION-GUARDS STORY B F4 DELIVERY COMPLETE (archived this burst 2026-07-07) -->
...
| **ADF-CODE-MARK-EXCLUSIVITY F2 SPEC DELTA AUTHORED (2026-07-07)** — prd-delta-571.md: BC-7.2.015 ADD (bandable BC-7.2.016..058) + BC-7.2.007 EC-2 MODIFY (allowlist framing: retain link/annotation; strip strong/em/strike/subsup/text). verification-delta-571.md. H-NEW-ADF-010 (calls A-E incl. JSM Call E). spec v1.3.25 (changelog re-synced round 4). BC-INDEX, CANONICAL-COUNTS, bc-7-output-render.md, bc-07-output-render.md updated. 3 guard scripts exit 0. | PO + spec-steward | COMPLETE | BC 612. Holdouts 83. spec v1.3.25. |
| **ADF-CODE-MARK-EXCLUSIVITY F2 ADV PASSES 1-5 / 4 FIX ROUNDS COMPLETE — STREAK 1/3 (2026-07-07)** — P1(coherence/registration): 2M+1L fixed. P2(verification-adequacy): 1CRIT(Call E fixture — 3 sub-defects)+1M+2L fixed. P3(implementability): 1M adjudicated deferred per LESSON-F2-WORKTREE-FIRST+4L fixed. P4(hostile-misreading): 2M(BC-INDEX-9TH-SURFACE RECURRENCE×2; spec-changelog stale)+3L fixed. P5(evaluator-simulation): CLEAN — 1LOW-informational non-resetting per DEC-153. STREAK 1/3. Pass 6 dispatched. New drift items: SPEC-CHANGELOG-RESYNC, ADVERSARY-WRITE-TOOL-MISMATCH, D-CHAIN-VALIDATOR-SUBSTRING-FALSE-POSITIVE. | state-manager | IN_PROGRESS | trajectory-tail →4→5→5→0. STREAK 1/3. Pass 6 in flight. |
...
| BC-INDEX-9TH-SURFACE | process-gap | BC-INDEX.md coverage statistics not covered by `check-bc-cumulative-counts.sh` as 9th surface. **RECURRENCE COUNT: 2** (ADF-CODE-MARK F2 pass-4). | LOW | OPEN — guard-extension candidate |
...
Trajectory (ADF-CODE-MARK-EXCLUSIVITY F2): →3→4→5→5→0 (passes 1-5; streak 1/3)

ADF-CODE-MARK-EXCLUSIVITY: F2 IN PROGRESS — 5 passes / 4 fix rounds complete, STREAK 1/3. Pass 6 (spec-ecosystem coherence) dispatched. Convergence criterion: STANDARD (DEC-153 precedent; 3 consecutive clean diverse-lens passes). Prior cycle (CITATION-GUARDS): Story B F4 CONVERGED (4 passes/2 fix rounds); Story B F3 CONVERGED (DEC-153 standard, DEC-155 — 15 passes/9 fix rounds). Full trajectories: `cycles/cycle-001/convergence-trajectory.md`.

## Concurrent Cycles

| Cycle | Status | Notes |
|-------|--------|-------|
| ADF-CODE-MARK-EXCLUSIVITY (issue #571) | ACTIVE — F2 IN PROGRESS | 5 passes/4 fix rounds; STREAK 1/3; trajectory →3→4→5→5→0; Pass 6 dispatched |
...
| Field | Value |
|-------|-------|
| **Date** | 2026-07-07 (ADF-CODE-MARK-EXCLUSIVITY F2 spec delta + 5 adversarial passes / 4 fix rounds complete; STREAK 1/3; pass 6 dispatched) trajectory-tail →4→5→5→0 |
| **Status** | **F2 IN PROGRESS — STREAK 1/3.** prd-delta-571.md + verification-delta-571.md authored. BC 611→612 (BC-7.2.015 + BC-7.2.007 EC-2). Holdouts 82→83 (H-NEW-ADF-010 calls A-E). spec v1.3.25. 5 adv passes / 4 fix rounds. Pass 5 CLEAN (streak 1/3). Pass 6 dispatched. |
| **Counters** | BC **612**. NFR **42**. ADR **16**. Stories **102** (both #101 + #102 delivered). Holdouts **83**. |
| **Convergence counter** | ADF-CODE-MARK-EXCLUSIVITY F2: STREAK 1/3. trajectory →3→4→5→5→0. Pass 6 in flight. STANDARD criterion (DEC-153). Full trajectories: `cycles/cycle-001/convergence-trajectory.md`. |
| **In-flight work** | Pass 6 adversarial review (spec-ecosystem coherence lens) dispatched. develop @ 0d8a8a5 UNCHANGED. factory-artifacts: this burst commit. No open PRs. No active feature worktrees. |
| **Pending decisions** | None — F1 gate fully cleared (DEC-157). Pass 6 result pending; if CLEAN → streak 2/3; if findings → fix round 5. |
...
| **STATE.md size** | ~268 lines (OK band). |
| **Resume command** | Open a fresh session; read `.factory/STATE.md`; run `/vsdd-factory:next-step`. ADF-CODE-MARK F2 in progress — pass 6 dispatched; await result. MUTANTS-FIRST-SCOPED-PR-CALIBRATION: src/adf.rs in scope for F4 = first code-mutant path candidate. |

## RESUME PLAN (cold-start, self-contained)

<!-- State snapshot: ADF-CODE-MARK-EXCLUSIVITY F2 IN PROGRESS (2026-07-07); STREAK 1/3. Pass 6 dispatched. develop @ 0d8a8a5. BC 612; Stories 102 both delivered. Holdouts 83. -->
...
>
> **F2 STATUS: 5 passes / 4 fix rounds complete. STREAK 1/3. Pass 6 dispatched.**
>
> **F2 CARRY-FORWARD FOR F3 STORY:**
> 1. CLAUDE.md gotcha update (~line 293, `^`x`^` behavior) MUST be in story file list (F4 applies it).
