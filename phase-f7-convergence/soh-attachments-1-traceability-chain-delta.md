...
| BC-3.9.008 | AID validated `^[0-9]+$` before any HTTP; DELETE 204→exit 0; 404→exit 64 + Jira body (D-168); SEC-576-004 CRLF/NUL guard | — | `tests/attachment_upload.rs::test_bc_3_9_008_*` | `src/api/jira/attachments.rs::delete_attachment`; `src/api/jira/attachments.rs::delete_attachment_targeted` | Step 4.5 STRICT |
...
| BC-3.9.013 | Delete error taxonomy: invalid AID non-numeric exit 64 zero HTTP (CWE-88); 404 exit 64 (D-168); 403/401/5xx/network | — | `tests/attachment_upload.rs::test_bc_3_9_013_*` | `src/api/jira/attachments.rs::delete_attachment_targeted` | Step 4.5 STRICT; D-168 asymmetry from delete_attachment (benign-skip) |
