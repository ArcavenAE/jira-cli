...
- **`jr issue attachment upload` SEC-576-004 CRLF/NUL filename guard (S-576-3, CWE-93):** `\r`, `\n`, and `\0` in the server-supplied filename are mapped to `_` before passing to `Part::file_name()` in `Content-Disposition`. This prevents header-injection / boundary framing attacks. The guard is in `src/api/jira/attachments.rs::upload_attachments`. Distinct from `display_sanitize_filename` (CWE-116) and `sanitize_attachment_filename` (CWE-22 disk path) — each guard is at its own layer. Tested by `test_sec_576_004_content_disposition_crlf_injection_guard`.
