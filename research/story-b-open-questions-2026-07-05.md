...
| Governance precedent | S-MAINT-DEAD-CITATION-CI (D-125-130), D-129 explicit BC-authorship decision | S-MUTATION-CI-TIMEOUT-1 / S-TESTTOOL-1 / D-144 — policy-doc-only | Draft cites both as ancestor patterns |
...
**Concrete false-green scenario the draft's fallback would ALLOW:** rename `CROSS_HIERARCHY_HINT` to `CROSS_HIER_HINT` in `src/cli/issue/edit.rs` but leave a doc-comment referring to the old name (e.g., `// See CROSS_HIERARCHY_HINT for the legacy form`). The BC citation `src/cli/issue/edit.rs::CROSS_HIERARCHY_HINT` becomes stale, but the secondary grep still finds `CROSS_HIERARCHY_HINT` in the doc-comment → citation false-greens. This is *exactly* the D-148 class the guard is designed to prevent, one level up.
