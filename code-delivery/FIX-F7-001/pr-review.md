...
- `docs/specs/issue-create-preflight-guards.md` — top-of-file "PARTIALLY SUPERSEDED — D-310" notice + inline superseded-row annotations in the behavior table.
...
- **D-310 / S-578-4 are real.** D-310 present in `src/cli/issue/create.rs`, `docs/adr/0014-jsm-request-type-dispatch.md`, and tests. S-578-4 shipped as PR #746 (`feat(issue): issue create --field platform-path createmeta resolution — reverses D-188 (#578 part 5)`); it reverses D-188 from S-639-1, exactly as documented.
- **Current guard logic matches the doc's "current behavior."** `create.rs` pre-flight now fires only on `on_behalf_of.is_some()` (line 122); the `--field`-alone check and the combined `--field`+`--on-behalf-of` check are both removed (comment lines 110-111). This confirms the supersession notices: D-310 reverses the `--field` half; `--on-behalf-of` (BC-3.8.013) is unchanged; the combined-row reduction to on-behalf-of-only is accurate.
