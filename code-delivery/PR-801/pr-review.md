...
| ADR-0011 status correction | **Verified.** `docs/adr/0011-type-level-profile-fence.md` reads **Accepted** (amended 2026-09-01, D-317, originally Deferred) — the new bullet matches the source document. |
...
| Suggestion | Update the entry to `profile: &Profile` and note the newtype (`src/profile.rs`, ADR-0011/D-317) as the fence. Natural fit for this same drift sweep. |
