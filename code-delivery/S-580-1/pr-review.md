...
| **Depth cap** | `MAX_FIELD_OPTION_DEPTH = 256` mirrors `MAX_ADF_DEPTH`'s precedent, enforced in all four recursive sites, with 256/257 boundary pins for both normalizers — and correctly uses `>=`, not the `>` off-by-one that D-132 had to correct in `adf.rs`. |
