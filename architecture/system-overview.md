...
**[PLANNED, F2 2026-08-25 — Field DX, issues #580/#578, ADR-0019; no `src/` code exists yet]:** Pure additions — `cli::field::{FieldOption, FieldValueKind, FieldValueSpec}` (jr-synthesized normalized-option and hint-kind data types, not wire mirrors of any single API response — kept CLI-local per ADR-0019 §1 Rationale, not under `types::jira::`/`types::jsm::`); `cli::field::{normalize_from_allowed_values, normalize_from_valid_values}` (pure functions living inside the otherwise-effectful `cli::field` module — same function-level purity carve-out already documented above for `cli::resolve_effective_limit`/`config::validate_profile_name`); `api::jira::issues::{CreateMetaField, CreateMetaFieldsResponse}` (new inline response types, reusing the existing pure `types::jira::editmeta::{AllowedValue, EditMetaFieldSchema}`). Effectful-shell additions — `cli::field::handle*` (HTTP + cache + stdout), `api::jira::issues::get_createmeta_fields` (new HTTP method, same class as every other L4 method on this `JiraClient` impl). No existing module's purity classification changes — `cli::issue::{edit,create,jsm_create}` remain effectful shell (unchanged classification; only richer payloads flow through their existing `parse_field_kv` call sites), and the new `api::assets::workspace::get_or_fetch_workspace_id` call sites added by the `:asset` hint (BC-3.4.030) reuse an already-effectful, already-classified function verbatim. See `component-graph.md §Field DX Delta` for the full DAG/purity cross-check.

**[PLANNED, F2 2026-09-06 — Markdown Mentions, issue #674, ADR-0023; no `src/` code exists yet]:** Pure additions (all in `adf.rs`, additive to the existing pure-core boundary, no signature change to the existing `markdown_to_adf(markdown: &str)`) — `find_mention_candidates` (read-only tree scan, zero HTTP), `markdown_to_adf_with_mentions(markdown, resolved: &MentionResolutions)` (the real converter body; `markdown_to_adf` becomes a 1-line wrapper calling it with `MentionResolutions::empty()`), `markdown_to_adf_no_mentions` (the `--no-mentions` bypass entrypoint), and the plain data types `MentionCandidates`/`MentionResolutions`/`MentionResolution` (no `Client`, no `async`, no I/O — owned by `adf.rs` since both the producer, `find_mention_candidates`, and the consumer, `markdown_to_adf_with_mentions`, live there; see ADR-0023 §2). `adf.rs::AdfRenderer::render_node` also gains a new pure `"mention"` reverse-render match arm (BC-7.2.019) — no purity reclassification, `render_node` was already pure. Effectful-shell additions — new module `cli::issue::mentions::resolve_mentions(client, text, no_input)` (async; `@Name` search + `disambiguate_user` reuse + bracket-form accountId preflight-validation, all deduplicated per unique candidate before any HTTP call), classified alongside `cli::issue::helpers`/`cli::issue::field_resolve` as an L2 CLI-layer support module (not a top-level command handler). No existing module's purity classification changes — `cli::issue::{create,edit,interactions,jsm_create}` remain effectful shell (unchanged classification; they gain one new `.await` call to `mentions::resolve_mentions` immediately before their existing `markdown_to_adf` call site), and `api::jira::users::{get_user,search_users}` are reused verbatim, zero code changes. `cli::issue::helpers::disambiguate_user` changes ONLY in visibility (`fn` → `pub(super) fn`) — it remains classified exactly as it already was (effectful shell, inside the effectful `cli::issue` module), with zero behavior change to its three existing callers. See ADR-0023 (the full pure/effectful seam rationale, including why `@Name` resolution must not live in the pure converter) and `component-graph.md §Markdown Mentions Delta` for the full DAG/purity cross-check.
